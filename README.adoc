...
When a PR with a label (e.g. **Release**) is merged, course metadata is posted to Asana using config in `src/commands/asana/` (e.g. `release.json`). Each config file has a `destinations` array: each entry needs a project GID, and optionally a section GID and assignee GID.
