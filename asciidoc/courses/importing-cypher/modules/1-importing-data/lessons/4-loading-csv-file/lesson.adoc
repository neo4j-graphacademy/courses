...
In this lesson, you'll use the link:https://neo4j.com/docs/cypher-manual/current/clauses/load-csv/[`LOAD CSV`] Cypher clause to load a CSV file.
...
The `LOAD CSV` clause has the following syntax:
...
The `FIELDTERMINATOR` wasn't specified in the `LOAD CSV` clause because the default value is a comma. If the field terminator is a pipe character, the `LOAD CSV` clause would need to include the field terminator:
