...
In the examples you have been exploring, a single file represents a single entity in the graph. For example, `persons.csv` contained `Person` nodes and `movies.csv` contained `Movie` nodes. In this lesson, you will learn how to import data from a single file into multiple entities in the graph. You will also learn how to make multiple passes over a single file to create the data structure.
