...
image::images/persons_notepad.png[A data model showing a Book and Author node, with a WROTE relationship between them.]

The data is relatively simple and a single Cypher query could be used to imported it into the graph:
...
(TODO - get Adam to check this is correctly described)
...
You can read more about the link:https://neo4j.com/docs/cypher-manual/current/execution-plans/operators/#query-plan-eager[Eager operator in the Neo4j documentation^] and how to avoid it while importing data in the link:https://www.markhneedham.com/blog/2014/10/23/neo4j-cypher-avoiding-the-eager/[Avoiding the Eager Operator^] blog post.
