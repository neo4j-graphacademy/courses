...
* The optional clause `IF NOT EXISTS` is used - without which Neo4j would raise an error if the constraint already existed.
...
.Drop a constraint
====
If you never need to drop a constrain you can use the following Cypher statement.
...
In the next lesson, you will import `Movie` nodes to the graph.