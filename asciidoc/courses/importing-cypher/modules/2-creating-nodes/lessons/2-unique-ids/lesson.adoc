...
The `IF NOT EXISTS` clause is also optional. If you do not specify it, Neo4j will throw an error if the constraint already exists.
...
* The optional clause `IF NOT EXISTS` is not used. Neo4j will raise an error if the constraint already exists.
* It applies to all nodes with the `Person` label.
* It requires the `tmdbId` property to be unique.

1. Run the Cypher statement to create the constraint:
+ 
[source, cypher]
----
CREATE CONSTRAINT Person_tmdbId
...
REQUIRE x.tmdbId IS UNIQUE
----
2. You can check that the constraint has been created by running the following Cypher statement:
+
[source, cypher]
----
SHOW CONSTRAINTS
----
3. You should see the constraint named `Person_tmdbId` in the results.

If you were to try to create a `Person` node with a duplicate `tmdbId` property value, Neo4j would raise an error.
...
You can drop a constraint using the following Cypher statement:
