...
The `Person` nodes `born` and `died` properties are both dates. You can use the `date()` function to convert the strings to date values.
...
Run the query to update the `born` and `died` properties to be `Date` values.
...
====
Using the `Date` data type allows you to extract the year, month, and day from the date. For example, 
