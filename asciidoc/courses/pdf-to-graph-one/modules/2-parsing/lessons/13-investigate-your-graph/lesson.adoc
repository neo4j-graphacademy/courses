...
[.col]
====
This is a simple aggregation -- but notice we're querying the **Mailbox**, not the User. A user may have multiple mailboxes, which we'll resolve in Course 3.
====
...
image::images/metadata_graph_limits.svg[A metadata graph on the left with three question marks pointing to gaps: topics, entities, threads -- answered by Course 2 on the right]

[.slide]

== What's next

Course 2 adds thread decomposition, chunking, and entity extraction -- the layers that turn your metadata graph into a full knowledge graph.
...
* The metadata graph is the foundation -- Course 2 adds thread decomposition and entity extraction on top
* The `method` property is your audit trail -- use it to investigate LLM-parsed records if results look unexpected

**Next course:** Thread decomposition, chunking, and entity extraction.
