...
In this course, you'll go from raw email PDFs to a structured metadata graph in Neo4j.

You'll extract text, parse it into structured records, and import the results -- producing a graph of people, mailboxes, domains, and emails ready for entity extraction in Course 2.
...
. **Email PDFs to Knowledge Graph** (this course): PDF -> structured metadata graph
. link:/courses/email-kg-two/[Entity Extraction^]: Thread decomposition, chunking, and entity extraction
. link:/courses/email-kg-three/[Entity Resolution^]: Deduplicate and resolve entities
...
link:https://codespaces.new/neo4j-graphacademy/email-kg-workshop-notebooks[Open in GitHub Codespace^,role="btn"]
...
* You're building a metadata graph from raw email PDFs -- Domain, Mailbox, User, and Email nodes
