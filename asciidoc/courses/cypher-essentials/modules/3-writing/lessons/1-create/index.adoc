...
In this block, we have created the _ACTED_IN_ relationship between the _Michael Cain_ node and the _Dark Knight_ node and set the _roles_ property for this relationship to an array of values - containing one value, _Alfred Penny_.
...
Here we use the variable `r` to reference the relationship that is created. After it is created, we can set the _roles_ property.
