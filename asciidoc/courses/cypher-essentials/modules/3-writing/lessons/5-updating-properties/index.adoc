...
In this lesson you will learn how to write Cypher statements to update properties of nodes and relationships.
...
In this code,  the actor, _Michael Cain_ exists but the movie, 'Batman Begins' does not. We find the _Person_ node and we create the _Movie_ node.
...
In the context of particular `MERGE`  or `MATCH` statement, you can set property values.
...
If you have a reference to a node or relationship, you can also use `SET` to modify the property. For example, if we wanted to mnodify _Michael Cain's_ role to be something different, we could do the following:
...
include::questions/1-set-keyword.adoc[]

include::questions/2-add-property.adoc[]
