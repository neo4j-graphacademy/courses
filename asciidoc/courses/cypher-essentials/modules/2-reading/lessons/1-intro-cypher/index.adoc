...
In this example, the entities are _Person_ nodes.
...
The two nodes in this pattern are _Movie_ nodes and _Person_ nodes.
...
[TIP]
Click the the **Run Query** button to execute the Cypher and see the results.
...
Our _Person_ nodes all have a name property.
We can use the __braces__ to specify the key/value pair of `name` and `Tom Hanks` as the filter.
...
For example, to return the _name_property we  use `p.name`.
...
This query  return the same data as the previous query.
