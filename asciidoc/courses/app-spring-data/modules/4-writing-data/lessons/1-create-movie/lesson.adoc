...
You will need to pass values for the new movie object you want to create. While the `Movie` domain class contains several fields, most are not required, so you can pick and choose what to send. For this example, you will send the `movieId` and `title` fields, which are outlined in a sample `movie.json` file. Create a new file in the `src/main/resources` directory of your project, then copy/paste the contents of the `movie.json` file below.
...
Test the `save()` method on the command line. Open a terminal window and run something like the following command:
