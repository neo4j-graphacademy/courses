...
Now update the existing `findAllMovie()` method in the `MovieController` class to call the new query method in the return.
...
Next, the controller's `findAllMovies()` method is modified to use the new custom method instead of `findAll()`.
