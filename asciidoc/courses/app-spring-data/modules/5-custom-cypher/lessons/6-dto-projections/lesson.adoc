...
First, you need to create a DTO class that contains the few properties. Open the `src/main/java/com/example/appspringdata` folder and add a new class named `MovieDTOProjection`. Then add the properties `title`, `released`, `poster`, and `castSize`.
