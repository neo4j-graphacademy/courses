...
When we were saving new movies back in Module 4, we only sent a few values, making our return results show a lot of `null` values. What if more of our data contained empty values or we wanted to provide a page for users to scroll through all movies, but didn't want to display every field? 
