...
We are going to set a property in the database when a movie is created or updated. While we could do this programmatically by adding a new field to the `Movie` class, this is a background field that the user does not need. Instead, we will use a custom Cypher query to avoid cluttering our domain class on read operations.
