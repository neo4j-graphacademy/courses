...
It's working! However, if you look at the data in the database, we are missing the `role` property on the relationships. You will map those by creating a separate entity class for the relationship to include relationship properties.

[TIP]
.Find the `role` property in the database
====
You can open Neo4j Browser and run the following query to view the relationships and the `role` property we have not yet mapped.
