...
In the `src/main/java/com/example/appspringdata` directory, create an interface called `PersonRepository` that extends the `Neo4jRepository` interface with the appropriate domain and identifier types.
...
In the same directory, add a new class called `PersonController` with annotations and create methods to implement `findAll()` and `findById()` for `Person` entities.
