...
Spring Data also implements a few standard methods, such as `save`, `findById`, `findAll`, and `deleteById`. This is why you don't need to define queries and methods to start out. You will see how to define custom implementations later in this course.
...
3. Create a constructor to inject the repository.
4. Define two methods to handle `GET` requests:
   * The first method (name it `findAllMovies`) will handle requests to the `/movies` endpoint and return a list of movies.
   * The second method (name it `findMovieById`) will handle requests to the `/movies/{movieId}` endpoint and return a single movie.
...
You have now created a domain class, repository interface, and controller class. Next, you will run the application and test it out.

== Check Your Understanding

include::questions/1-derived-methods.adoc[leveloffset=+1]

[.summary]
== Summary

In this lesson, you tested accessing the `Movie` entities from your application - domain, repository, and controller classes.
