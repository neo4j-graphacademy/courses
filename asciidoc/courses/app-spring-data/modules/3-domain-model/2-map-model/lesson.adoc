...
----
<<<<<<< HEAD
include::{sdn-repository-raw}/main/example/results.java[tag=model]
----
====

In the application model, we end up with two main entities - `Order` and `Person`, then we create two sub-entities of `Person` - `Customer` and `Employee`. This looks very similar to the graph model. The `@Node` annotation tells Spring that this entity should map to a node entity in the database. Similarly, an `@Relationship` annotation should map to a relationship entity in the database.
=======
class Order {
