...
You can verify that the connection details are set up correctly by running the application. Running the application as-is will only result in an error if general syntax is incorrect. However, it will not test whether you can actually connect to the instance or not.
...
----
include::code/AppSpringDataApplicationTests.java[indent=0]
----
...
Run the test class in the IDE to verify that the test passes. If it doesn't, then the database credentials or connection details are incorrect.

//TODO: Update with Gitpod screenshot!
