...
[source,cypher,role=nocopy]
----
MATCH path = (a:Actor {name: 'Liam Neeson'})
    -[:ACTED_IN]
        ->(m:Movie)<-
    [:RATED]-(u:User)
RETURN path
...
// Create a subgraph
MATCH (source)
WHERE source:User OR source:Movie OR source:Genre
OPTIONAL MATCH (source)-[r]->(target)
WHERE target:User OR target:Movie OR target:Genre
...
----
// Run an algorithm
CALL gds.leiden.mutate('rec-simple', {
...
[source,cypher,role=nocopy]
----
// Use the algorithm's results to gain actionable insights
CALL gds.graph.nodeProperties.stream('rec-simple', 'communityId')
    YIELD nodeId, propertyValue

WITH gds.util.asNode(nodeId).name AS node, 
