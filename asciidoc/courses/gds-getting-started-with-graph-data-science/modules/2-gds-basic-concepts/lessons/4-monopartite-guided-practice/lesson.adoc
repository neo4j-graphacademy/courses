...
[source,cypher]
----
MATCH (source:Movie)<-[:ACTED_IN]-
  (:Actor)-[:ACTED_IN]->(target:Movie)
WITH gds.graph.project(
  'movies-only',
  source,
  target,
...
[source,cypher]
----
MATCH (source:Genre)<-[:IN_GENRE]-
  (:Movie)-[:IN_GENRE]->(target:Genre)
WITH gds.graph.project(
  'genre-cooccurrence',
  source,
  target
...
NOTE: Throughout these exercises, you'll use `.stream()` mode to run algorithms and view results. Don't worry if the syntax isn't fully clear yet—Module 3 will teach execution modes in detail. For now, focus on understanding the projection patterns and what insights each graph structure can reveal.

Run the command below to see how degree centrality ranks the Movies in the 'movies-only' graph.

[source,cypher]
----
CALL gds.degree.stream('movies-only')
YIELD nodeId, score
...
Take a look for yourself on link:https://www.imdb.com/title/tt0101393/fullcredits/[IMDb].
...
[source,cypher]
----
CALL gds.pageRank.stream('movies-only')
YIELD nodeId, score
RETURN gds.util.asNode(nodeId).title AS movie, 
  score AS collaborations
ORDER BY score DESC
