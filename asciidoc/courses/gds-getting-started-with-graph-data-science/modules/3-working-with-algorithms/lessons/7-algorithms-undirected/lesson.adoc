...
[source,cypher]
----
CALL gds.graph.drop('actor-network', false) YIELD graphName
RETURN graphName
----
...
[source,cypher]
----
CALL gds.pageRank.stream('actor-network-directed', {})
YIELD nodeId, score
...
[source,cypher]
----
CALL gds.pageRank.stream('actor-network-undirected', {})
YIELD nodeId, score
RETURN gds.util.asNode(nodeId).name AS actor, score
ORDER BY score DESC
LIMIT 10
...
[source,cypher]
----
CALL gds.leiden.stream('actor-network-directed', {})
...
[source,cypher]
----
CALL gds.leiden.stream('actor-network-undirected', {})
YIELD nodeId, communityId
WITH communityId, COUNT(*) AS size
RETURN communityId, size
ORDER BY size DESC
...
CALL gds.leiden.stream('actor-network-undirected', {})
YIELD nodeId, communityId
WITH communityId, collect(????) AS actors, count(*) AS size
ORDER BY size DESC
LIMIT 1
...
[source,cypher]
----
CALL gds.leiden.stats('actor-network-undirected', {
  maxLevels: 1
})
