...
First, use the `m.released` property to created an `ACTED_IN_{year}` relationship between the Actor and the Movie:
...
Then, use the same property to created a `DIRECTED_{year}` relationship between the Actor and the Movie:

.2. Create ACTED_IN_{year} Relationships
[source,cypher]
----
...
If the numbers do not match, please refresh your browser to reset the lesson.
====