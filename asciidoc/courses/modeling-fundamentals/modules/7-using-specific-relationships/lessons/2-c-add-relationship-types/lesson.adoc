...
:sandbox: true
:verify: MATCH (:User)-[r:RATED_5|RATED_4|RATED_3]-() RETURN count(r) = 5 AS outcome

In the previous lesson, you added a number of specialized relationships to the graph for the _ACTED_IN_ and _DIRECTED_ relationships.
...
Unlike the refactoring where we removed the _generes_ and _languages_ properties from the nodes, we will not remove the _rating_ property from the _RATED_ relationship.
...
. Rewrite and run the query using the specialized relationships for the question:  *What reviewers gave a movie a rating of 5?*

Copy this query into the query pane, modify it to refactor the graph as described above, and then run it:

[TIP]
--
Change the from node to be _User_, the relationship to _RATED_ and use `toString(r.rating)` to dynamically create the relationship.
--
...
Modify and run this query for the domain question so that it uses the specialized relationships.
...
In this challenge, you demonstrated that you can refactor the graph to specialize relationships.
