...
With the refactoring of the graph, we rewrite the above 2 queries for our use cases.
...
This last use case is one where we don't even need to retrieve any Movie nodes to test the use case which is a huge performance improvement for a large graph.
