...
This is in contrast to join tables where the data must be duplicated so that the relationships in the data can be queried.
...
=== New question of our domain

We have a new question that we want for a use case of our domain.
...
So we now have a query to support our new question:
...
There will only be one node with the language value of _English_.
...
[source,cypher]
----
MATCH (m:Movie)-[:USES_LANGUAGE]-(l:Language)
WHERE  l.name = 'Italian'
RETURN m.title
----

This is the only question that deals with languages so we need not retest all of our queries after the refactor.
...
In the next challenge, you perform a refactoring that eliminates duplication and test it.