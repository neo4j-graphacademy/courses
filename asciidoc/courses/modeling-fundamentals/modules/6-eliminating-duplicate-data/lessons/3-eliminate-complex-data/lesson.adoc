...
. Duplicate data. Many nodes may have production companies in a particular location and the data is repeated in the many nodes.
. Queries related to the information in the nodes requires that all nodes be retrieved.

=== Refactoring complex data

If there is a high amount of duplicate data in the nodes or if key questions of your use cases would perform better if all nodes need not be retrieved to get at the complex data, then you might consider refactoring the graph to be:
...
include::questions/1-complex-data.adoc[]
