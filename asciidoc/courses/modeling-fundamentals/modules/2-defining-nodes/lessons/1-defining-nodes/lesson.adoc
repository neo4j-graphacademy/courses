...
In addition to the ID that is used to uniquely identify a node, we must revisit the use cases to determine the type of data a node must hold.
...
Run this Cypher code to add the Person and Movie nodes to the graph which will serve as our instance model:
