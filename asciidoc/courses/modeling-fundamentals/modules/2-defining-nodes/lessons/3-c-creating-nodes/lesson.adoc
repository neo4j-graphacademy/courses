= Creating a Node
:type: challenge
:order: 3
:usecase: blank-sandbox
:sandbox: true
////
:verify: MATCH  (u:User) WHERE u.name = 'Sandy Jones' AND u.userId:534

               MERGE (:User {name: "'Clinton Spencer'", userId: 105})
                RETURN true AS outcome
////

We want to add a couple of _User_ nodes to the graph so we can do some testing of our model.

The properties that a _User_ node will have are:

* userId (integer)
* name (string)

Use the sandbox window to the right to:

. Create two  User nodes for:
.. [copy]#Sandy Jones# with the userId of [copy]#534#
.. [copy]#Clinton Spencer# with the userId of [copy]#105#


[TIP]
--
Use `MERGE` to create these two nodes.

--

include::questions/verify.adoc[]

[.summary]
== Summary

In this challenge, you demonstrated that you can create some nodes to support your instance model.

Your instance model should now look like this:

image::images/after-challenge2-instance-model.png[Instance Model thus far,width=400,align=center]

In the next module, you will how to add relationships to your model.
