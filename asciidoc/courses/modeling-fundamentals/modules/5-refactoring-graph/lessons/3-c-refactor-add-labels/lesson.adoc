...
:verify:  MATCH (d:Director) WHERE d.name = 'Danny DeVito' OR d.name = 'Junk Director' RETURN count (*) = 2 as outcome

In the previous lesson, we added the _Actor_ label to the nodes in the graph.

Modfy the query in the sandbox query pane to label the _Person_ nodes that have the outgoing relationship of _DIRECTED_ to be labeled _Director_.
