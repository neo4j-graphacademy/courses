...
Furthermore, both of these relationship type start at _Person_ nodes and end in _Movie_ nodes.
...
For example, we saw in the _Neo4j Fundamentals_ course that the property can be added to a relationship to further describe it.
...
Just like you analyze the questions for your domain for naming labels, relationship types, and node properties, you use the questions to come up with properties for relationships.

Here is a question of our domain:

What role did a person play in a movie?

The details for this question are:
...
We know that the _role_ for a particular _ACTED_IN_ relationship will answer this question.
...
And here is the instance model;
...
In this lesson, you learned how to identify the relationships (and their properties) that will be added to your graph data model based upon the questions for your use cases.
In the next challenge, you will further develop the model by adding another relationship type to the model.