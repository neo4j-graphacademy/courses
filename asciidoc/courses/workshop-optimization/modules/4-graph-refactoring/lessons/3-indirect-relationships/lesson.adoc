...
. Find the `Employee`, and `Region` nodes through the existing `IN_TERRITORY` and `IN_REGION` relationships.
. Create a new `PART_OF_REGION` relationship between the `Employee` and `Region` nodes.
. Return the employee, the new relationship, and the region to verify
