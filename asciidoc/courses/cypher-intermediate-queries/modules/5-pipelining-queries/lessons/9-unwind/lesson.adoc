...
For each Movie node, it unwinds the languages list to create a list called languages.
Note that we use `DISTINCT` to only add unique language names to the list.
Notice also that we use the trim() function to ensure there are no extraneous whitespace characters in the language name.
After we have collected all the languages, we unwind that list.
