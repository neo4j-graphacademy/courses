...
In this query, we find all `:Person` nodes with the name, Tom Hanks. We then traverse the `:ACTED_IN` relationships to all `:Movie` nodes and filter for movies with a `year` property equal to 2013.

We are specifying the pattern to traverse through thr graph, and then filtering on what data is retrieved within that pattern.
...
// TODO: Elaine, are the commas correct here?  "acted in the movie, Captain Phillips"
...
// TODO: Elaine, we should mention this before ranges
...
WITH 10 AS number
RETURN
    number < 5 AS lessThan, // Less Than (false)
    number <= 10 AS lessThanOrEqual, // Less Than or Equal (true)
    number > 10, AS moreThan, // More Than (false)
    number >= 10 // More Than or Equal (true)
----
...
// TODO: Elaine, rather than repeating "you can also...", can we instead explain the operators and/or process?

// TODO: Elaine, Testing numbers? Explain greater than/Lesson than first?

To test for property values within a range, you can use a combination of less than and greater than above.

You can test a range of values:
...
Depending on your data model, it may be useful to test that a label has a label.
...
----
MATCH (p:Person)
WHERE  p.born.year > 1960 AND p:Actor AND p:Director
...
Rather than using the Actor or Director labels, it uses the relationship types `:AXCTED_IN` and `:DIRECTED` to imply that the node at the other end of the relationship has the correct label:
...
// TODO: Elaine, I don't think we need to be so specific with the number of results?
// It returns 8 rows.
