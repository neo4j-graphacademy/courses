...
You can also test inequality of a property:
...
Rather than using the Actor or Director labels, it uses the relationships to the movie which imply the labels in the graph:
...
This query retrieves all people after 1960, who acted in a movie and who also directed the same movie.
...
This query returns 132 rows, but notice that some people directed and acted in multiple movies, for example, Ben Stiller.
