...
m.title = 'Captain Phillips'
RETURN p.name
----


// DONE: Elaine, are the commas correct here?  "acted in the movie, Captain Phillips"
// took out comma
...
// DONE: Elaine, we should mention this before ranges
// took out WITH clause as they have not learned yet and modified the query
...
RETURN
    m.year < 1995 AS lessThan, // (false)
    m.year <= 1995 AS lessThanOrEqual, // (true)
    m.year > 1995 AS moreThan, // (false)
    m.year >= 1995 AS moreThanOrEqual // (true)
----

=== Testing Ranges

// DONE: Elaine, rather than repeating "you can also...", can we instead explain the operators and/or process?first?
...
// DONE: Elaine, I don't think we need to be so specific with the number of results?
// leave this commmented out: It returns 8 rows.
