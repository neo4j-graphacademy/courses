...
In a `CALL` clause, you specify a query that typically returns a node or set of nodes.
The set of nodes returned in the `CALL` clause can be used by the enclosing query.
...
Then within the subquery, the query executes to find all users who gave that movie a rating of 5.
...
In this lesson, how to call a subquery in Cypher that will help reduce memory requirements for your queries, as well as `UNION`.
