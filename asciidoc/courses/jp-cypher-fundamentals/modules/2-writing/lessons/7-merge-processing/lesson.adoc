...
もし、パターンが見つからなければ、データを作成することができる。

=== `MERGE` の動作のカスタマイズ

ノードが作成されたときやノードが見つかったときにプロパティを設定できるように、実行時の振る舞いを指定することもできます。
...
このCypherのコードを少なくとも2回実行して、どのようなプロパティが設定されるかを観察してください。プロパティはテーブルビューで見ることができます。
...
例えば、_The Cider House Rules という映画を表す _Movie_ ノードを作成したいとします。
次のようなクエリを書きます。
...
1.Neo4jは、_Michael Cain_という名前を持つ_Person_ノードを見つけようとします。
2.2. 次に、グラフ内の_ACTED_IN_リレーションを展開しようとします。
3.3. 関係のもう一方の端にあるノードについて、_Michael Cain_ノードをチェックします。

現在のデータベースの状態では、_Person_ノードはありますが、_Movie_ノードは**ありません**。
そのため、Neo4jはパターン全体を作成しようとします。まず、新しい_Person_ノードを作成し、次に_Movie_ ノードを作成し、その後にリレーションシップを作成します。

これは、このコードによって一意性制約に違反することを伝えるエラーをスローする原因となります。

代わりに、このコードを3つの部分に分解することができます。  まず_Person_ノードをマージし、次に_Movie_をマージし、そして2つの間の_ACTED_IN_リレーションシップをマージします。
