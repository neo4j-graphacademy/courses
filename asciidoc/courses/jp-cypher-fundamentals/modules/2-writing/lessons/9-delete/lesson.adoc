...
Jane Doe_の_Person_ノードを作成したとします。
...
このコードは1つのノードを作成し、_Jane Doe_から_The Matrix_へのリレーションシップを作成します。
...
Jane Doe_ノードを削除しようとすると、グラフ内にリレーションシップがあるため、エラーが発生します。
...
このコードでは、リレーションシップと_Person_ノードを削除しています。
...
Jane Doe ノードには Person と Developer という 2 つのラベルがあります。そのノードを見つけるために `MATCH` を使うことができます。
