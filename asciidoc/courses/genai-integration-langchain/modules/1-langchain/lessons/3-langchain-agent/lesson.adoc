...
The context contains weather information about London and San Francisco.
====
...
The agent uses the `OpenAI` LLM and a simple prompt to generate an answer based on the retrieved information.
...
----
+
The functions return data to be updated in the application state.
. The `workflow` that defines the order in which the functions are executed:
[source, python]
----
