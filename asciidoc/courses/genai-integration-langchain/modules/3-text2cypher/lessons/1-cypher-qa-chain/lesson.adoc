...
In this lesson, you will use the Cypher QA (question-answering) chain to turn query the graph using natural language queries.
...
If asked the question "What year was the movie Babe released?", the chain will generate messages similar to:
...
[IMPORTANT]
You may find that the generated Cypher queries are not always optimal or correct, as the LLM may not always understand the graph schema or the question correctly.
...
[source,python]
----
include::{repository-raw}/{branch}/genai-integration-langchain/solutions/cypher_qa_models.py[tag=cypher_mopdel]
----
...
include::questions/1-different-models.adoc[leveloffset=+1]
