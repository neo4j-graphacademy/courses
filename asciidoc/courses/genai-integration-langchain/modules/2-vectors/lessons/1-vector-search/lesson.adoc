...
In this lesson, you will learn how to use vectors indexes with LangChain to perform vector search and Retrieval Augmented Generation (RAG).
...
. Create an embedding model to convert a query into vectors
...
When specifying the vector index you must also state the Node properties that contain the text (`text_node_property`) and the embedding (`embedding_node_property`).
