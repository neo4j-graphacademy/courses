...
The code is similar to the simple agent you explored in the previous module.
...
Run the application and experiment with different questions, for example:

    include::{repository-raw}/{branch}/genai-integration-langchain/solutions/vector_retriever.py[tag=examples]

== Context

The `context` is a list of `Document` objects returned by the `similarity_search` method including the movie plot and all the node properties as metadata.
...
It is often useful in development to output the `context` to see what data is being used to answer the question.

Add the following code to the program to print the `context` after the answer is generated:
