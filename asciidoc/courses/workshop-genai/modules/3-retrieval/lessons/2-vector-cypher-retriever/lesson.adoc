...
The index is configured to 1536 dimensions (as provided by the `text-embedding-3-small` embedding model) and use cosine similarity for distance calculations.
...
WITH ai.text.embed(
    ...
    "OpenAI",
    { token: "sk-...", model: "text-embedding-3-small" }
) AS userEmbedding

MATCH (chunk:Chunk)
SEARCH chunk IN (
  VECTOR INDEX chunkEmbedding
  FOR userEmbedding
  LIMIT 5
) SCORE AS score
...
.OpenAI token
=====
You will need to replace the `$token` with your OpenAI API key.
