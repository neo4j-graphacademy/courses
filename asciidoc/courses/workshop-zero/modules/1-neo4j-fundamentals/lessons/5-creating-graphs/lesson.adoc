...
* A `Movie` node: `MERGE (m:Movie {title: "Arthur the King"})`
* A `User` node: `MERGE (u:User {name: "Adam"})`
* A `RATED` relationship between them, with a `rating` property of `5`: `MERGE (u)-[r:RATED {rating: 5}]->(m)`
