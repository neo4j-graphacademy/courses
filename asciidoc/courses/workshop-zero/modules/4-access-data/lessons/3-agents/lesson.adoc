...
You are going to use the Aura Agents *Create with AI* feature to create an agent that can interact with your Neo4j database.

Your task is to:

. Create a new agent and provide a prompt describing the agent's role and capabilities.
. Use the agent to answer questions about the data in your Neo4j database.
. Add a new tool to the agent to extend its capabilities.

[.slide-only]
====
**Continue with the lesson to create an agent and explore its capabilities**
====

[.transcript-only]
=== Create an agent with AI

. Open the *Agents* Data Service
// +
// console::Open Agents[tool=agents]
...
. Enter the prompt to create the agent : [copy]#You're an expert customer service agent for Northwind Traders, a food distribution company. Your role is to answer questions about customers, orders, and sales.#
+ 
...
Explore the agent's configuration and tools.
...
** Name: [copy]#Search for customer by name#
** Description: [copy]#Search for customers using case insensitive partial name.#
** Add a Parameter: 
*** Name - [copy]#name# 
*** Data Type - `string`
*** Description - [copy]#The name of the customer#
+
...
In this lesson, you create an Aura AI agent that can interact with your Neo4j database. 

You explored the agent's reasoning and tool use, and added a new tool to allow the agent to search for customers by name.

In the next challenge, you can revisit and experiment any of the Aura tools.
