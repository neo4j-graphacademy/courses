...
Download the Northwind dataset from todo link.

== Create a graph data model

You need to create a new blank graph data model and open the Northwind dataset you just downloaded:

. Open the *Import* tool in Aura and select *Graph Models*. 
+
image::images/aura-import-graph.png["Aura Console - Import - Graph Models"]
. Create a new graph data model.
...
Review the data source tables, the fields and try to identify:

- Which tables will become Node labels
...
* The node properties - the `name`, what data `type` is used, and which source table `column` is used for each property.
...
You can view the Customers in the graph using Query:
