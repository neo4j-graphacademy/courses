...
image::images/fastrp_concept.svg[Graph properties transformed into vector embeddings, highlighting centrality and community detection.]
...
[.col]
====
image::images/embeddings_concept_details.svg[Nodes in a graph with corresponding embeddings showing similarity and structure encoding.]
====
...
[.col]
====
image::images/fastrp_similarity_citations.svg[FastRP encodes node similarity through shared citations without direct links.]
====
...
====
image::images/fastrp_initialization.svg[A graph with 5 nodes, each labeled with a random vector like 0.2, -0.1, 0.8 The vectors are shown as small bar charts next to each node, with random positive and negative values.]
====
...
image::images/fastrp_propagation.svg[The same 5-node graph with arrows showing information flowing along edges. Node A's vector is being averaged with vectors from its neighbors B and C. Dotted lines show the flow of vector information between connected nodes.]
...
image::images/iteration_weights.svg[Three stacked vectors labeled Iteration 1, Iteration 2, and Iteration 3, with weights 0.0, 1.0, and 1.0 shown next to each. The weighted vectors are being summed together into a single combined vector on the right.]
...
image::images/fastrp_projection.svg[A tall vector with many dimensions on the left labeled 'Combined vector (1435 dims)' being compressed through a funnel-shaped random projection matrix into a shorter vector on the right labeled 'Final embedding (128 dims)'.]
...
image::images/embeddings_concept_details.svg[Similar nodes appear close together in the embedding space.]
...
[.col]
====
Our 1,433-dimensional word frequency vector, plus: 

* Betweenness
* PageRank

becomes a 1,435-dimensional scaled feature vector.
====

[.col]
====
image::images/scaled_properties.svg[Multiple input properties combined into a single scaled feature vector.]
====
...
[.col]
====
image::images/kmeans_clustering.svg[K-Means clustering algorithm.]
====
