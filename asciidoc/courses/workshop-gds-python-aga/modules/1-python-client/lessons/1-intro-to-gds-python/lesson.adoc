...
Maybe you've completed the link:https://graphacademy.neo4j.com/courses/workshop-gds/?ref=workshop-gds-python-aga[Graph Data Science in Practice^] workshop. Or perhaps you have equivalent experience. Regardless, now it's time to take those same skills into the environment where most real-world data science happens: Python.
...
link:https://codespaces.new/neo4j-graphacademy/gds-workshop-notebooks[Open in GitHub Codespace^,role="btn"]
...
image::images/gds_python_client_connecting.svg[Flowchart showing the GDS Python client connecting Python code to the Neo4j server.]


[.slide]
== Everything still applies

This means everything you've learned about GDS in the Browser still applies. The algorithms haven't changed. The projections work the same way. You're just using a different interface to access them.
...
[.col]
====
image::images/cora_citation_network.svg[Diagram of the Cora citation network with nodes for papers and directed edges for citations.]
====
...
Each algorithm will follow the same pattern: deep-dive on the Movies dataset (which you may already know from the previous workshop), then hands-on practice with Cora.
