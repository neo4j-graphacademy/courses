...
If you've used GDS in the Browser, you've done this using Cypher. Now you'll do the same thing in Python.

[.slide]
== From Cypher to Python

In the Browser, you would write Cypher projections like this:
...
.Connect
----
from graphdatascience import GraphDataScience # <1>

gds = GraphDataScience(uri, auth=(username, password)) # <2>
----

[source,python,role=noplay nocopy]
.Project - Run - Analyze - Drop
----
G, _ = gds.graph.project( # <3>
    "movies-graph",
    ["Actor", "Movie"],
    {"ACTED_IN": {"orientation": "UNDIRECTED"}}
)
df = gds.degree.stream(G) # <4>
...
[.col]
====
<1> Import the GraphDataScience class
<2> Create a GraphDataScience instance with the connection URI and credentials
<3> Project with `UNDIRECTED` orientation so edges flow both ways — required for many centrality algorithms
<4> Stream results into a DataFrame for immediate analysis
<5> Always drop the projection and close the connection when finished
====
