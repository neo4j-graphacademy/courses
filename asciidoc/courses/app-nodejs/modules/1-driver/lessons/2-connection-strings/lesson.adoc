...
Variations of the `bolt` scheme can be used to connect directly to a single Neo4j instance within a clustered environment.  This can be useful if you have a single server configured for data science or analytics.
...
If everything is successful, the driver will then communicate with the server and figure out the best way to execute each query.

You do not need to do any additional configuration when connecting to a single instance or a cluster of servers.
...
In this lesson, we learned how to configure the driver.
