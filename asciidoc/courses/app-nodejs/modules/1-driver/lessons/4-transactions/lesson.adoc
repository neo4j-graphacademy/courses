...
[TIP]
.Default Access Mode
By default, the `defaultAccessMode` is set to `WRITE`, but this can be overwritten by the transaction call.
...
When you intend to read data from Neo4j, you should open a Read transaction.  In a clustered environment or on Neo4j Aura, read queries are distributed across the cluster.

The session offers a `readTransaction` method, which in turn takes a function which represents the unit of work.  The function will accept a single parameter, a Transaction, on which you can call the `run` method with two arguments: the cypher query as a string and an optional object containing a set of parameters.
...
If anything goes wrong in the duration of the unit of work or there is a problem on Neo4j's side, the transaction will be automatically rolled back and the database will remain in its previous state.
...
In a clustered environment, write queries are sent exclusively to the leader of the cluster.  The leader of the cluster is then responsible for processing the query and synchronising the transaction across the cluster.

The call is similar to a read transaction, but instead you would call the `writeTransaction` method.
...
async function createPerson(name) {
  // Create a Session for the `customers` database
  const session = driver.session({
    database: 'customers',
  })
