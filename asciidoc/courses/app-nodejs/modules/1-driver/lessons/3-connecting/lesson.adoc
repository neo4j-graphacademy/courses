...
As we discussed in the link:../1-about/[About the Driver lesson], it is best practice to create a single instance of the driver in our application.
...
// Adam: we don't want this block here do we?
.src/neo4j.js
[source,js]
----
include::{repository-raw}/main/src/neo4j.js[tag=driver]
...
// Adam: server will fail to start?  How about the driver object will not be instantiated
...
//Adam:  before the server starts --> before the application starts
