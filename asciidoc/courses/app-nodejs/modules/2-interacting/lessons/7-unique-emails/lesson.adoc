...
In the link:../5-registering[Registering a User challenge^], you updated the `register()` method in the link:./lab/[`AuthService`^] to create a new User node in the database.
...
lab::Open `src/services/auth.service.js`[]

In the `authenticate()` method, you will need to:
