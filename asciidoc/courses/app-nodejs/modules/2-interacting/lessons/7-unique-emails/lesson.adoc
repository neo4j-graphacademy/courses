...
In the link:../5-registering[Registering a User challenge^], you updated the `authenticate` method in the link:{repository-blob}main/src/routes/auth.services.js[`AuthService`^] to save new User node in the database.

There is still one `// TODO` comment remaining in the service.
