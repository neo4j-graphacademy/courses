...
In the link:../5-registering[previous challenge^], you updated the `authenticate` method in the link:{repository-blob}main/src/routes/auth.services.js[`AuthService`^] to save new User node in the database.


But there is still one `// TODO` comment remaining in the service.
...
Instead, can pass the responsibility of handling the duplicate user error to the database by creating a Unique Constraint on the `:User` label, asserting that the `email` property must be unique.
...
Any errors will be sent back up the stack and will most likely result in a `500 Bad Request` error.

You will also need to rearrange the code slightly into `try`/`catch` blocks.
...
This will allow the front end to
...
To complete this challenge, you will need to create a new constraint in your Sandbox and modify the code to add a try/catch block
...
If you wanted to go further, you could use Regular Expression to extract more specific information about the `ConstraintValidationFailed` error.

Now that a user is able to successfully register, let's update the `authenticate` method to find our user in the database.
