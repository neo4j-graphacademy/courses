...
The connections themselves are managed internally by the driver and are not directly exposed to the user.
...
You cannot create multiple databases in Neo4j Aura.
...
In the query above, the the `$` prefix of `$location` (1) indicates that this value relates to the parameter defined in the second argument (2) of the `run` method call.
...
Additionally, unlike `session.run`, if a _transient_ error is received by the driver, for example a connectivity issue with the server, the driver will automatically retry the unit of work.
...
This method differs from the `readTransaction` and `writeTransaction` methods, in that the transaction will have to be manually commited or rolled back depending on the outcome of the unit of work.
...
====
The following code defines a function that accepts a name parameter, then executes a write transaction to create a `:Person` node in the `peoples` database.
