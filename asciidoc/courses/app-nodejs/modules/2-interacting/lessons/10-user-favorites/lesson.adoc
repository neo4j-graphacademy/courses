...
For the first part of this challenge, modify the `add` method to open a new database session, run the Cypher query to ceate te relationship, close the session and return the movie details along with an additional `favorite` property.
...
In this module, you have written the code to create a `:HAS_FAVORITE` relationship between a User and a Movie within a write transaction.
