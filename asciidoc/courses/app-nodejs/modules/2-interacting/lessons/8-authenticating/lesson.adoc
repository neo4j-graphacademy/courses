...
In this challengem you will need to rewrite the `authenticate` functionality of the link:{repository-blob}main/src/routes/auth.services.js[`AuthService`^] to do the following:
...
4. The strategy creates a new instance of the link:{repository-blob}main/src/routes/auth.services.js[`AuthService`^] and calls the `authenticate` method with the username and password defined in the request.


The `authenticate` method should perform the following actions:
...
In order for the strategy to work correctly, the `authenticate` method must return an object which represents the user on successful login, or return `false` if the credentials are incorrect.
...
To implement database authentication, you will need to modify the `authenticate` method in the `AuthService`.
...
You will need to use a `MATCH` query to find a `:User` node with the email address passed to the method as a parameter.
...
Next, you will need to verify that the unencryted password matches the encrypted password saved as a property against the `:User` node.
...
The code is already written to the example, so this can be repurposed at the end of the method.
...
In this lesson, you have updated the `AuthService` to authenticate a User using the data held in the Sandbox.
