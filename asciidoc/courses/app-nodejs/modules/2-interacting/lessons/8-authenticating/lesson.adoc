...
In this challenge you will rewrite the `authenticate()` function of the link:{repository-blob}main/src/routes/auth.services.js[`AuthService`^] to find the User node with the corresponding email and compare the password before issuing a JWT token.
