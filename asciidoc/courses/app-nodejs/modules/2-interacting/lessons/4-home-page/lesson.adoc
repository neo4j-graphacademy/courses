...
You will modify the `all()` method of the link:{repository-blob}main/src/services/movie.service.js[`MovieService`^] to do the following:
...
This session provides a `readTransaction()` method for which you pass a function to represent the unit of work.
