...
When executin ga Cypher query, certain exceptions and error cases may arise.
This could be a _transient_ error that may be resolved if retried, for example a problem connecting to the database instance, or something more permanent, for example a Syntax Error or a Constraint Error.

In the Neo4j JavaScript Driver, the Promise returned by the `run` method will be rejected, and an instance of a link:https://neo4j.com/docs/api/javascript-driver/current/class/lib6/error.js~Neo4jError.html[`Neo4jError`^] will be returned.

The `Neo4jError` class extends the native JavaScript `Error`, and as such contains a `message` property which contains detailed information on the error that has occurred.

== Error Codes

The `Neo4jError` also includes a `code` property, which provides a higher level information about the query.

Each status code follows the same format, and is formed of four parts:
...
2. The Classification provides high level classification of the error - for example, a client-side error or an error with the database
...
4. The Title provides specific information on the error that has occurred
