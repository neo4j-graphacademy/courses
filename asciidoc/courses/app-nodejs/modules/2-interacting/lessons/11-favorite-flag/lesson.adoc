...
Your next challenge is to modify the link:{repository-raw}/main/src/services/movie.service.js[`MovieService`] to dynamically determine whether a Movie is on the current User's My Favorite List.


== Running Multiple Queries within a Transaction

So far, we have only used the `tx` object within the Unit of Work to run a single query.

This will be fine for the majority of cases, but there may also be occasions where more than one Query may be required.
...
Instead, it would be cleaner to instead execute two separate queries within the same transaction; one query to fetch a list of the User's favorites, and another to retrieve a list of movies.
