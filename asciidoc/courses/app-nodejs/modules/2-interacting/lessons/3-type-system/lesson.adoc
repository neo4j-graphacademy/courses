...
Some values like strings, floats, booleans and nulls have a direct mapping to JavaScript types but more complex types need some care and attention.
...
As we progress through this module, we will use the knowledge gained so far to read data from, and write data back to the database.
