...
These numbers can be re-used, a best practice is to always look up a node by its property rather than relying on an internal ID.
...
//Adam: Is this right?
Each of these types has a `toString()` method that can be used in conjunction with `Date.parse()` to convert the Cypher date into a native `Date` object.
