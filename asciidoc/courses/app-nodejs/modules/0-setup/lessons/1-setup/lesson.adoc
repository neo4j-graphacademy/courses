...
In order to run the project, we will need to make sure that Node.js is installed.
...
Currently, the responses are hardcoded, but as we progress through the course, we will learn how to query Neo4j to find this information.
...
* `public/` - This folder holds the minified build files for the SPA.  Do not edit these files.
...
Now that we have the project up and running, let's take a look at the Neo4j Sandbox instance that has been created as part of our enrollment.
