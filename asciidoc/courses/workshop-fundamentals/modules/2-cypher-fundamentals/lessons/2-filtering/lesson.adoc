...
Movies starring someone with "Robert" in their name:
...
Movies starring someone whose name starts with "Robert":
...
You can use `IS NULL` and `IS NOT NULL` to filter based on null values.
