...
For example, this query retrieves the _Person_ nodes and _Movie_ nodes where the person acted in a movie that was released in 2008 or 2009:
...
Here we want to retrieve  _Person_ nodes of people who acted in movies released between 2000 and 2003:
...
Here is an example of a query where we only want to return _Movie_ nodes where Jack Nicholson acted in the movie, and the movie has the _tagline_ property.
