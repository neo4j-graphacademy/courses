...
The default access mode is set to write, but this can be overwritten by explicitly calling the `read_transaction()` or `write_transaction()` functions.
...
The session provides a `read_transaction()` function, which expects a single parameter, a function that represents the unit of work.
...
This returns a Transaction object identical to the one passed in to the unit of work function when calling `read_transaction()` or `write_transaction()`.

This method differs from the `read_transaction` and `write_transaction()` functions, in that the transaction will have to be manually committed or rolled back depending on the outcome of the unit of work.
