...
Your next challenge is to modify the link:{repository-raw}/main/api/dao/movies.py[`MovieDAO`] to dynamically determine whether a movie is on the current user's My Favorite List.
...
At the bottom of the `MovieDAO`, a placeholder `getUserFavorites()` method exists which is currently hardcoded to return an empty array.
