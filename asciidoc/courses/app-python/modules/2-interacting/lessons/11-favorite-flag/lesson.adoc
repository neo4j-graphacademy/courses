...
Modify the `getUserFavorites` to run the following query against the `tx` object to return a list of IDs for each movie that the user has added to their favorites.
...
The `get_movies` function defined in the `all` method of the `MoviesDAO` already uses `tx.run()` to run a Cypher query within the current transaction.
