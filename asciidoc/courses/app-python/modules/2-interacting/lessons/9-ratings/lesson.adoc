...
In this challenge, you will modify the `add()` method in the link:{repository-blob}main/api/dao/reviews.py[`ReviewDTO`^] to save ratings into Neo4j.
...
The query should return a single row, so the `single()` method can be called to consume the first row from the results.

.Unit of Work
[source,python,indent=0]
----
include::{repository-raw}/main/api/dao/ratings.py[tag=create_rating]
----
...
[source,python,indent=0]
----
include::{repository-raw}/main/api/dao/ratings.py[tag=call_create_rating]
----
...
If the returned value is `None`, either the `User` or `Movie` could not be found, throw a `NotFoundError`.
This will be handled by a Flask middleware.

[source,python,indent=0]
----
include::{repository-raw}/main/api/dao/ratings.py[tag=not_found]
----

Otherwise, use the brackets method to extract the `movie` value from the result and return it.

[source,python,indent=0]
----
include::{repository-raw}/main/api/dao/ratings.py[tag=return]
----
