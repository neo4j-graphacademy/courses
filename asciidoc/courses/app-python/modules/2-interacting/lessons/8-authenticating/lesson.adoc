...
In this challenge you will rewrite the `authenticate()` function of the link:{repository-blob}main/api/routesauth.services.py[`AuthDAO`^] to do the following:
...
This token is then stored by the UI and appended in a `Bearer` token.
The API then decodes the token and uses the .

The token generation happens in the link:{repository}/main/api/dao/auth.py[`AuthDAO`^] using the `_generate_token` method.

This token is decoded by the link:https://pythonhosted.org/Flask-JWT/[Flask-JWT] library.


=== Login Route

When a user submits the login form on the website, a request is sent to `http://localhost:5000/api/login` with a username and password.
...
[source,python,indent=0]
----
include::{repository-raw}/main/api/dao/auth.py[tag=get_user]
----


=== Execute the function within a Read Transaction

After opening up a new session, call the `read_transaction` method to execute the `get_user` function above.

.Call the Transaction Function
[source,python,indent=0]
----
include::{repository-raw}/main/api/dao/auth.py[tag=call_get_user]
----
...
[source,python,indent=0]
----
include::{repository-raw}/main/api/dao/auth.py[tag=user_not_found]
----
...
[source,python,indent=0]
----
include::{repository-raw}/main/api/dao/auth.py[tag=compare_passwords]
----
...
[source,python,indent=0]
----
include::{repository-raw}/main/api/dao/auth.py[tag=auth_jwt]
----
