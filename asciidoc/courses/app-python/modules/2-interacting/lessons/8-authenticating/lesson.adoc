...
In this challenge you will rewrite the `authenticate()` function of the link:{repository-blob}main/src/routes/auth.services.py[`AuthService`^] to do the following:
...
1. A route handler in link:{repository-blob}/main/src/routes/auth.routes.py[`src/routes/auth.routes.js`^] listens for a `POST` request.
2. The `passport.authenticate` middleware triggers the _local_ authentication strategy.
3. This calls the callback function defined in link:{repository-blob}/main/src/passport/neo4j.strategy.py[`src/passport/neo4j.strategy.js`^].
4. The strategy creates a new instance of the link:{repository-blob}main/src/routes/auth.services.py[`AuthService`^] and calls the `authenticate()` method with the username and password defined in the request.
...
----
include::{repository-raw}/main/src/services/auth.service.py[tag=authenticate]
----
...
[source,python,indent=0]
----
include::{repository-raw}/{branch}/src/services/auth.service.py[tag="query"]
----
...
[source,python,indent=0]
----
include::{repository-raw}/{branch}/src/services/auth.service.py[tag="norecords"]
----
...
[source,python,indent=0]
----
include::{repository-raw}/{branch}/src/services/auth.service.py[tag="password"]
----
...
[source,python,indent=0]
----
include::{repository-raw}/{branch}/src/services/auth.service.py[tag="authenticate"]
----
