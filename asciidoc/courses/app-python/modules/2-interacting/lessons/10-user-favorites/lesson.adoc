...
Clicking the link:http://localhost:5000/favorites[My Favorites^] link in the main navigation will take you to page which contains a list of Movies that you can revisit later.
...
----
include::{repository-raw}/main/api/dao/favorites.py[tag=session]
----

=== 2. Define a new Transaction Function
...
There should only be one result, so you can call the `single()` method to get the first result.


.Run the Cypher Query
[source,python,indent=0]
----
include::{repository-raw}/main/api/dao/favorites.py[tag=add_to_favorites_run]
----
...
Otherwise, return the `movie` value from the first record of the result.
...
Call the new method in a new Write Transaction before returning the movie.
...
// Call the `session.read_transaction()` method to execute a new write transaction, this time passing a lambda function to represent the unit of work.
// Pass the `user_id` and `movie_id` as named parameters to the function.
