...
For the first part of this challenge, modify the `add()` method to open a new database session, run the Cypher query to create the `:HAS_FAVORITE` relationship, close the session and return the movie details along with an additional `favorite` property.
