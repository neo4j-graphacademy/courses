...
Here is an exampe query which retrieves a list of `:Person` nodes related to a given Movie.
...
The `result` object obtained using the `tx.run` function returns a `Result` object.
This object provides a number of options to iterate through the stream of _records_.
...
In the example above, the `result` is iterated over within a List Comprehension to extract the `p` value specified in the `RETURN` clause of the Cypher query.


[source,python]
----
include::{repository-raw}/main/example/results.py[tag="get_actor_nodes"]
----
