...
You can find the route handler, the function that handles the request, in link:{repository-blob}main/api/routes/movie.py[`api/routes/movie.py`^]:

.api/routes/movie.py
[source,python,role=nocopy]
----
include::{repository-raw}/main/api/routes/movie.py[tag=list]
----
