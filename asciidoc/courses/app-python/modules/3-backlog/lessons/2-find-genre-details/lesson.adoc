...
When the user clicks a genre in the list, they are taken to a list of movies for that genre.  This list is populated by an API request to `/api/genres/[name]`, for example http://localhost:5000/api/genres/Comedy.
...
----
include::{repository-raw}/main/src/services/genre.service.py[tag=find]
----
...
[source,python,indent=0]
----
include::{repository-raw}/{branch}/src/services/genre.service.py[tag=find]
----
====
