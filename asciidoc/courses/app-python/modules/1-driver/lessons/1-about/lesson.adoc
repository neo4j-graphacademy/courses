...
To connect to and query Neo4j from within a Node.js application, you use the link:https://neo4j.com/developer/python[Neo4j Python Driver^].
...
The Neo4j Python Driver is available through the npm registry and can be installed with `pip`.
...
The `neo4j` package exports a `Python` object.  This object provides a `driver()` function for creating a new driver instance.

The `driver()` function accepts one mandatory parameter, a **connection string** for the Neo4j cluster or DBMS - for example `neo4j://localhost:7687` or `neo4j+s://_dbhash_.databases.neo4j.io:7687`.
