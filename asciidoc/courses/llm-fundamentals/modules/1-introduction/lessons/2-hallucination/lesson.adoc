...
As we learned in the previous lesson, LLMs tend to make things up.
...
Let's take a closer look at some of the reasons why this may occur.

=== Temperature

LLMs are configured with a temperature, that corresponds to the amount of randomness that the underlying model should use when generating the text.
...
For example, a higher temperature may be good when constructing a work of fiction or a novel joke.
...
====
In June 2023, two US lawyers were link:https://www.reuters.com/legal/new-york-lawyers-sanctioned-using-fake-chatgpt-cases-legal-brief-2023-06-22/[sanctioned for submitting an LLM-generated legal brief] that contained six fictitious case citations.
====
...
Large Language Models (LLMs) are often considered "black boxes" due to the difficulty in deciphering their decision-making processes. The complexity of these models combined with potential training on erroneous or misleading data means that their outputs can sometimes be unpredictable or inaccurate.

For example, if asked about a controversial historical event, an LLM might produce an answer that's biased or misrepresentative, largely due to the data it was trained on. Furthermore, it would be near impossible to transparently trace back how the model arrived at that conclusion.
...
If the user is asking a question on information that is hard to find, or outside of the public domain, it will be next to impossible for an LLM to provide an accurate response.

Luckily, this is where a knowledge graph comes in.
...
Prompt engineering is an iterative process of crafting specific and deliberate questions that guide the LLM toward the desired response.
By refining the way questions are posed, developers can achieve better results from existing models without the need for retraining.
...
You could even throw in _"Return the response as JSON"_ and provide an example output to make it easier to parse in the programming language of your choice.

[TIP]
.Be Positive
====
When writing a prompt, I aim to provide positive instructions.
For the same reason a toddler will ignore you telling them not to draw on the walls, the model will act based on what you don't want in mind, rather than what you need.
====
// TODO: better example
...
By presenting a series of relevant examples, the model can deliver more accurate answers, especially for niche or specialized tasks.
...
Fine-tuning involves additional training of a language model on a smaller, task-specific dataset after its primary training phase. This approach allows developers to specialize the model for specific domains or tasks, enhancing its accuracy and relevance.
...
This method is by far the most complicated method, which involves both technical knowledge and domain expertise.
...
Grounding allows a language model to reference external, up-to-date sources or databases to enrich its responses.
