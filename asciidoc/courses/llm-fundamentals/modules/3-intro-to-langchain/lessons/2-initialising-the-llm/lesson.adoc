...
[IMPORTANT]
.Python v3.12+
As of writing, link:Langchain doesn't support Python v3.12 or above[https://github.com/langchain-ai/langchain/issues/11479^]. You can download Python v3.11 from link:https://www.python.org/downloads/[python.org/downloads^].

== Choosing your LLM

Langchain supports multiple LLM providers (link:https://openai.com/[OpenAI^], link:https://cohere.com/[Cohere^], link:https://ollama.ai/[Ollama^]). The quality and cost vary. LLMs are also trained for different purposes, so it is worth experimenting with models and prompts. Depending on your use case, different LLMs may return better results.
...
.Virtual Environment
====
You may find it helpful to create a Python virtual environment such as link:https://virtualenv.pypa.io/en/latest/[virtualenv^] to use during this course.
====
...
You can install the `openai` Python package using `pip
...
* The `llm` is callable; you can pass a string to it and get a response
