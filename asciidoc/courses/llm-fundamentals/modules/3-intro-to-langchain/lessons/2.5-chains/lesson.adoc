...
You can change the prompt to instruct the LLM to return a specific output type. For example, JSON:
...
You can ensure the response is parsed as JSON specifying `SimpleJsonOutputParser` as the `output_parser`:
