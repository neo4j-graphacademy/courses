...
[source,python]
----
include::code/chat-agent.py[tags=**]
----
...
[source,python]
----
include::code/chat-agent.py[tags=tools]
----
. An agent is created that uses the tool:
+
[source, python]
----
include::code/chat-agent.py[tags=agent]
----
. The agent is wrapped in a `RunnableWithMessageHistory` chain that allows it to interact with the memory:
+
[source, python]
----
include::code/chat-agent.py[tags=chat_agent]
----

Update the `Neo4jGraph` connection details and the `openai_api_key` to run the program.
...
[source, python]
----
include::code/chat-agent-trailer.py[tags=import-youtube]

include::code/chat-agent-trailer.py[tags=youtube]
----
...
[source, python]
----
include::code/chat-agent-trailer.py[tags=trailer-search]
----
...
[source, python]
----
include::code/chat-agent-trailer.py[tags=tools]
----
...
[source, python]
----
include::code/chat-agent-trailer.py[tags=**]
----
====
...
As the agent also uses the conversation memory you can refer back to the previous questions, such as finding a trailer for a movie it has recommended:
    ...
    2. [Link 2](https://www.youtube.com/watch?v=cv-dH5gHi1c&pp=ygUQVGhlIFRyZWUgb2YgTGlmZQ%3D%3D)
