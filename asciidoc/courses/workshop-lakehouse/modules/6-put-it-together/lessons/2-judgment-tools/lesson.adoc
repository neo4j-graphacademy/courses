...
vehicles? Ground the candidate parts in the documents, use the
...
in our library at all? Enumerate the codes in the warehouse work
orders, then check each against the document library.
----
...
// CO-WORK: the field-side half (these codes are high-comeback in the warehouse) is a warehouse query the agent runs; the sandbox Cypher above only proves the documentation-absence half. Decide whether the Check-Database verifier covers both halves or just the doc side.
...
and roughly how many cars does each affect? Use the themes shape over
the whole library, then read the warehouse for the fleet counts.
----
...
// CO-WORK: confirm the local similarity-contrast mechanism the learner runs to *feel* the sampling failure (plain search.py / content_search over the broad query -> handful of hits), vs themes covering all 112. Wording below is a draft.
