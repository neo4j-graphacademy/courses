...
----
MATCH (r:Document)
WHERE r.uri STARTS WITH 'technical-library/recalls/' AND r.themeId IS NOT NULL
MATCH (member:Document {themeId: r.themeId})
RETURN r.themeId AS theme, r.displayName AS recall,
...
A trouble code is _not_ a graph node - it lives in section text and in BigQuery, so there is no graph edge from a code to a theme.
The route is: full-text search the code, take the matched section's owning document, read its `themeId`.

[%collapsible]
====
First, find the code in the library text:

[source,shell]
----
python skill/scripts/search.py "P0562"
----

The top hit is a section URI like `technical-library/manuals/man-falcon-charging.pdf#diagnostic-trouble-codes/system-voltage-codes`.
Collapse it to the owning document (`split(uri, '#')[0]`) and read that document's theme and reading list:

[source,cypher]
.Owning document to theme to reading list
----
MATCH (d:Document {uri: 'technical-library/manuals/man-falcon-charging.pdf'})
WHERE d.themeId IS NOT NULL
MATCH (member:Document {themeId: d.themeId})
RETURN d.themeId AS theme,
       collect(DISTINCT member.displayName) AS readingList
----

Search, then one collapse to the owning document, then the theme's full reading list - the same `split(uri,'#')[0]` ownership your projection used.

**Try experimenting:**

* Try `C0035` - search it, take its owning document, and see whether it routes to a different theme
...
In the finale, the warehouse joins - and themes gain an outcomes dimension.
