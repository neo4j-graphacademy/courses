...
RETURN tables = 6 AND refs = 5 AS outcome,
       CASE
           WHEN tables = 0
           THEN 'No Table nodes found. Run the connector: python load/build_connections.py'
           WHEN tables <> 6
           THEN 'Found ' + toString(tables) + ' tables, expected 6. Re-run python load/build_connections.py.'
           WHEN refs <> 5
