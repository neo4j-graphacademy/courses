...
In this lesson, we will cover two of the most commonly used algoriths that come with the GDS library.
...
The following Cypher statement will calculate the shortest weighted path between  *Paris Charles de Gaulle* (CDG) and *Lisbon Portela Airport* (LIS) and stream the results.
...
In the next challenge, you will use Dijkstra's shortest path algorithm to find the shortest weighted path between two nodes.
