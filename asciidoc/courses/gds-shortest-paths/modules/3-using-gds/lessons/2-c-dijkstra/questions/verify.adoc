...
Execute the Dijkstra shortest path algorithm to find the shortest weighted route from Nashville (BNA) to Phuket (HKT).
Make sure you have projected the *routes* graph as indicated in the lesson.
...
The statement returns a single value from the procedure call, `totalCost`, which can be copied and pasted into the textbox above.
