...
In the link:/courses/llm-fundamentals/3-tools/1-vector-tool/[Adding the Neo4j Vector Retriever tool^] challenge, you also learned how to create an instance of the `Neo4jVector` Store.
...
In the `tools/` folder, create a new file called `vector.py`.  Add the following line to import the `Neo4jVector`.
The `llm` and `embeddings` objects created earlier in the course will be required, so import them also.

.Importing the Neo4jVector class
[source,python]
----
include::{repository-raw}/tools/vector.py[tags=importvector,importllm]
----

Because link:/courses/llm-fundamentals/2-vectors-semantic-search/3-c-create-vector-index/[the index already exists in the database^], you can call the `from_existing_index` static method.

.Creating a Neo4jVector
[source,python]
----
include::{repository-raw}/tools/vector.py[tag=vector]
----
...
[source,python]
----
include::{repository-raw}/tools/vector.py[tag=retriever]
----
...
[source,python]
----
include::{repository-raw}/tools/vector.py[tags=importretrievalqa]
----
...
[source,python]
----
include::{repository-raw}/tools/vector.py[tags=qa]
----
...
You may recall in the previous module, that you created an empty `tools` array in `agent.py`.
You can register the tool using the `.from_function()` static method on the `Tool` class.

Start by importing the `Tool` class into `agent.py`.

.Importing the Tool class
[source,python]
----
include::{repository-raw}/tools/vector.py[tags=importtool]
----

Then add the tool to the `tools` array using the `.from_function()` static method.

.Registering the Tool
[source,python]
----
include::{repository-raw}/tools/vector.py[tags=tool]
----

The function expects three arguments:

1. The name of the tool, in this case the `Vector Search Index`.
2. A description that the agent LLM will use when deciding which tool it should use for a particular task.
3. The function to call once this tool has been selected.
