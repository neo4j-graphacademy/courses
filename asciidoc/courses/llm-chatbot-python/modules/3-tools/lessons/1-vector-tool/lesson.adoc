...
A `plotEmbedding` property has been added to the `Movie` nodes in the database, which contains the vector embeddings of the `plot` property, and a vector index, `moviePlots` has been created. 
