...
To get you started, we have created a Chat interface using the link:https://docs.streamlit.io/library/api-reference/chat[Chat elements provided by Streamlit^].
...
We have designed the project to work with Python **v3.11 or higher**.
...
You can use the Github UI to Clone the repository or download a ZIP archive.
...
----
git clone git@github.com:neo4j-graphacademy/llm-chatbot-python.git
----
...
The project has three dependencies, `streamlit`, `langchain` and `neo4j-driver`.
...
[source,python]
----
include::{repository-raw}/main/bot.py[tag=config]
----
...
video::images/working-app.mp4[opts=autoplay]
...
In the next module, you will start writing the code to start interacting with the LLM.
