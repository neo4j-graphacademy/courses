...
[.video]
video::umjiMPiwf3c[youtube,width=560,height=315]

//https://youtu.be/umjiMPiwf3c
...
When you execute `SHOW INDEXES` on the current graph you have been working with, you see that the uniqueness constraints defined in the graph are b-tree indexes.
That is, the implementation of a uniqueness constraint in the graph is as a b-tree, along with the uniqueness lookup characteristics of the index.
