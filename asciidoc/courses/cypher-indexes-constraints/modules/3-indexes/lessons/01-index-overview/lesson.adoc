...
Constraints are implemented as indexes, but there are more types of indexes that you can create and use:
...
This means that not only must you plan what indexes are appropriate for your use cases, but also that the order of your query predicates will impact what indexes are used at runtime.
...
This query could benefit from a b-tree index in the graph for the name property associated with the Person label:
...
[source,cypher]
----
MATCH (m:Movie) WHERE m.year > 1999 AND m.title CONTAINS "Toy"
...
A RANGE index supports node or relationship property types that are numeric.
It is optimized for queries that may specify `>`,`+>=+`,`<`, `+<=+` in their predicates.

[NOTE]
--
This course does not cover RANGE indexes.
After you have learned how to create a TEXT index, you can try creating and testing a RANGE index.
You can read more about RAGE indexes in the https://neo4j.com/docs/cypher-manual/current/indexes-for-search-performance/#_syntax[Cypher Reference Manual^y
--

=== POINT indexes

A POINT index supports node or relationship property types that are point types.
It is optimized for queries that may specify `point.*` or `point()` functions their predicates.

[NOTE]
--
This course does not cover POINT indexes.
You can read more about POINT indexes in the https://neo4j.com/docs/cypher-manual/current/query-tuning/indexes/#administration-indexes-spatial-bounding-box-searches-single-property-index[Cypher Reference Manual^y
--
