...
At runtime, only one index is used.
...
It is optimized for queries that may specify `STARTS WITH`, `CONTAINS, or `ENDS WITH` in their predicates, but is also used for any type of test of a string property.
