...
Identifying the indexes for you graph depends on the most important use cases (queries) of your application.
...
MATCH (p)-[:ACTED]_IN]->(m:Movie)
WHERE t.title CONTAINS $titleSubString
...
Use cases for the application may change so the identifying and creating indexes to improved query performance may be an ongoing process during the lifecycle of your application.
