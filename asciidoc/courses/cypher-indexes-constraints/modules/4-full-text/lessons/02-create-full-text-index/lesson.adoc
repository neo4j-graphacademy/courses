...
A full-text index is designed queries that need to search string properties using possibly complex search predicates.
...
You use the `CREATE` clause to create a full-text index, but what makes full-text indexes different from other index types (b-tree, TEXT, RANGE, POINT), is that they are not automatically used in your Cypher queries.
...
----
CREATE FULLTEXT INDEX <index_name> IF NOT EXISTS FOR (x:<node_label>) ON EACH [x.<property_key>]
----
...
The db hits statistics do not reflect the true db hits because the `PROFILE` will never expose details of a procedure.
...
You will see that the more you execute this query, the shorter the elapsed time until it bottoms out at ~18 ms.
...
[source,cypher,rel="norun nocopy"]
----
CREATE FULLTEXT INDEX <index_name> IF NOT EXISTS FOR ()-[x:<RELATIONSHIP_TYPE>]-() ON EACH [x.<property_key>]
----
...
[source,cypher,rel="norun nocopy"]
----
CALL db.index.fulltext.queryRelationships("<index_name>", "<lucene_query>") YIELD relationship
...
[source,cypher,rel="norun nocopy"]
----
CREATE FULLTEXT INDEX <index_name> IF NOT EXISTS FOR (x:<node_label1> | <node_label2> | ...) ON EACH [x.<property_key1>, x.<property_key2>,...]
----
...
----
PROFILE MATCH (m:Movie)
WHERE (m.plot CONTAINS "british" OR m.plot CONTAINS "British")
...
RETURN a.name AS Name, a.bio AS Bio, a.title AS Title,  a.plot AS Plot
----
...
[source,cypher]
----
CREATE FULLTEXT INDEX Actor_bio_Movie_plot_title_ft IF NOT EXISTS FOR (x:Actor | Movie) ON EACH [x.bio, x.plot, x.title]
----
...
[source,cypher]
----
PROFILE CALL db.index.fulltext.queryNodes("Actor_bio_Movie_plot_title_ft",
"(plot: british AND title: death) OR (bio: british  AND bio: actress)")
YIELD node
RETURN node.name AS Name, node.bio AS Bio, node.title AS Title,  node.plot AS Plot
----

This query returns both movie and actor nodes that match the properties specified in the second argument to queryNodes().
This query returns in 5 ms, much faster than the previous query that used the TEXT indexes.
...
Some of your queries may want to rank how close a pattern is found in the properties for the index.
The db.index.fulltext.queryNodes() procedure can return a score that reflects how close a match a property is to the test string.
