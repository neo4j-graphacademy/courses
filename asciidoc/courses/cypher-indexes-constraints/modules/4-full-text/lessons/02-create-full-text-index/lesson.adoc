...
Here is a query that contains a complex search predicate on a single property:
...
Here is a query that contains a predicate that tests multiple properties:
...
Here is a query that contains a predicate that tests multiple properties with different node labels:
...
A full-text index will help improve the performance of each of these queries.


== Syntax for creating a full-text index for node properties

Here is the syntax for creating a full-text index for a single property of a node:
...
Notice also that it has more db hits than the query that uses the btree and the elapsed time is greater.
...
[NOTE]
--
In many queries, the elapsed ms times are the same for two queries so we use the lower db hits as a way of choosing which query is better.
db hits are generally a good measure to compare queries, but with an index such as a TEXT index, the db hits measurement may not be adequate for testing the query performance.
TEXT index usage may yield higher db hits, but lower elapsed times.
--
