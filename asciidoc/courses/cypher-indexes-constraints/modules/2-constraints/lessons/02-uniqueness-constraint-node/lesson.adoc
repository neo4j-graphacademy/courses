...
----
CREATE CONSTRAINT <constraint_name> IF NOT EXISTS FOR (x:<node_label>) REQUIRE x.<property_key> IS UNIQUE
----
...
=== Creating a uniqueness constraint for a single property
...
This code creates the uniqueness constraint named Movie_movieId_uniqueness.
...
[source,cypher,rel="norun nocopy"]
----
CREATE CONSTRAINT <constraint_name> IF NOT EXISTS FOR (x:<node_label>) REQUIRE (x.<property_key1>, x.<property_key2>)  IS UNIQUE
----
...
This code creates the uniqueness constraint named Movie_year_title.
Every Movie node in the graph will have a unique value for the year and title.
That is two movies with the same title cannot have the same year in the graph.
...
[source,cypher]
----
CREATE CONSTRAINT Movie_year_uniqueness IF NOT EXISTS FOR (x:Movie) REQUIRE x.year IS UNIQUE
----
...
In this lesson, you learned how to create an existence constraint for a node.
