...
////
[.video]
video::Ed187DcFPWE[youtube,width=560,height=315]

//https://youtu.be/Ed187DcFPWE
...
It should return 17 rows and have an elapsed time of ~4 ms in the Plan view.
...
A hint with a specified index type is only fulfillable when the planner knows that using an index of the specified type does not change the results.
...
// video redo
// need to re-record this with V5 version
This query executes about the same at ~7 ms in the Planner view.
