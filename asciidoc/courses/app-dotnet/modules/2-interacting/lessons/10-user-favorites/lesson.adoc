...
:test-filename: 07-FavoritesList
:test-pattern: _07_FavoritesList
...
1. The server directs the request to the route handler in link:{repository-blob}/main/Neoflix/Controllers/AccountController.cs[the `AccountController`^], which verifies the user's JWT token and stores it in the request, before handling the request
2. The route handler uses an instance of the link:{repository-blob}/main/Neoflix/Services/FavoriteService.cs[`FavoriteService`^].
...
[source,c#,indent=0]
----
include::{repository-raw}/{branch}/Neoflix/Services/FavoriteService.cs[tag="create"]
----
...
In this case, throw a `NotFoundException` with an appropriate error message.

[source,c#,indent=0]
----
include::{repository-raw}/{branch}/Neoflix/Services/FavoriteService.cs[tag="throw"]
----
...
[source,c#,indent=0]
----
include::{repository-raw}/{branch}/Neoflix/Services/FavoriteService.cs[tag="return",indent=0]
----
...
[source,c#,indent=0]
----
include::{repository-raw}/{branch}/Neoflix/Services/FavoriteService.cs[tag=add]
----
====
...
Use the code from the `AddAsync()` method above to implement the `remove()` function.
...
[source,c#,indent=0]
----
include::{repository-raw}/{branch}/Neoflix/Services/FavoriteService.cs[tag=remove]
----
====
...
[source,c#,indent=0]
----
include::{repository-raw}/{branch}/Neoflix/Services/FavoriteService.cs[tag=all]
----
