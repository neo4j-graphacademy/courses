...
As Neo4j is written in .NET (the _j_ in Neo4**j** stands for .NET after all), there are some discrepencies between the types stored in the Neo4j database and native C# types.

Some values like strings, numbers, booleans, dates, and nulls have a direct mapping to C# types but more complex types like nodes, relationship, points, durations need special handling.
...
[cols="1,1,1"]
|===
| .NET Type | Neo4j Cypher Type | Notes
