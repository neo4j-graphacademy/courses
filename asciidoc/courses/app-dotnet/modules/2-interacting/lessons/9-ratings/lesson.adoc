...
:test-filename: 06-RatingMovies
:test-pattern: _06_RatingMovies


The challenges in this course come thick and fast!

In this challenge, you will modify the `AddAsync()` method in the link:{repository-blob}/main/Neoflix/Services/RatingService.cs[`RatingService`^] to save ratings into Neo4j.
...
1. The server directs the request to the route handler in link:{repository-blob}/main/Neoflix/Controllers/AccountController.cs[`AccountController.cs`^], whose `/ratings/` route gets the authenticated user's id from the request.
2. The route handler uses an instance of the link:{repository-blob}/main/Neoflix/Services/RatingService.cs[`RatingService`^].
3. The `AddSync()` method is called on the `RatingService`, and is passed the id of the current user plus the id of the movie and a rating value from the request body.
4. It is then the responsibility of the `AddSync()` method to save this information to the database and return an appropriate response.
...
[source,c#,indent=0]
----
include::{repository-raw}/main/Neoflix/Services/RatingService.cs[tag=add]
----
...
The singular `movie` result should be converted to a dictionary.

[source,c#]
----
include::{repository-raw}/{branch}/Neoflix/Services/RatingService.cs[tag=write,indent=0]
----

If `cursor.FetchAsync()` returns false, either the user or movie could not be found.
Any `null` values returned by this transaction function will be handled further on in the code.

[TIP]
.Reusing Existing Ratings
...
If no movie or user is found, the method will return throw a `NotFoundException` exception.

[source,c#,indent=0]
----
include::{repository-raw}/{branch}/Neoflix/Services/RatingService.cs[tag=throw,indent=0]
----


=== Return the Results

Finally, return the updated movie.

[source,c#]
----
include::{repository-raw}/{branch}/Neoflix/Services/RatingService.cs[tag=addreturn,indent=0]
----
...
[source,c#,indent=0]
----
include::{repository-raw}/{branch}/Neoflix/Services/RatingService.cs[tag=add]
----
====
