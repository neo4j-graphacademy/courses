...
Once you have run all of the queries you intend to run within that session, you should call the `.close()` method to close the session.
This releases any resources held back to the driver pool.
...
3. Also optional, an object of transaction config.
