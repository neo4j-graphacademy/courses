...
1. Use the `neo4j` object to create a new instance of the Driver with the credentials provided:
  * `NEO4J_URI` is the connection string
  * `NEO4J_USERNAME` and `NEO4J_PASSWORD` should be used to create the authentication token.

2. Once you have created the Driver, open a new session and run the following query: +
+
.Find the Director
[source, cypher]
----
...
3. To find the answer, click the Debug icon to the left of the window and run **Create Driver** job, or use the integrated terminal window to run the following command: +
+
.Run The Challenge
...
4. Once you have the result, copy and paste it into the text box below and click **Check Answer**.
