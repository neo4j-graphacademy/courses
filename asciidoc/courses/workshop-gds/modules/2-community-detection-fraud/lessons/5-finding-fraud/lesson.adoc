...
----
MATCH (source:User)-[r:HAS_CC|USED|HAS_IP]->(target:Card|Device|IP)
...
----
MATCH (u1:User)-[:HAS_CC|USED]->(n:Card|Device)<-[:HAS_CC|USED]-(u2:User)
WHERE n.degree <= 10
  AND u1 < u2
...
----
MATCH path = (u1:User)-[r:P2P_SHARED_CARD|SHARED_IDS*10]-(u2:User)
RETURN path
LIMIT 200
----
...
----
MATCH (source:User)
OPTIONAL MATCH (source)-[r:P2P_SHARED_CARD|SHARED_IDS]->(target:User)
...
WITH community
MATCH path = (u1:User)-[:P2P_SHARED_CARD|SHARED_IDS|HAS_CC|USED]-(n)--(u2:User)
