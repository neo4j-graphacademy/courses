...
[.transcript-only]
====
This workflow separates your source data from analysis, enabling fast, iterative experimentation on a _projection_ of your graph without modifying your original graph.
...
[source,cypher,role="noplay nocopy"]
.Project a graph of actors and movies
----
MATCH (source:Actor)-[r:ACTED_IN]->(target:Movie)
----
...
.Creating the in-memory graph projection
----
...
...
[source,cypher,role="nocopy noplay"]
.Naming the graph and specifying nodes
----
...
WITH gds.graph.project( // (2)
  'actors-graph',
...
.Returning projection statistics
----
...
RETURN g.graphName AS graph,
  g.nodeCount AS nodes,
...
First, we call the `gds.degree.stream` function, declare what algorithm we want to run and then declare what **mode** we would like to run that algorithm in.
...
[source,cypher,role="nocopy noplay"]
.Yielding and returning results
----
...
YIELD nodeId, score
RETURN gds.util.asNode(nodeId).name AS name, score
