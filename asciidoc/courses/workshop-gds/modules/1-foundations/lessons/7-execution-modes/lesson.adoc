...
====
<1> Project the graph
<2> Assign the result to a variable `g`
<3> Return the graph name, node count and relationship count from the variable `g`
====
...
<3> Return the community count, modularity and random levels
====
...
<2> Yields count of properties written to the projection
<3> Return the count of properties written to the projection
====
...
<2> Yields count of properties written to your database
<3> Return the count of properties written to your database
====
...
<2> Yields graph size and memory requirements
<3> Return the node count, relationship count and required memory
====
