...
このレッスンでは、Neo4jがサポートする、*プロパティ・グラフ*を作成するための追加要素について学びます。

=== ノード、ラベル、プロパティ

ノードとは、データ中の「モノ」を表すグラフ要素であることを思い出してください。
さらに2つの要素を使って、データにコンテクストを与えることができる。
...
Neo4jでは、ラベルはCypher文の出発点となるため、重要です。

例えば、**Michael** と **Sarah** を例にとると、この文脈ではこれらのノードは両方とも *persons* であることがわかる。

Michaelは*male*、Sarahは*female*と識別されるように、これらのノードにラベルを追加してグラフを装飾することができます。
この文脈では、Michaelはある会社の*従業員*であるが、Sarahの雇用形態については何も情報がない。

MichaelはGraph Incという*会社*で働いているので、会社を表すノードにそのラベルを追加することができる。

[TIP]
Neo4jでは、ノードは0、1、または多くのラベルを持つことができます。


==== Node properties

ここまでは、ノードがMichael、Sarah、Graph Inc.を表すと仮定している。
ノードにプロパティを追加することで、これを具体化することができます。

プロパティはキーと値のペアであり、必要に応じてノードに追加したり削除したりすることができる。
...
サラのフルネームは*Sarah Faraday*で、_maidenName_ は*Barnard*です。
...
[TIP]
プロパティは、特定のラベルを持つ各ノードに対して存在する必要はない。
もし、あるノードに対してプロパティが存在しない場合、それは `null` 値として扱われる。
...
リレーションシップは、ソース・ノードとデスティネーション・ノードの間に作成されるため、リレーションシップを作成する前に、これらのノードが存在しなければなりません。

前のモジュールで説明した有向・無向グラフの概念を考慮すると、_MARRIED_TO_リレーションの方向は存在しなければならず、いくつかの追加コンテキストを提供するかもしれませんが、クエリの目的には無視することが可能です。
Neo4jでは、_MARRIED_TO_リレーションは方向性を持っていなければなりません。
...
例えば、私たちはマイケルから _一つ一つの_ リレーションシップを通してトラバースすることができますし、サラのノードだけで終わるために_MARRIED_TO_リレーションシップを指定することができます。

これをサポートするCypherのステートメント文のサンプルです。
...
ノードと同様に、リレーションシップもプロパティを持つことができる。  これらは、重み付きグラフにおけるコストや距離を参照したり、リレーションシップに追加のコンテキストを提供したりすることができます。

image::images/relationship-properties.jpg[Relationships with properties]

このグラフでは、_MARRIED_TO_関係にプロパティを置いて、マイケルとサラが結婚した日付を保持することができます。
この_WORKS_AT_関係には、その従業員が会社で果たした役割を示す_roles_プロパティがあります。
...
このレッスンでは、Neo4jのプロパティ・グラフの実装が、ノードのラベルとノードとリレーションシップの両方のプロパティを含んでいることを学びました。
