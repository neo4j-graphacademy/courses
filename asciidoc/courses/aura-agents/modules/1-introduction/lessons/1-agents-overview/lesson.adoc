...
* **Execute tools**: Run the selected tool(s) against your graph. Each tool returns data. The agent may run more than one before answering.
* **Generate response**: Turn the retrieved data into a natural language answer.

The agent decides which tools to run. Tools only return data. They do not choose when to run or what to return. The agent uses that data to build the response.
...
include::questions/3-create-with-ai-prompt.adoc[leveloffset=+1]
