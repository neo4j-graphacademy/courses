...
Agents have two **access modes** (visibility options) that control who can reach the agent and how:
...
**External**: The agent is exposed so that external applications can call it. You get two endpoint types—REST API and MCP server—that host applications (e.g. Cursor, Claude Desktop) use to send messages and receive answers. External agents incur usage-based charges; see the link:https://neo4j.com/docs/aura/aura-agent/[Aura Agent documentation^] for pricing.
...
When you select **External**, the **Enable MCP server** toggle appears. Enable it if you want to connect from an MCP host (e.g. Cursor). Both External access and the MCP toggle must be on for MCP connections.
