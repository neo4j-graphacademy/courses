...
* **One tool per pattern** — Give each distinct job or question family its own tool when the query shape or purpose differs. A single template should not try to cover unrelated asks; narrow tools get clearer descriptions and fewer wrong picks.
* **Templates first** — If you can write the full Cypher now with only `$parameter` slots for values, use a Cypher Template. Reserve Text2Cypher for questions whose structure changes with the ask.
