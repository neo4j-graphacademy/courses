...
In this challenge, you implement a simple Aura Agent from scratch: you define the role, add Cypher Template tools and a Text2Cypher tool that match your schema, then test the agent. Creating from scratch gives you full control over every tool, parameter, and instruction.

== Before you start

* Have an AuraDB instance with your knowledge graph loaded. If you are using the example, load Northwind.
...
Set the **Agent Name** and **Description** to match your design from the Design an Agent lesson. Set **Instructions** to define the agent's role, include node labels and relationship types, and state that the agent should decline off-topic or harmful requests. The Design an Agent lesson explains how to include schema in the instructions.
...
The graph contains these node labels: Customer, Order, Product, Category, Supplier.
...
If you are using Northwind, you can add the following four Cypher Template tools as a working example. Property names in the Cypher (for example `id`, `companyName`) depend on the Northwind loading script you used. If your dataset uses different property names, adjust the queries accordingly.
...
Add a Text2Cypher tool as a fallback for questions your Cypher Templates do not cover. The tool description must state when to use it (and when not). The tool automatically receives the schema. Add domain context such as relevant entities and which attributes are suitable for aggregation. See the Design an Agent lesson for details.

[NOTE]
.Cypher Template vs. Text2Cypher
====
See the Design an Agent lesson for the test: Cypher Template when the query shape is fixed and only values change. Use Text2Cypher when the query structure changes. Text2Cypher generates Cypher at runtime and may produce errors.
====

image::images/text-2-cypher-tool.png[Text2Cypher tool configuration showing name and description fields]

Give the tool a name, such as "Query graph" or "Ad-hoc query," and a description that states when to use it and adds domain context. See the Design an Agent lesson for details. Example for Northwind:
...
A description like "answer any question" will cause the agent to use Text2Cypher for everything. Keep the "ONLY when no other tool covers" constraint. The tool receives the schema automatically. The description adds when-to-use and domain context for more accurate Cypher.
...
Extend your agent with additional Cypher Template tools that fit your schema and design. Add them without step-by-step guidance: open the agent, click **Add Tool** → **Cypher Template**, and define name, description, parameters, and Cypher for each. Then click **Update agent** to save and test. The Design an Agent lesson explains why this step is required.
