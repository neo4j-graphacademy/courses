...
Go to link:https://console.neo4j.io/graphacademy[Aura Console^] → **Data Services** → **Agents** → **Create Agent**. You have two options.

=== Create with AI

Select **Create with AI** to let Aura generate a starting set of tools and instructions from a prompt. This is a good starting point when you want a working draft to iterate on, especially for a new graph.
...
Select **Create from scratch** for full control over every tool and instruction from the start. This is the approach this lesson walks through.
...
This makes tool selection transparent. Read the generated Cypher to verify it matches your intent.
