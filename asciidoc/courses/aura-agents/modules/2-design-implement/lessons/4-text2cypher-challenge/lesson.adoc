...
. **When to use it**: when no other tool covers the question and a dynamically generated query is needed.
. **Domain context**: the agent already receives the full database schema automatically — do not list all labels and relationship types. Instead, add context the schema does not provide: the shape of identifiers (for example, "Customer IDs are uppercase codes like ALFKI"), which categorical property values are useful for filtering, and which numeric properties are suitable for aggregation.
