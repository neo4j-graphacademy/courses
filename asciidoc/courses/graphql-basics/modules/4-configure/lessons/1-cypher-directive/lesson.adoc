...
The `@cypher` directive takes a single argument `statement` - the Cypher statement to execute to populate the field. The Cypher statement can reference a `this` variable, the currently resolved node, in this case a `Movie` node.
...
Rebuild the schema to update the GraphQL API to include the new `similar` field. This field can now be included in the GraphQL selection set to find the most similar movies:
