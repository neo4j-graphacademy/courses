...
----
[NOTE]
The `!` after the `title` field type indicates that this field is required and cannot be null (i.e. it is non-nullable).
...
`Connection` fields that are generated for the relationship type e.g. the `actorsConnection` of `movies` field can be used to access the `role` property of the `ACTED_IN` relationship between an `Actor` and a `Movie`:
