...
----
[NOTE]
The `!` after the field types indicates that this field is required and cannot be null (i.e. it is non-nullable). Lists cannot contain null values, so the `!` is added to the list type.
