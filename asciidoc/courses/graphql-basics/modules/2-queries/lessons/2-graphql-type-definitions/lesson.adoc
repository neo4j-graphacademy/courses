...
* Create a new GraphQL types for other nodes 
* Create relationships between the types

== GraphQL Type Definitions And The Property Graph Model

So far you have used a simple GraphQL type definitions with the movie recommendations dataset:
...
First, let's add fields to the `Movie` type in the GraphQL type definitions to make additional node properties available in the GraphQL API.
...
+ 
Once the schema has been built the additional fields can be used in your GraphQL queries. 
. Run the following query, either by adding the fields using the Explorer checkbox or by typing directly into the Query pane.
+
[source,GraphQL]
----
query MyQuery {
