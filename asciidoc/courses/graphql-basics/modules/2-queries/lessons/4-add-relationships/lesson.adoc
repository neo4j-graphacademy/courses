...
A `User` can provide a 1-5 rating for a `Movie` by creating a `RATED` relationship between them. A float `rating` property holds the user's rating for the movie.
