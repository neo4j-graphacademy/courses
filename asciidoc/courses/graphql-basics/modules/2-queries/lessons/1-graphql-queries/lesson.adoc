...
>  The GraphQL Library generates the same 3 query fields as well as mutation operations for each type / node label defined in the type definitions. You will learn about mutations in the next module.
...
The `Query.movies` generated query field takes two optional arguments: 
...
The `options` argument was used in the example query to limit the number of results.  A `sort` option can also be added to change the sort order of the results.
...
> Note that in addition to editing the GraphQL queries in the query editor you can use the Explorer feature integrated in the Neo4j GraphQL Toolbox to interactively explore the schema documentation and toggle fields on and off in the selection set:
