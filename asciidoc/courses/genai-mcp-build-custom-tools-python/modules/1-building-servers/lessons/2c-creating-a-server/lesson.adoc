...
Now for a challenge! In the previous lesson, you learned about the MCP Python SDK and FastMCP. Now it's time to put that knowledge into practice by creating your own MCP server from scratch.

Your challenge is to create a simple MCP server that demonstrates the core concepts you've learned, then run and test it using the tools covered in the previous lesson.
