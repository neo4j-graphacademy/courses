...
[.summary]
== Summary
In this lesson you learned about how GDS works and the high-level workflow in GDS.  You also learned about GDS concurrency and Neo4j memory configurations to support GDS workloads. In the module you will learn about graph management, the graph catalog, and working with graph projections in more detail.