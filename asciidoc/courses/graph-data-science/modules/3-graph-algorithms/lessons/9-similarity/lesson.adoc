...
* Node Similarity allows you to choose between jaccard and overlap similarity metrics.
* KNN metrics are driven by the node property types. For a list of integers you can choose to between jaccard and overlap metrics. For a list of floating point numbers you can choose between cosine similarity, pearson, and euclidean metrics.

Using different metrics will of course alter the similarity score and change the interpretation slightly. You can read more about the different metrics for Node Similarity https://neo4j.com/docs/graph-data-science/current/algorithms/node-similarity/#algorithms-node-similarity-syntax[in the Node Similarity documentation^] and for KNN https://neo4j.com/docs/graph-data-science/current/algorithms/knn/#algorithms-knn-introduction-metrics[in the K-Nearest Neighbors documentation].
...
Let's take the embeddings we calculated from the node embedding lesson and use them to determine similarity between the actors and directors based on movies they have been involved with. If you do not have the graph projection from the previous node embeddings lesson saved you can regenerate it with the below:
...
After that we can run similarity.  We will use the default cosine metric. For purposes of demonstrationI will limit `topK` to one so we can see the top pairs for each node.
