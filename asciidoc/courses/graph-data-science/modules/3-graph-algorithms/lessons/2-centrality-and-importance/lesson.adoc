...
In summary, pageRank estimates the importance of a node by counting the number of incoming relationships from neighboring nodes weighted by the importance and out-degree centrality of those neighbors. The underlying assumption is that more important nodes are likely to have proportionately more incoming relationships from other import nodes. Our documentation offers a thorough technical explication of pageRank if you are interested in digging in deeper.
