...
Once the pipeline is trained you can use it to predict new links in the graph. The pipeline can be re-applied to data with the same schema.  Below we show a streaming example.
...
include::questions/2-pipeline-configuration.adoc[leveloffset=+1]
