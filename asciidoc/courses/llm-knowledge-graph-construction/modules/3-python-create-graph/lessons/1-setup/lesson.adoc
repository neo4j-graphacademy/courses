...
We have link:{repository-link}[created a repository^] for this course - link:{repository-link}[github.com/neo4j-graphacademy/llm-knowledge-graph-construction]. 
...
[IMPORTANT]
You will need to login to link:https://gitpod.io[Gitpod^] with a Github account.
...
You can test your setup by running `llm-knowledge_graph\test_environment.py` - this will attempt to connect to the Neo4j sandbox and the OpenAI API.
