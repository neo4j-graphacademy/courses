...
Typically, you would follow these steps to construct a knowledge graph:
...
Depending on your requirements for querying and searching the data, you may need to _vectorize_ it.
You can use an LLM (or another embedding model) to create embeddings for each data chunk.

Including a vector index over the embeddings allows you to perform similarity searches, clustering, and other operations on the data.
...
- Optionally, provide additional context or constraints for the extraction, such as the type of entities or relationships you are interested in.
...
If you want to construct a knowledge graph based on the link:https://en.wikipedia.org/wiki/Neo4[Neo4j Wikipedia page^], you would:

. Gather the text from the page.
+ 
You may filter out the references or releases section because it is irrelevant to your knowledge graph.
. Chunk the text into paragraphs.
. Create embeddings for each paragraph.
. Pass the text to the LLM.
+ 
You include a prompt instructing the LLM to identify the entities and relationships in the text.
+
[source, text]
----
...
----
+ 
The LLM outputs the entities and relationships in a JSON.
+
[source, json]
----
[
...
. You can then use the data to construct the graph in Neo4j by creating nodes and relationships based on the entities and relationships extracted by the LLM.
+
[source, cypher]
----
CREATE (neo4jInc:Company {id: 'Neo4j Inc'})
CREATE (neo4j:GraphDatabase {id: 'Neo4j'})
CREATE (java:ProgrammingLanguage {id: 'Java'})
CREATE (neo4j)-[:DEVELOPED_BY]->(neo4jInc)
