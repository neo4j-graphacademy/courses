...
For the first part of this challenge, modify the `Save()` method to open a new database session, run the Cypher query to create the `:HAS_FAVORITE` relationship, close the session and return the movie details along with an additional `favorite` property.
...
There should only be one result, so you can use the `Single()` method to get the first result.
...
include::{repository-raw}/working/pkg/services/favorites.go[tag=all]
----
...
To do this, you will need to open a session, execute the above Cypher query in a read transaction and then consume the results.
...
[source,go,indent=0]
----
// include::{repository-raw}/main/pkg/services/favorites.go[tag=collect]
include::{repository-raw}/working/pkg/services/favorites.go[tag=collect]
----
