...
In this challenge, you will modify the `add()` method in the link:{repository-blob}main/pkg/services/ratings.go[`RatingService`^] to save ratings into Neo4j.
...
1. The server directs the request to the route handler in link:{repository-blob}working/pkg/routes/account.go[`api/routes/account.go`^], which verifies the user's JWT token before handling the request.
...
[source,go,indent=0]
----
include::{repository-raw}/working/pkg/services/ratings.go[tag=add]
...
In the next Challenge, you will follow the same steps implement the My Favorites feature.
