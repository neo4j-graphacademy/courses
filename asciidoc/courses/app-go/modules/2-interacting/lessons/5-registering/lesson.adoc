...
The dummy register logic is already written into the `Save()` method of the `AuthService` at `pkg/services/auth.go`.  As we can see from the snippet below, at the moment it will only accept an email address of `graphacademy@neo4j.com`.


[source,go,indent=0]
.pkg/services/auth.go
----
include::{repository-raw}/working/pkg/services/auth.go[tag="register"]
...
The `userWithToken` function then combines the two into a format that the format that the UI expects.
...
.Open a new Session
----
include::{repository-raw}/working/pkg/services/auth.go[tag="session"]
// include::{repository-raw}/main/pkg/services/auth.go[tag="session"]
----
...
.Create the User
----
include::{repository-raw}/working/pkg/services/auth.go[tag="create"]
// include::{repository-raw}/main/pkg/services/auth.go[tag="session"]
----
...
----
include::{repository-raw}/working/pkg/services/auth.go[tag="extract"]
// include::{repository-raw}/main/pkg/services/auth.go[tag="session"]
----
...
.Return the User and JWT Token
----
include::{repository-raw}/working/pkg/services/auth.go[tag="return"]
// include::{repository-raw}/main/pkg/services/auth.go[tag="session"]
----
