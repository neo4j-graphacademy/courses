...
include::{repository-raw}/working/pkg/routes/movies.go[tag=list]
----
...
2. The User's ID is extracted from the request
3. The `FindAll` services is called on the `MovieService` with the paging values and User ID and the results are serialized as JSON.


=== The Movie Service

The magic happens in the `MovieService`, located at link:{repository-blob}main/pkg/services/movies.go[`pkg/services/movies.go`^].
For this route we are concerned with the `FindAll()` method.

If we take a closer look at the `FindAll()` method, we can see that it currently returns a hardcoded list of popular movies from a another file in the repository.

[source,go,indent=0]
.pkg/services/movies.go
----
// include::{repository-raw}/main/pkg/services/movies.go[tag=all]
include::{repository-raw}/working/pkg/services/movies.go[tag=all]
----
...
Within the `FindAll` method, first open a new session by calling the `.NewSession()` method on the Driver instance injected into the `MovieService`.
...
The unit of work is passed a single argument, an instance of `neo4j.Transaction`. which has a `.Run()` method that you can use to execute a Cypher statement.
...
.User Favorites
====
This method includes a call to the `getUserFavorites()` function.
At this stage the function returns an empty array, but we will implement this feature in a future lesson.
====
