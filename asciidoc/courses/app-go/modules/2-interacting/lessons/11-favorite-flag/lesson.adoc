...
Your next challenge is to modify the link:{repository-raw}/main/api/services/movies.go[`MovieService`] to dynamically determine whether a movie is on the current user's My Favorite List.
...
At the bottom of the `MovieService`, a placeholder `get_user_favorites()` method exists which is currently hardcoded to return an empty array.

[source,go,indent=0]
----
include::{repository-raw}/main/api/services/movies.go[tag="getUserFavorites"]
----
...
[source,go,indent=0]
----
include::{repository-raw}/{branch}/api/services/movies.go[tag="getUserFavorites"]
----
...
The `get_movies` function defined in the `all` method of the `MoviesService` already uses `tx.run()` to run a Cypher query within the current transaction.
...
All that is left to do is to add a new item to `m { .* }` in the Cypher query to check whether the `tmdbId` property of the current movie is in the favorites array.


[source,go,indent=0]
----
include::{repository-raw}/{branch}/api/services/movies.go[tag="allcypher"]
----
...
[source,go,indent=0]
----
include::{repository-raw}/{previous-branch}/api/services/movies.go[tag="allcypher"]
----
