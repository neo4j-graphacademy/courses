...
As Neo4j is written in Java (the _j_ in Neo4**j** stands for Java after all), there are some discrepencies between the types stored in the Neo4j database and native Go types.

Some values like strings, floats, booleans, and nulls have a direct mapping to Go types but more complex types need special handling.
...
Node is a type provided by the Neo4j Go Driver to hold the information held in Neo4j for the node.
...
[cols="1,3,1,1"]
|===
| Neo4j Cypher Type | Go driver type | Go built-in type | `tzinfo`
...
For more information, see link:https://neo4j.com/docs/api/Go-driver/4.4/temporal_types.html[Temporal Data Types^&^].
...
To create a Cartesian Point in Go, you can import the `neo4j.spatial.CartesianPoint` class.
...
For more information, link:https://neo4j.com/docs/api/Go-driver/current/api.html#cartesianpoint[see the Go reference^].
...
For more information, link:https://neo4j.com/docs/api/Go-driver/current/api.html#wgs84point[see the Go reference^].
