...
In this challenge you will rewrite the `authenticate()` function of the link:{repository-blob}main/api/services/auth.go[`AuthService`^] to find the User node with the corresponding email and compare the password before issuing a JWT token.
...
The token generation happens in the link:{repository-blob}/main/api/services/auth.go[`AuthService`^] using the `_generate_token()` method.

When The API receives a request which includes the `authorization` header, the link:https://Gohosted.org/Flask-JWT/[Flask-JWT] library attempts to decode the value and makes the payload available by importing `current_user` from `flask_jwt`.
...
[source,go,indent=0]
----
include::{repository-raw}/main/api/services/auth.go[tag=authenticate]
----
...
[source,go,indent=0]
----
include::{repository-raw}/{branch}/api/services/auth.go[tag=get_user]
----


=== Execute the function within a Read Transaction

After opening up a new session, call the `ReadTransaction` method to execute the `get_user` function above.

.Call the Transaction Function
[source,go,indent=0]
----
include::{repository-raw}/{branch}/api/services/auth.go[tag=call_get_user]
----
...
[source,go,indent=0]
----
include::{repository-raw}/{branch}/api/services/auth.go[tag=user_not_found]
----
...
[source,go,indent=0]
----
include::{repository-raw}/{branch}/api/services/auth.go[tag=compare_passwords]
----
...
[source,go,indent=0]
----
include::{repository-raw}/{branch}/api/services/auth.go[tag=auth_jwt]
----
...
[source,go,indent=0]
----
include::{repository-raw}/{branch}/api/services/auth.go[tag="authenticate"]
----
