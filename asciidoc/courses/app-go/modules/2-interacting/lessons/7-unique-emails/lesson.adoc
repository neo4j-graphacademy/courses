...
In the link:../5-registering[Registering a User challenge^], you updated the `register()` method in the link:{repository-blob}main/api/services/auth.go[`AuthService`^] to create a new User node in the database.
...
----
include::{repository-raw}/{previous-branch}/api/services/auth.go[tag=register]
----
...
[source,go,indent=0]
----
include::{repository-raw}/{branch}/api/services/auth.go[tag="catch"]
----
...
[source,go,indent=0]
----
include::{repository-raw}/{branch}/api/services/auth.go[tag="register"]
----
====
