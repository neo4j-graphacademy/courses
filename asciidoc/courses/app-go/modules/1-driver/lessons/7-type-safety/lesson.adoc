...
In the code sample above, the `neo4j.CollectTWithContext()` function is used to return array of `personActedInMovie` structs.
...
The output of the function is an array of values returned by the callback function.


== SingleTWithContext

Alternatively, if you expect one result, you can use the `neo4j.SingleTWithContext()` function.
The function signature is similar, but instead the function will return a single value instead of an array.
...
2. A boolean to represent whether the value is `null` or non-existent in Neo4j
