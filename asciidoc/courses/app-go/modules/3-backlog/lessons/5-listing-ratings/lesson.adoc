...
The list of ratings is populated by the `FindAllByMovieId()` method in the link:{repository-blob}pkg/services/ratings.go[`RatingService`^], which currently returns a hardcoded list of ratings.

.pkg/services/ratings.go
[source,go,indent=0]
----
// include::{repository-raw}/main/pkg/services/ratings.go[tag=forMovie]
include::{repository-raw}/working/pkg/services/ratings.go[tag=forMovie]
----
