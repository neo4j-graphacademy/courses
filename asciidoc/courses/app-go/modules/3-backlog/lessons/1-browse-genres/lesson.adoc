...
This list is populated by the API route at http://localhost:3000/api/genres, with the list being produced by the `all()` method within the link:{repository-blob}/main/api/services/genres.go[`GenreService`^].

.api/services/genres.go
[source,go,indent=0]
----
include::{repository-raw}/main/api/services/genres.go[tag=all]
----
...
[source,go,indent=0]
----
include::{repository-raw}/{branch}/api/services/genres.go[tag=all]
----
