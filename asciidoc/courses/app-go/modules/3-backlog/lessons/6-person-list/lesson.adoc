...
The list is populated by the `all()` method in the link:{repository-blob}main/api/services/people.go[`PeopleService`^].

.api/services/people.go
[source,go,indent=0]
----
include::{repository-raw}/main/api/services/people.go[tag=all]
----
...
[source,go,indent=0]
----
include::{repository-raw}/{branch}/api/services/people.go[tag=all]
----
