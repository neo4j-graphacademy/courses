...
:test-filename: 14_person_list_test
:test-function: TestPersonList
:test-output: true
:optional: true
...
The list is populated by the `FindAll()` method in the link:{repository-blob}main/pkg/services/people.go[`PeopleService`^].

.pkg/services/people.go
[source,go,indent=0]
----
// include::{repository-raw}/main/pkg/services/people.go[tag=all]
include::{repository-raw}/working/pkg/services/people.go[tag=all]
----
...
* Modify the `FindAll()` method to query Neo4j and return a list of people.
* If the `q` parameter is defined, add a `WHERE` clause to check that the `name` property contains the value provided.


[%collapsible]
.Click to reveal the completed `FindAll()` method
====

.pkg/services/people.go
[source,go,indent=0]
----
include::{repository-raw}/{branch}/pkg/services/people.go[tag=all]
----
...
In this Challenge, you implemented the `FindAll()`  method in the `PeopleService` to return a paginated list of people from the Neo4j database and implemented a conditional `WHERE` clause on the `Person` `name` property.
