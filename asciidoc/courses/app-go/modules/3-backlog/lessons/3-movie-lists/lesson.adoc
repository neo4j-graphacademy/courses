...
In this challenge, you will implement the remaining methods in the link:{repository-blob}/main/api/services/movies.go[`MovieService`^] for retrieving a list of movies:
...
// .api/services/movies.go
// [source,go,indent=0]
// ----
// include::{repository-raw}/10-genre-details/api/services/movies.go[tag=all]
// ----
...
All you need to do is take the code from the `all()` method of the `MovieService` and modify the pattern in the first line of the Cypher query in the first argument of the `tx.run()` call, then if necessary add the additional parameters to the second argument.
...
[source,go,indent=0]
----
include::{repository-raw}/main/api/services/movies.go[tag=getByGenre]
----
...
Find the `get_by_genre()` method in `api/services/movies.go` and modify the function to find a list of movies by genre.

.api/services/movies.go
[source,go,indent=0]
----
include::{repository-raw}/{branch}/api/services/movies.go[tag=getByGenre]
----
...
[source,go,indent=0]
----
include::{repository-raw}/main/api/services/movies.go[tag=getForActor]
----

Find the `get_for_actor()` method in `api/services/movies.go` and modify the function to find a list of movies by actor.
...
[source,go,indent=0]
----
include::{repository-raw}/{branch}/api/services/movies.go[tag=getForActor]
----
...
[source,go,indent=0]
----
include::{repository-raw}/main/api/services/movies.go[tag=getForDirector]
----

Find the `getForDirector()` method in `api/services/movies.go` and modify the function to find a list of movies by director.
...
[source,go,indent=0]
----
include::{repository-raw}/{branch}/api/services/movies.go[tag=getForDirector]
----
