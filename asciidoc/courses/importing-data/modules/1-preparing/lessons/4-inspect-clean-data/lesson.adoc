...
Before you start working with the sourece CSV data, you must understand how delimiters, quotes, and separators are used for each row.

If the headers do not correspond to the data representing the fields, you cannot load the data reliably.
You must also know whether you can assume the use of the default delimiter, otherwise, you will need to use the `FIELDTERMINATOR` keyword.

In most cases, you will need to download the CSV files so you can inspect the data in them.
If the CSV file is at a URL, you should download it to inspect it, if you are unsure of how clean the data is.

Additionally, you must determine if the CSV file is totally readable and if you may need to perform some additional clean up.
...
By default all of these fields in each row will be read in as string types.
...
Here is the code that will read all data in a CSV file that contains headers and is specified as a URL:
...
For this course, you will be working with data that has already been cleaned up, but if you receive a CSV file from another source, you should ensure that it is clean.

== Check your understanding

include::questions/1-clean.adoc[]
