...
If you want to use a different field terminator, you must specify the `FIELDTERMINATOR` in your `LOAD CSV` clause.
...
This is the data that could be used to create the _:ACTED_IN_ relationship that you have seen in the Movie graph.

== De-normalized data

Here is an example where we have de-normalized data for the same type of data:
...
For example, The movie and person data (including the ID) is repeated in multiple rows, but for a particular movie, a different actor's role is represented.
That is, a Movie and Person node will be represented by multiple rows, but an actor's role will be represented by a single row.
...
You will learn later in this module that you must create uniqueness constraints in the database before you load the data.
