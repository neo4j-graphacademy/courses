...
For example, if a field in a row needs to be stored in the graph as a date or list, it will be by default stored in the graph as a string and you will need to post-process the graph after the import.
...
==== Step 1: Place the CSV file(s) on your local system and ensure they have headers and are clean.

You should examine the files and make sure they have headers and are "clean", as you learned about in an earlier lesson.

==== Step 2: Open the Neo4j Data Importer tool

Access the Data Importer tool from any Web browser using this URL: https://data-importer.graphapp.io/

==== Step 3: Load the CSV files on your local system into the graph app

You can either drag the CSV files into the Files pane on the left or use *Browse* to select them as shown here.

image::images/select-files-to-import.gif[Select CSV files,width=400,align=center]

==== Step 4: Examine the CSV header names used in the CSV files to determine:
...
==== Step 6: Define mapping details for the node
...
==== Step 7: Create the relationships between nodes
...
image::images/add-relationbship.gif[Add relationship,width=400,align=center]

==== Step 8: Define the mapping details for the relationship
...
==== Step 9: Perform the import

. Specify connection URL and credentials.
. View the import results.

==== Step 10: View the imported data in Neo4j Browser
