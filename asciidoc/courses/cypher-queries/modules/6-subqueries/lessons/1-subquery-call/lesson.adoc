...
When you execute a series of `MATCH` clauses, all nodes and relationships that are retrieved are in memory.
...
A subquery typically returns nodes.
...
The subquery returns a Movie node for every Movie in the graph that has a rating.
Each node that is returned by the subquery is then tested to see if it was released in the year 2000.
...
Here is an example where the subquery is executed after the initial query.
...
The first `MATCH` returns a row for every movie in the graph.
It passes the Movie node to the subquery.
...
Back in the outer query, the title is returned, and the count of the number of rows returned from the subquery.
