...
You can create a single instance of the `Neo4jGraph` class to use across your application.
...
1. Defines a singleton instance
2. Defines a function that will return the singleton instance if it has been created, or create one if it does not exist.


Inside the function, add an `if` statement to check if the `graph` object has not already been created.
...
Finally, return the `graph` variable.
...
In this lesson, you created a singleton `Neo4jGraph` object for use across the application.
