...
To install the dependencies, you can run the `npm install` command.
...
To connect to Neo4j, you will need to set your Neo4j Sandbox credentials as environment variables.
...
We have included an example `.env.example` file in the project root with the variables you will need to set to use Neo4j and OpenAI.
...
We have added the `.env.local` file to the `.gitignore` so the contents are not accidentally uploaded to Github.
====
...
When the user sends a message, the message will be rendered in a box aligned to the right of the screen, similar to other chat apps.
...
When a response is received, the message is appended to the state and appended to the chat interface.


=== Handling Submissions

The user's input is passed through a link:{repository-blob}/main/src/pages/api/chat.ts[route handler in the `src/pages/api/chat.ts` file^] which extracts the user's input and the current Session ID from the request.

The input and Session ID are then passed to link:{repository-blob}/main/src/modules/agent/index.ts[the `call()` function in the `src/modules/agent/index.ts` file^].
The `call()` function will eventually provide an interface to the LLM, but for now it simply returns the user's input after a short delay.
