...
====
To convert the conversation history to a string, the `RunnablePassthrough.assign()` method  formats the array of Chatbot responses in the `history` key as a single string.

The input variable for `history` is an array of `ChatbotResponse` objects.
The `.map()` method on the array converts the `input` and `output` properties of each object to a string, and the `.join()` method to joins them into a single string.
