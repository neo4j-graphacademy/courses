...
To make use of a Vector Store, you will need a Vector Search Index.

Use the `CREATE VECTOR INDEX` command to create a vector index called `moviePlots` if it does not already exist.
...
The statement creates a new index called `moviePlots`, indexing the vectors contained in the `embedding`  property.
The vectors stored in the `embedding` property will need `1536` dimensions.
...
[source,typescript]
----
include::{repository-raw}/{branch}/{lab-solution}[tag=store]
----
...
In this case, the title is returned along with the names of actors and directors along with a canonical link to the movie on The Movie Database (TMDB).
...
This will be used to provide transparency on the context provided to help the LLM generate its response.
====
