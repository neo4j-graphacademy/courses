...
You've practiced writing queries. Now you'll build the complete recommendation query that answers: **"What products do people like me buy, that I haven't bought yet?"**
...
RETURN DISTINCT myProduct.productName
ORDER BY myProduct.productName;
----
...
----
MATCH (me:Customer {customerId: 'ALFKI'})-[:PLACED]->(:Order)-[:CONTAINS]->(myProduct:Product)  // (1)
MATCH (myProduct)<-[:CONTAINS]-(:Order)<-[:PLACED]-(other:Customer)                             // (2)
WHERE other <> me                                                                                // (3)
RETURN other.companyName, count(DISTINCT myProduct) AS sharedProducts                           // (4)
...
RETURN DISTINCT rec.productName, count(DISTINCT other) AS recommendedBy                         // (5)
...
RETURN rec.productName, count(DISTINCT other) AS recommendedBy                                  // (6)
...
**This is the core recommendation query!**
...
SELECT p.productName, p.unitPrice, p.unitsInStock, r.recommendedBy
...
In the next module, you'll learn about data quality and constraints.
