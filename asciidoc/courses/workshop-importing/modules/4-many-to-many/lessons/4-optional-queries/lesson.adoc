= Optional: Practice Multi-Hop Queries
:type: lesson
:order: 4
:optional: true
...
.ANTON's top product
----
MATCH (c:Customer {customerId: 'ANTON'})-[:PLACED]->(:Order)-[r:ORDERS]->(p:Product)  // (1)
...
----
MATCH (p:Product)                      // (1)
WHERE NOT (p)<-[:ORDERS]-(:Order)      // (2)
RETURN p.productName, p.unitPrice      // (3)
ORDER BY p.unitPrice DESC;             // (4)
----

<1> **All products** - Start with every product
<2> **Negative pattern** - Filter to products WITHOUT incoming ORDERS relationships
...
----
MATCH (me:Customer {customerId: 'ALFKI'})-[:PLACED]->(:Order)-[:ORDERS]->(p:Product)  // (1)
MATCH (p)<-[:ORDERS]-(:Order)<-[:PLACED]-(other:Customer)                             // (2)
WHERE other <> me                                                                      // (3)
...
----
MATCH (p1:Product {productName: 'Chai'})<-[:ORDERS]-(o:Order)-[:ORDERS]->(p2:Product)  // (1)
WHERE p2 <> p1                                                                          // (2)
...
----
MATCH (c:Customer {customerId: 'ALFKI'})-[:PLACED]->(:Order)-[r:ORDERS]->(p:Product)-[:IN_CATEGORY]->(cat:Category)  // (1)
