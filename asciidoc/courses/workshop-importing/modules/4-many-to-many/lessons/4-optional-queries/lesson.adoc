...
You have the complete Customer→Order→Product path. In this lesson, you will practice complex multi-hop queries and aggregations.
...
[.slide]
== Finding a customer's most ordered product
...
.ANTON's top product
----
MATCH (c:Customer {id: 'ANTON'})-[:PLACED]->(:Order)-[r:CONTAINS]->(p:Product)  // (1)
RETURN p.name,                                                                  // (2)
...
* Add customer: `MATCH (c:Customer)-[:PLACED]->(o:Order)...` and return `c.name`


[.slide]
== Finding customers with similar purchases
...
----
MATCH (me:Customer {id: 'ALFKI'})-[:PLACED]->(:Order)-[:CONTAINS]->(p:Product)  // (1)
...
[.slide]
== Discovering product co-purchases
...
----
MATCH (p1:Product {name: 'Chai'})<-[:CONTAINS]-(o:Order)-[:CONTAINS]->(p2:Product)  // (1)
WHERE p2 <> p1                                                                          // (2)
RETURN p2.name,                                                                  // (3)
...
[.slide]
== Analyzing customer product affinity
...
----
MATCH (c:Customer {id: 'ALFKI'})-[:PLACED]->(:Order)-[r:CONTAINS]->(p:Product)-[:IN_CATEGORY]->(cat:Category)  // (1)
RETURN cat.name,                                                                                              // (2)
       sum(r.quantity) AS totalQuantity,                                                                              // (3)
...
In this lesson, you practiced multi-hop queries:
