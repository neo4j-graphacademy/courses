= Multi-Hop Traversals: The Foundation of Recommendations
:type: lesson
:order: 3
...
You created ORDERS relationships and completed the Customer→Order→Product path. Now you'll see the full power of graph traversals.
...
This enables powerful traversals:
...
----
MATCH (c:Customer {customerId: 'ALFKI'})-[:PLACED]->(:Order)-[:ORDERS]->(p:Product)  // (1)
...
[.slide]
== Query 2: Who Bought This Product?
...
----
MATCH (c:Customer)-[:PLACED]->(:Order)-[:ORDERS]->(p:Product {productName: 'Chai'})  // (1)
...
[.slide]
== Query 3: Customer's Favorite Products
...
----
MATCH (c:Customer {customerId: 'ALFKI'})-[:PLACED]->(:Order)-[r:ORDERS]->(p:Product)  // (1)
...
----
MATCH (:Customer)-[:PLACED]->(:Order)-[r:ORDERS]->(p:Product)  // (1)
RETURN p.productName,                                           // (2)
       sum(r.quantity) AS totalQuantity,                        // (3)
...
----
MATCH (c:Customer)-[:PLACED]->(:Order)-[r:ORDERS]->(:Product)  // (1)
...
[.slide]
== The Pattern for Recommendations
...
1. Find products I bought: `(me:Customer)-[:PLACED]->(:Order)-[:ORDERS]->(myProduct)`
2. Find customers who bought those products: `(myProduct)<-[:ORDERS]-(:Order)<-[:PLACED]-(other:Customer)`
3. Find what THEY bought that I haven't: `(other)-[:PLACED]->(:Order)-[:ORDERS]->(rec:Product)`

**You're one step away from the complete recommendation query!**


[.slide]
== SQL Comparison: The Complexity Grows
...
**Cypher (concise, fast):**
```cypher
MATCH (c:Customer)-[:PLACED]->(:Order)-[r:ORDERS]->(:Product)
