...
// Adapted: Focused on ORDERS relationships with properties, pivot table transformation

[.slide.discrete]
== Challenge: Complete the Customer→Product Path

You learned how pivot tables become relationships. Now you will transform the `order_details` table into ORDERS relationships.

In this challenge, you will complete the critical path needed for product recommendations: Customer→Order→Product.


[.slide]
== Step 1: Download the order_details CSV
...
[.slide]
== Step 3: Upload order-details.csv
...
[.slide]
== Step 4: Create ORDERS Relationship
...
* Create an ORDERS relationship between them


[.slide]
== Step 6: Add Relationship Properties
...
[.slide]
== Step 7: Review Your Complete Model
...
1. Click *Run Import*
2. The tool will create **2,155 ORDERS relationships**
...
[source,cypher]
.Count ORDERS relationships
----
MATCH (o:Order)-[r:ORDERS]->(p:Product)  // (1)
RETURN count(r) AS ordersRelationships;  // (2)
----

<1> **Match pattern** - Find Order-Product connections
<2> **Count** - How many ORDERS relationships exist

You should see **2,155 ORDERS relationships** (the entire order_details table became relationships!).
...
----
MATCH (c:Customer {customerId: 'ALFKI'})-[:PLACED]->(o:Order)-[r:ORDERS]->(p:Product)  // (1)
...
* **Created** ORDERS relationships from Order to Product (2,155 relationships)
