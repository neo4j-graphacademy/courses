...
You learned how to import nodes using the Import tool.
...
[.slide]
== Automatic constraints in Import

The Import tool automatically creates uniqueness constraints when you mark ID properties.

For Northwind, these constraints are created when you set ID properties:
...
In the Import tool, look for the constraint indicators next to ID properties (usually a key icon).
...
* `Category.categoryName` - Finding categories by name


[.slide.discrete]
=== Adding indexes in the Import tool

Some versions of the Import tool allow you to add indexes directly. Look for an "Add Index" button in the details panel.
...
CREATE INDEX category_name FOR (c:Category) ON (c.categoryName);
----
...
* Properties mapped from tables/files
* Uniqueness constraints will be configured automatically
* Consider which properties need indexes
...
* **Indexes** - Speed up queries by creating efficient lookup structures
* **Import tool** - Automatically creates uniqueness constraints for ID properties
...
In the next module, you will learn about relationships and how to connect nodes in your graph.
