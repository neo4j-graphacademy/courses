...
**Talking Points:**
1. **Introduce Neo4j Aura as fully managed DBaaS**
...
3. **Download and save credentials (CRITICAL)**
...
4. **Introduce Neo4j components** (briefly - details in next lessons)
...
2. **Explain WHY each entity is a node** (critical - focus on reasoning)
...
3. **Contrast: Why NOT make certain things entities?**
...
5. **Explain why graph is better** (DEMO with SQL vs Cypher)
...
1. **Recap: Nodes represent entities**
   - "Nodes are for things that exist independently and have their own data"

2. **Walk through Northwind nodes with reasoning:**
...
3. **Explain Labels - WHY they matter**
...
6. **Multiple labels (brief mention):**
...
2. **Explain WHY relationships are better than relational approach** (CRITICAL - spend time here)
...
3. **Show the Northwind relationships:**
   ```
...
4. **Explain many-to-many handling:**
...
5. **Relationship properties** (CRITICAL - explain the WHY)
...
2. **Show Northwind properties** (don't read all, just highlight)
...
5. **Property vs Node Decision** (CRITICAL - spend time here)
...
3. **Test each use case with Cypher**
...
2. **Why Neo4j makes refactoring easy:**
   - "Schema is optional and flexible"
   - "No complex migrations like in relational DBs"
   - "Can be done incrementally"

3. **Common refactoring patterns** (show 2-3 examples)
...
3. **Compare import options** (show table)
...
4. **Why Data Importer for this workshop:**
...
2. **Tour the interface** (point to each area)
...
2. **DEMO: Import Customer node** (do this slowly, let them follow)
...
3. **Let participants do Product node:**
...
4. **Quickly show remaining nodes:**
   - Category (categoryId)
   - Supplier (supplierId)
   - Order (orderId)
   - Employee (employeeId)
...
1. **Explain relationship requirements:**
...
2. **DEMO: Customer PLACED Order** (do this slowly)
...
3. **Let participants create these relationships:**
   - Product PART_OF Category
   - Product SUPPLIED_BY Supplier
   - Employee SOLD Order

4. **DEMO: Order ORDERS Product** (has properties - important!)
...
7. **Open Query tool and run first query:**
   ```cypher
   MATCH (c:Customer)
   RETURN c
   LIMIT 10;
...
1. **Simple queries - Find nodes by label:**
...
5. **Access relationship properties:**
...
8. **Filtering aggregates with WITH:**
...
1. **Recap what we've accomplished:**
