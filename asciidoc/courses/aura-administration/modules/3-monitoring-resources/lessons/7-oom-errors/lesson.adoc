...
Review your batch operation queries and ensure they use `LIMIT` clauses to restrict result set sizes. Break large operations into smaller batches using `CALL { } IN TRANSACTIONS` to process data incrementally rather than loading everything into memory at once.

Check that queries use aggregations efficiently. Instead of collecting all nodes into memory and then counting them, use `count()` directly to avoid materializing large result sets.

Paginate large result sets using `SKIP` and `LIMIT` to process data in manageable chunks. Keep transactions short and commit promptly to avoid holding memory for extended periods.

=== Intermittent Out of Memory Errors

If Out of Memory errors occur intermittently throughout the day without a clear pattern, your instance likely lacks sufficient memory capacity for the workload. In these cases, you may need to scale your instance to provide more heap memory.

Intermittent Out of Memory errors indicate that your workload occasionally exceeds the available memory, even during normal operations. This suggests the instance size is insufficient for peak load or that data growth has increased memory requirements beyond the current capacity. Scale your instance to a larger size that provides adequate headroom for your workload, then monitor the Out of Memory error metric to confirm the issue is resolved.
