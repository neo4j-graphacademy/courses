...
[NOTE]
.Query parameter obfuscation
====
Query parameter values are obfuscated in the logs for security. If your query uses a parameter like `$id`, the log will show `$id=*****` rather than the actual value.
====
...
* **Queries with high allocatedBytes** - These queries are significant memory consumers. Large memory allocations can impact overall instance performance.
* **Queries with high planning time** - Indicates time spent creating execution plans. If you use parameterized queries, planning time should be minimal since plans are cached and reused.
