...
* The time when you first noticed the replication lag
* Which cluster member(s) are affected
* Any recent changes to your application or query patterns
* Whether the issue coincides with high write activity or specific operations

Provide this information to Neo4j support. Cluster replication is managed at the infrastructure level—you don't have direct access to cluster topology, network configuration, or inter-node communication settings. Support can view cluster health metrics, run network diagnostics, and restart or reconfigure cluster members as needed.
