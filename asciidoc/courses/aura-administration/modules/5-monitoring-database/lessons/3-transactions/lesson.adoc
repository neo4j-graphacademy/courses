...
The expected value for rollbacks is very low, typically less than 1% of committed transactions. Occasional rollbacks are normal, but high rollback rates indicate application errors, constraint violations, or concurrent write conflicts.
...
Explicitly specify read transactions in your application code to route route queries across the available instances in clustered environments.
