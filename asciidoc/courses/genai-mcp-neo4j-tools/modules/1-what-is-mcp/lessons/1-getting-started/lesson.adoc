...
An example of a host may be Claude Desktop, Amazon Q, or Cursor.
...
Tools can output a result as text or JSON.


== A real-world scenario 

Neo4j has created a number of MCP servers with tools to help with the entire lifecycle of a project.

For example, the `mcp-neo4j-cypher` *server* provides tools for describing the schema of a graph and executing Cypher queries.

To use these as part of your development workflow, you would need to install the server to your local machine.

For example, you can configure Claude Desktop as a *host* to use the `mcp-neo4j-cypher` server with the following configuration:
...
1. Give the server a unique name that you can recognise within the host.
2. The server is written in Python, so the `uvx` command will install the server and run it on your local machine.
3. The arguments specify the package to load from PyPI, and to use Standard Input/Output (stdio) to communicate with the server.
4. The environment variables specify the connection details that the MCP server will use.

The `uvx` command will download the package and execute it locally.  For MCP servers built with JavaScript, you will use the `npx` command instead.

Once configured, you can use the chat interface to query the database.
...
Claude Desktop will use the `get_neo4j_schema` tool to obtain information about the database, use the information to generate a Cypher statement, use the `read_neo4j_cypher` tool to execute the query and then generate a friendly response.
