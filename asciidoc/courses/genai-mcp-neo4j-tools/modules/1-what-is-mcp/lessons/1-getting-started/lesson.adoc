...
The link:https://modelcontextprotocol.io[Model Context Protocol^] (MCP) is an open protocol developed by link:https://anthropic.com[Anthropic^] that enables AI agents and assistants to access tools and resources in a standardized way.

To use MCP effectively it is important to understand the concept of an **Agent**.
...
* **MCP Architecture** consists of four key components:
  ...
  - **Hosts** (like VS Code) manage clients and decide which tools to use
