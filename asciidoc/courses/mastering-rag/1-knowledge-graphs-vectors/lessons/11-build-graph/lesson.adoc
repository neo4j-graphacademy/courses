...
In this lesson, you will create a graph of the course content using the `neo4j` GraphRAG package for Python driver and OpenAI API. 
...
The pipeline allows us to chain various components, such as text splitters, embedders, and LLM graph extractors together in order to build a knowledge graph
...
We then connect to LLM extractor to the Neo4j writer component to let the pipeline know we want to write the nodes, relationships, chunks, and embeddings we extracted in previous steps to the Neo4j database.
