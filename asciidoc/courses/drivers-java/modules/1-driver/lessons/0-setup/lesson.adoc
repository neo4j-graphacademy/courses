...
If you wan to experiment with Neo4j and Java during this course you can setup a development environment.
...
[source, xml]
----
include::{repository-raw}/{branch}/pom.xml[]
----
====

== Setup the environment

Create a copy of the `/src/main/resources/example.properties` file and name it `application.properties`.
...
You can test your setup by running `src/test/java/com/neo4j/app/AppTest.java` - this will attempt to load the values in the `application.properties` file and connect to the Neo4j sandbox.
...
You will see `Tests run: 2, Failures: 0, Errors: 0, Skipped: 0` and a `BUILD SUCCESS` message if you have set up your environment correctly. If any tests fail, check the contents of the `.application.properties` file.
