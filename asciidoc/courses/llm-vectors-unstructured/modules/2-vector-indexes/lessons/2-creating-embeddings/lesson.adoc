...
// TODO - add how to created embeddings using Cypher - when there is some consistency in the functions apoc.ml, genai.vector.encode, etc.
...
You will load the data into two nodes, `Question` and `Answer`, with a relationship, `ANSWERS`. The `Question` and `Answer` nodes will store the original text and an embedding as properties.
...
Review the following Cypher code to load the data before running it:
