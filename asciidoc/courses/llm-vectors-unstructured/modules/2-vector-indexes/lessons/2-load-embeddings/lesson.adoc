...
You can view link:{repository-blob}/main/quora_filter.py[the code that filtered the data^] in the link:https://github.com/{repository}[llm-vectors-unstructured repository^].
====
...
The link:{repository-raw}/main/solutions/quora_embeddings.py[`solutions/quora_embeddings.py`^] program in the link:{repository}[llm-vectors-unstructured^] repository created the embeddings by calling the OpenAI API for each question and answer, then adding the embeddings to the CSV file.
