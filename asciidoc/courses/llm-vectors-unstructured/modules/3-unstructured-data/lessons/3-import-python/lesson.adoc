...
You will load the content from the course link:https://graphacademy.neo4j.com/courses/genai-fundamentals/[Neo4j & Generative AI Fundamentals^].
...
* `asciidoc` - contains all the course content in asciidoc format
** `courses` - the course content
*** `genai-fundamentals` - the course name
...
. Create a `CharacterTextSplitter` object to split the content into chunks of text. You can learn more about text splitters in the link:https://api.python.langchain.com/en/latest/character/langchain_text_splitters.character.CharacterTextSplitter.html[LangChain documentation^].
...
For example, you can find lesson chunks relating to a specific question, "What are Generative AI models?":
