...
For example, the first lesson of the Neo4j & Generative AI Fundamentals course has the following path:

[source]
----
courses\genai-fundamentals\modules\1-generative-ai\lessons\1-what-is-genai\lesson.adoc
----

You can extract the following metadata from the path:

- `Course.name` - `genai-fundamentals`
- `Module.name` - `1-generative-ai`
- `Lesson.name` - `1-what-is-genai`
...
Next, create a function to extract the metadata from the chunk:
...
The Cypher statement references the `$course`, `$module`, `$lesson`, `$url`, `$text`, and `$embedding` parameters contained in the `data` dict.

[TIP]
.Pass parameters as a dictionary
====
The Cypher parameters are passed as a dictionary, `data`, to the `run` method.

The `run` method maps the dictionary keys to the parameter names in the Cypher statement.
====
...
[source, cypher]
----
MATCH (c:Course)-[hm:HAS_MODULE]->(m:Module)-[hl:HAS_LESSON]->(l:Lesson)-[cp:CONTAINS]->(p:Paragraph)
RETURN *
----
