...
- `Lesson.url` - `https://graphacademy.neo4j.com/courses/{Course.name}/{{Module.name}}/{Lesson.name}`
...
====
Remember to update the code to include your OpenAI `api_key` and use the `neo4j_url`, `username`, and `password` of your Neo4j sandbox.
...
image::images/course-graph-result.svg[Result from the Cypher, a graph showing Course, Module, Lesson and Chunk nodes and their relationships]
...
You can use the vector index and the graph to search for lessons.

[source, cypher]
.Find lessons about "Hallucination"
----
