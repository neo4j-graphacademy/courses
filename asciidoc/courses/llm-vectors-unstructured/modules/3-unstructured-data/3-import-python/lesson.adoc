...
. Uses the link:https://api.python.langchain.com/en/latest/document_loaders/langchain_community.document_loaders.directory.DirectoryLoader.html[`DirectoryLoader`^] class to load the content from the `asciidoc/courses` directory.
...
Once you have chunked the content, you can use Langchain to load, create embeddings, and store the chunks in a Neo4j graph database.

The link:https://python.langchain.com/docs/integrations/vectorstores/neo4jvector[`Neo4jVector`^] can be used to load and create an vector index for the chunks.
...
====
You must update the code to include your `open_api_key` and use the `url`, `username`, and `password` of your Neo4j sandbox.
...
. Creates a vector index called `lesson_chunks`.
...
For example, finding lesson chunks about "Hallucination".
...
CALL db.index.vector.queryNodes('lessonChunks', 6, userEmbedding)
YIELD node, score
