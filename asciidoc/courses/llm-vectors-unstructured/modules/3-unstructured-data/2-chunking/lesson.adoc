...
For example, if you split a set of Documents by paragraph, you could name the nodes `Documents` and `Paragraph` with a relationship `CONTAINS`. Alternatively, if you split a document by a number of characters, you may simply use the node label `Chunk`.
