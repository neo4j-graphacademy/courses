...
When storing the course content, you will create a node for each `Chunk` and a relationship `CONTAINS` between the `Lesson` and `Chunk` nodes.

image::images/graphacademy-lessons-chunks.svg[A graph showing lesson and chunk nodes. Connected by `CONTAINS` relationship.]
