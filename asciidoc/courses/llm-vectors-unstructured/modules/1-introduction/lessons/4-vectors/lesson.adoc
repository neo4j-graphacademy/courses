...
Vectors are used to represent data in a way that is easy to manipulate and analyze. 
...
For example, a vector with three numbers has a dimensionality of 3. A vector with 100 numbers has a dimensionality of 100.
...
An embedding is a vector that represents the data in a way that is useful for a specific task.

For example, in the previous lesson, you looked at an embedding for a movie plot. 
This embedding was a vector that represented the plot of a movie in a way that was useful for finding similar movies.
...
While its possible to create embeddings for individual words, it is more common to create embeddings for entire sentences or paragraphs. 
The meaning of a word can also change based on its context. 
