...
Cypher has a `percentCont()` function you can use to evaluate a set of numeric values to determine the value that is at a percentile within the values.
...
For this query, the elements are sorted is ascending order. [10,20,30,40,50,60,70,80]
Then the number of elements are determined. (8)
Then the number of elements <= 50%  place in the list is calculated as p1. (4)
The last value at p1 is saved as v1. (40)
Then the number of elements >= 50% place in the list is calculated as p2. (4)
...
This query will return one of the values in the list that is closest to the percetile:
