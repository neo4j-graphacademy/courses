...
In general, pattern comprehension performs better than using `collect()` because it uses a subquery.
...
The behavior of pattern comprehension is similar to using OPTIONAL MATCH and for this particular query, the elapsed time is reduced.
