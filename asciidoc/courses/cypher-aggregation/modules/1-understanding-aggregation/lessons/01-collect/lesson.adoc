...
----
 ...
 We specify a criteria (x:Movie) that we only want to look at Movie nodes.
...
WHERE size(Actors) <= 3
RETURN m.title AS Movie, Actors to come up with a query without subquery that returns same results as below
----
...
This query performs better than the previous query.
...
In the next Challenge, you will modify some queries to aggregate data returned.
