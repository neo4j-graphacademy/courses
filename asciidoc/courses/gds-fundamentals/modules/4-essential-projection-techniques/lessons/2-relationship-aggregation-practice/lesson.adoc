...
First, copy the following query into the sandbox, fill it in and run it to drop any existing graphs:
...
CALL gds.graph.????()
YIELD ????
WITH graphName AS droppedGraph
CALL gds.graph.????(????)
YIELD graphName
RETURN droppedGraph
----
...
====
[source,cypher]
.Project Actor-Director collaborations
----
...
. Drop the graph if it exists (`'user-genre-ratings'`)
. Match the pattern with the RATED relationship variable
. Aggregate with `count(r)` to count ratings
. Project with source, target, and relationship properties
. Make it undirected
...
====
[source,cypher]
.Drop existing user-genre-ratings graph
----
...
====
[source,cypher]
.Drop existing director-collaborations graph
----
...
You've practiced aggregating relationships using `count(r)` to count relationships between node pairs and assign a weight. In the next lesson, you'll face a challenge that requires you to design and build an aggregated projection completely independently.
