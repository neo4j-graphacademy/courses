...
[NOTE]
.Louvain is not fully deterministic
====
Louvain makes randomized tie-breaking decisions during community detection, so the exact `ranLevels` and `communityCount` you see may vary slightly from the numbers quoted here (and from one run to the next). The overall pattern -- convergence well before 10 levels, and a few hundred to a few thousand communities -- should still hold.
====
...
[source,cypher]
.Run Louvain with maxLevels configuration
----
CALL gds.louvain.stats('actor-network', {
  maxLevels: 3
})
...
.Run degree centrality in stats mode (replace ?????)
----
...
[source,cypher]
.Mutate PageRank scores to projection
----
...
[source,cypher,role=noplay]
.Mutate degree centrality to projection (replace ?????)
----
CALL gds.degree.?????(?????, {
  mutateProperty: '?????'
})
...
[source,cypher]
.Write PageRank scores to database
----
...
[source,cypher]
.Query PageRank scores from database
----
MATCH (a:Actor)
...
[source,cypher,role=noplay]
.Write degree centrality to database (replace ?????)
----
CALL gds.degree.?????(?????, {
  ?????: 'collaborationCount'
})
...
[source,cypher]
.Query degree centrality from database
----
MATCH (a:Actor)
...
[source,cypher]
.Estimate memory for PageRank write
----
...
.neo4j.conf caution
====
It is unwise to edit your neo4j.conf without fully understanding how or why.
====

You can estimate any execution mode:

[source,cypher]
.Estimate memory for PageRank stream
----
...
[source,cypher,role=noplay]
.Estimate degree centrality write (replace ?????)
----
CALL gds.degree.?????.?????('actor-network', {
  writeProperty: 'collaborationCount'
})
...
====
[source,cypher,role=nocopy noplay]
.Solution: Estimate degree centrality write
----
