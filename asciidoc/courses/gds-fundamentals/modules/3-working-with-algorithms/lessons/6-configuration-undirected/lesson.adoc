...
The command returns a table where every `actor` value is `null` and every `degree` value is the same number. Something has clearly gone wrong -- but what?
...
You should now see a list of movie titles in the `movie` column, and each row should share the same `degree` score -- the maximum cast size found in this dataset.
...
In this graph, several movies share the same maximum cast size. So, when we use 'number of `ACTED_IN` relationships' as the metric, we receive a slew of top results all tied at the same score.
