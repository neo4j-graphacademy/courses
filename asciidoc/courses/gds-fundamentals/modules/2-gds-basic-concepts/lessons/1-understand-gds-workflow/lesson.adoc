...
. **Call GDS** +
The `WITH gds.graph.project(...)` line is a regular Cypher `WITH` clause -- it just happens to call the `gds.graph.project()` function. This creates the in-memory graph from the nodes matched by the preceding Cypher statement, and aliases the returned projection record as `g`.
+
...
.Native Projection
====
There are several methods of graph projection in Neo4j. You may, for example, be familiar with Native projection. To project the same graph using Native projection, you would run this:
...
.Learn as you go
====
You'll see lots of unfamiliar functions like `gds.util.asNode(nodeId).name` and `.stream` throughout the course. Don't worry if you don't understand it yet -- you'll learn about and practice using each of these functions in detail as you progress.
====
