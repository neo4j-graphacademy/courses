...
1. Call the graph list procedure to return all graph information
...
----
RETURN gds.graph.exists('user-movie') AS userMovieExists, gds.graph.exists('fake-graph') AS fakeGraphExists // <1>
----

1. Return existence status for multiple graphs using exists as a function
...
1. Call the exists function with graph name (fill in function and name) to verify deletion
...
1. Drop graph with error suppression (won't fail if graph doesn't exist)
