...
YIELD graphName, schemaWithOrientation, degreeDistribution, density // <2>
RETURN graphName, schemaWithOrientation, degreeDistribution, density // <3>
----
...
This returns extended information about the `user-movie` graph. The `schemaWithOrientation` field contains node labels, relationship types, properties, and relationship orientation.
...
Scroll through the results and you should see several graphs including `user-genre` from the monopartite lessons, `user-movie` and `actor-movie` from the bipartite lessons, and any others you've created along the way.
...
----
CALL gds.graph.?????(?????) // <1>
YIELD graphName, schemaWithOrientation // <2>
RETURN graphName, schemaWithOrientation // <3>
----
...
- YIELD the fields you're interested in (graphName, schemaWithOrientation, memoryUsage, etc.)
