...
Now create a bipartite projection of movies and genres called 'movie-genre', then run node similarity on it—this time, complete both steps yourself.
...
**Step 2:** Run node similarity on your projection. Replace the graph name and choose a relationship type to write:
...
You've now practiced creating multiple bipartite projections from the Movies dataset and running node similarity on each one—consolidating your understanding of how label preservation enables different types of analysis. Each projection transformed the same data into different analytical contexts: User-Movie (collaborative filtering), Actor-Movie (collaboration patterns), and Movie-Genre (content-based similarity). In the next lesson, you'll put this knowledge to the test with a challenge that requires you to create your own bipartite projection and run node similarity independently.
