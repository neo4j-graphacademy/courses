...
[.transcript]
We will perform exploratory analysis as the first step of the analysis to get to know the dataset better.

== Survey demographics

We will begin by examining the population information.
First, we will take a look at the age distribution.
The APOC plugin offers a `apoc.agg.statistics` function that allows us to easily evaluate a distribution.
