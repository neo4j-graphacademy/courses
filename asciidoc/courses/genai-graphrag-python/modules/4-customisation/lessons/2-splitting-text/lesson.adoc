...
You create a custom text splitter by extending the link:https://neo4j.com/docs/neo4j-graphrag-python/current/api.html#textsplitter[`TextSplitter`^] class:
...
The `SectionSplitter` class splits the text based on a section heading (`== `), creates `TextChunk` objects for each section, and returns a `TextChunks` object containing all the chunks.
