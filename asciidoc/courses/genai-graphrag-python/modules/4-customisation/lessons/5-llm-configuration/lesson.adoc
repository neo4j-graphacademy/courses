...
For example, you could run the OpenAI open source model link:https://huggingface.co/openai/gpt-oss-20b[`openai/gpt-oss-20b`^] locally using an application such as link:https://lmstudio.ai/[LM Studio^]:
...
.Model parameters
====
Through `model_params` you can change how the model responds by adjusting model parameters such as `temperature` and `reasoning_effort`.
