...
----
include::{repository-raw}/{branch}/genai-integration-langchain/vector_cypher_rag.py[]
----
...
----
include::{repository-raw}/{branch}/genai-integration-langchain/solutions/vector_cypher_rag.py[tag=retriever]
----
...
. Pass the text from those chunks to the `GraphRAG` pipeline as context to the LLM.
...
The response includes information from the chunks.

Print the context passed to the LLM by adding the following to the end of the code:

[source, python]
.Print context
----
include::{repository-raw}/{branch}/genai-integration-langchain/solutions/vector_cypher_rag.py[tag=print_context]
----
...
The retriever will now execute the Cypher query for each chunk `node` adding more context.
...
_You can learn more about knowledge graphs in the Neo4j blog post linked here: link:https://neo4j.com/blog/what-is-knowledge-graph[What Is a Knowledge Graph?^]_. Additionally, you can explore further lessons on knowledge graphs on the GraphAcademy website, specifically in the course "GenAI Fundamentals," including the sections "What is a Knowledge Graph" and "Creating Knowledge Graphs."_
