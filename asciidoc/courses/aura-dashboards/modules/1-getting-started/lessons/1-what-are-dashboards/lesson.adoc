...
Ever wondered how the movie recommendation systems work, or how studios track with genres are trending? 
...
// Given the ease of use and management that Aura provides, it is an excellent choice for developers and organizations looking to leverage the power of graph databases without the operational overhead, and that includes analysing and visualising data with Dashboards.
...
The picture below shows a sample dashboard that visualizes movie data from the popular movie graph dataset, sorted by decade, represented as a line card:caption: 
