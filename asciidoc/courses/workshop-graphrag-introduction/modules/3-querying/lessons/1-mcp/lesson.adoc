...
* **MCP Hosts** are applications (for example, Claude Desktop, VS Code, or custom agents) that manage client connections and determine which tools to use
...
=====
The `NEO4J_DATABASE` environment variable specifies which database to query when executing Cypher statements.
=====
