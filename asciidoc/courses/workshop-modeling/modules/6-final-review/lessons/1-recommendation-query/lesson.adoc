...
To optimize the query, we can retrieve the list of products the customer has bought _once_ at the start of the query, rather than once per product purchased.
...
You can use the `EXCLUDE` relationship to collect a list of categories the customer has excluded, and use the `NOT` and `any()` to filter out products in excluded categories.
 ...
 filter out products in excluded categories.
