...
MATCH (me:Customer {id: 'ALFKI'})
      -[:PLACED]->(:Order)
      -[:CONTAINS]->(myProduct:Product)
WITH me,
  collect(DISTINCT myProduct) AS myProducts,
  [(me)-[:EXCLUDE]->(cat)|cat] AS excluded
UNWIND myProducts AS product
----
====
...
[source,cypher,role=noplay]
----
MATCH (other)-[:PLACED]->(o:Order)
      -[:CONTAINS]->(p:Product)
WHERE NOT p IN myProducts
  AND NOT any(cat IN
    [(p)-[:IN_CATEGORY]->(cat)|cat]
    WHERE cat IN excluded)
...
[.slide.col-2]
== Understanding category exclusions
...
WITH me,
  ...
  [(me)-[:EXCLUDE]->(cat)|cat] AS excluded    // <2>

UNWIND myProducts AS product                  // <3>

WHERE NOT any(cat IN                          // <4>
  [(p)-[:IN_CATEGORY]->(cat)|cat]
  WHERE cat IN excluded)
----
====
...
[.col]
====
<1> **Collect products** - Aggregate with `collect()`
<2> **Get exclusions** - List expression for preferences
<3> **Unwind** - Check each product individually
<4> **Category filter** - `any()` checks exclusions
====
