...
[source,mermaid]
.Respresenting customers who have placed an Order
....
graph LR
    C((Customer))
    O((Order))
...
**Think of relationships as verbs** - they describe actions or connections between nouns (Customer _placed_ an Order, Order _contains_ a Product).
...
[.slide]
== Why graph traversals scale well
...
====
[source,mermaid]
.Marriage is bidirectional
----
graph LR
    Dan((Dan)) ---|MARRIED_TO| Ann((Ann))
----

`MARRIED_TO` is symmetric - if Dan is married to Ann, Ann is married to Dan.
====
...
----
graph LR
    Dan((Dan)) -->|LOVES| Ann((Ann))
    Ann((Ann)) -->|INDIFFERENT_TO| Dan((Dan))
----

`LOVES` has a direction - if Dan loves Ann, it doesn't mean Ann loves Dan.
====
...
RETURN c.companyName, o.id, o.shipCountry         // <3>
LIMIT 5
----

<1> **Match pattern** - Find customers and their orders
<2> **Filter by date** - Filter orders by date range using the datetime() function
<3> **Return properties** - Show customer name, order ID, and ship country
