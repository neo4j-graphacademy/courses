...
`MARRIED_TO` is symmetric - if Alice is married to Bob, Bob is married to Alice. No arrow needed.
====

[.col]
====
[source,cypher]
.LIKES could be one-way
----
...
`LIKES` has a direction - Alice likes Bob doesn't mean Bob likes Alice. Use an arrow.
====
