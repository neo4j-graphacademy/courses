...
----
MATCH (me:Customer {id: 'ALFKI'})-[:PLACED]->(:Order)-[:CONTAINS]->(p:Product)
...
The query touches only the connected data it traverses—index lookup finds the starting node, then following relationships is pointer chasing in memory, with no repeated index scans or join materialization.

// **In SQL (equivalent logic):**

// [source,sql]
// .Complex SQL recommendation query
// ----
...
// **Why it's slow:** The database must scan and join entire tables (or large index ranges) for each join, so cost grows with table sizes and the number of joins.
...
// Graph databases are not optimized for problems where relationships don't matter or aggregations dominate.
...
If your queries involve 3+ JOINs or multi-hop relationships, consider a graph.
...
[source,cypher]
.Using CREATE - not idempotent
----
CREATE (c:Customer {id: 'ALFKI', name: 'Alfreds Futterkiste'})
...
----
MERGE (c:Customer {id: 'ALFKI'})
ON CREATE SET c.name = 'Alfreds Futterkiste', c.createdAt = datetime()
