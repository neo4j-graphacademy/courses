...
1. **Anchor:** Find Customer with id = 'ALFKI'
...
[source,text]
.Query without index - linear scan
----
Neo4j scans ALL Customer nodes → checks each id → finds match
Time grows with the number of customers (linear scan).
----
...
The index makes lookup fast and consistent regardless of how many customers exist.
----
...
**Note:** If you used the Import tool's "ID property" feature, these may already exist!
...
You should see constraints for Customer.id, Product.id, and Order.id.
...
Even multi-hop queries are fast when the anchor is indexed!
...
**For this query:** Label scan is OK because the query needs all customers. No index needed.
...
.Recommended indexes
----
unitPrice, country, date
----

✅ **Labels specified in MATCH**

[source,text]
.Specify labels for performance
----
...
[source,text]
.Constraints vs indexes decision guide
----
Constraint: Customer.id (unique)
Constraint: Product.id (unique)
Constraint: Order.id (unique)
