...
Neo4j offers several import methods depending on your data source, volume, and requirements.
...
flowchart TD
    Start["What's your<br/>data source?"]:::muted --> Source

    Source{"Source type?"}
    Source -->|"Tabular files<br/>(CSV, TSV)"| Volume{"Data volume?"}
    Source -->|"Relational DB<br/>(PostgreSQL, MySQL, etc.)"| DB["Import tool<br/>(direct connection)"]:::highlight
    Source -->|"Data warehouse<br/>(BigQuery, Snowflake)"| DW["Import tool<br/>(direct connection)"]:::highlight
    Source -->|"Streaming data<br/>(Kafka, Kinesis)"| Stream["Spark / Kafka<br/>Connector"]:::option
    Source -->|"Custom API<br/>or application"| Custom["Neo4j Drivers<br/>(Python, Java, etc.)"]:::option

    Volume -->|"< 1M rows"| Small["Import tool ✓"]:::highlight
    Volume -->|"1M - 10M rows"| Medium{"Need<br/>transformation?"}
    Volume -->|"> 10M rows"| Large["neo4j-admin import<br/>(offline, fastest)"]:::option

    Medium -->|"Simple mapping"| MediumSimple["Import tool"]:::highlight
    Medium -->|"Complex ETL"| MediumComplex["LOAD CSV +<br/>Cypher"]:::option

    classDef highlight fill:#f4f5ff,stroke:#c7d2fe,stroke-width:1.25px,color:#3730a3
    classDef option fill:#fff7ed,stroke:#fed7aa,stroke-width:1.25px,color:#9a3412
...
| **Import tool** | < 1M rows, visual modeling, databases/warehouses
| **LOAD CSV** | 1-10M rows, complex transformations
| **neo4j-admin** | > 10M rows, fastest bulk load (offline)
| **Drivers** | Custom apps, real-time integration
| **Spark/Kafka** | Streaming data, big data pipelines
|===
====
