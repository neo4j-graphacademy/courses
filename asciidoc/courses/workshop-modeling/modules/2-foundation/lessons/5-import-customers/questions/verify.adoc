[.verify.slide]
= Validate Import

Once you have imported the Customer nodes, click the **Check Database** button to verify that the task is complete.

verify::[]

[TIP,role=hint]
.Hint
====
Follow the same steps as the previous lesson where you imported Product nodes.

Make sure to:

. Set the label to `Customer`
. Map from `customers.csv`
. Set `customerId` as the ID property
. Click Preview to review
. Click Run Import
====

[TIP,role=solution]
.Solution
====
The Customer node should be configured with:

. **Label:** `Customer`
. **File:** customers.csv
. **ID property:** customerId
. **Key properties:** companyName, country, city

After running the import, you should have 91 Customer nodes in your graph.

:zip-url: https://cdn.graphacademy.neo4j.com/courses/workshop-modeling/modules/2-foundation/lessons/5-import-customers/data/5-import-customers.zip
:zip-filename: 5-import-customers.zip

include::../../../../../includes/load-solution.adoc[]
====
