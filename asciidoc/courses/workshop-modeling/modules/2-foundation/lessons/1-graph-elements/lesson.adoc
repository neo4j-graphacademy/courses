...
* Using an index for fast lookup: O(1) constant time
* Without an index: Scanning all nodes O(n) linear time
====
...
* Traversing is O(k) where k = number of connections
* Much faster than computing JOINs in SQL
====
...
1. Scan customers table → O(n)
2. JOIN orders table → O(n × m)
3. JOIN order_details → O(n × m × p)
4. JOIN products → O(n × m × p × q)


Query cost grows with **table sizes**.
====
...
1. Index lookup customer → O(1)
2. Follow PLACED pointers → O(k₁)
3. Follow CONTAINS pointers → O(k₂)
4. Return products → O(k₃)


Query cost grows with **connections**, not table size.
====

This is why the recommendation query we will build is fast - it only touches relevant data.
...
You will see this in action when we import customers and products.
