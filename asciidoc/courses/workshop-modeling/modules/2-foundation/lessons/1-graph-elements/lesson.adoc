...
RETURN c.id, count(o) AS orderCount
...
**Why this matters:** SQL JOINs rebuild connections at query time. Graph databases store connections directly, so traversals are pointer follows in memory - much faster for relationship-heavy queries like recommendations.
