...
flowchart TD
    ...
    Volume1 -->|"< 1M rows"| Import["Import tool ⭐<br/>(What we use today)"]
    Volume1 -->|"1M - 10M rows"| LoadCSV[LOAD CSV<br/>in Cypher]
    Volume1 -->|"> 10M rows"| Admin[neo4j-admin import<br/>Offline bulk load]

    DBChoice -->|Few tables,<br/>simple queries| Export["Export to CSV<br/>→ Import tool"]
...
* **Learning focused** - Great for understanding graph modeling
