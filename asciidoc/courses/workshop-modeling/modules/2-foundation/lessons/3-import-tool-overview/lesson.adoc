...
flowchart TD
    Start["What are you importing?"]:::muted --> Source{Data Source?}
    ...
    Source -->|APIs/Real-time| Custom[Custom Application<br/>Neo4j Drivers]:::primary
    Source -->|Multiple Sources| ETL[ETL Tool<br/>Apache Hop]:::primary

    Volume1 -->|"< 1M rows"| Import["Import tool ⭐"]:::highlight
    Volume1 -->|"1M - 10M rows"| LoadCSV[LOAD CSV<br/>in Cypher]:::primary
    Volume1 -->|"> 10M rows"| Admin[neo4j-admin import<br/>Offline bulk load]:::primary

    DBChoice -->|Few tables<br/>and simple<br/>volume/complexity| Import
    DBChoice -->|Complex queries<br/>or transformations| ETL

    Import --> Note1["✓ No-code visual interface<br/>✓ Upload tabular files<br/>✓ Define model visually<br/>✓ Best for learning"]:::forest
    LoadCSV --> Note2["✓ Programmatic control<br/>✓ Cypher transformations<br/>✓ Batch processing"]:::muted
    Admin --> Note3["✓ Fastest for large datasets<br/>✓ Requires database offline<br/>✓ Pre-formatted files"]:::muted
    ETL --> Note4["✓ Multiple data sources<br/>✓ Complex transformations<br/>✓ Scheduled pipelines"]:::muted
    Custom --> Note5["✓ Real-time integration<br/>✓ Complex business logic<br/>✓ Full control"]:::muted
