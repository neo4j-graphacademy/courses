...
* **No JOINs**—relationships are pre-materialized pointers
* **O(k) complexity**—query cost scales with connections (k), not table size (n)
* **Index lookup** finds anchor node in O(1), then follows pointers
...
* Performance model - O(1) index lookup + O(k) pointer traversal
...
* Transform pivot tables into relationships
...
[.slide]
=== Module 4: Build the Recommendation Query
...
We have created a Neo4j instance for you to use throughout this workshop.
...
Any links to the Aura Console will open in a new browser with your instance credentials pre-filled.
endif::[]
...
We will use the Import tool to design your graph data model and the Query tool to query your graph database.
...
* **The solution** - Cypher does it in 11 lines with direct relationship traversals
* **Performance** - O(k) graph traversal vs O(n × m) table joins
* **What you'll build** - Complete graph with Products, Customers, Orders, collaborative filtering algorithm
* **Workshop structure** - 4 modules: Foundation → Modeling Relationships → Many-to-Many → Build Query
