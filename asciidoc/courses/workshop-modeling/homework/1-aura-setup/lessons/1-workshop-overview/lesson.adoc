...
* **80x faster**—graph traversals follow pointers in memory; cost scales with connections traversed, not table sizes, and deep traversals are more predictable and often much faster than equivalent multi-join SQL
...
* Learn the Import tool tool
* Understand Query and Explore tools
* Load all data files with one button
...
* Transform join table into relationships
...
* 10 lines vs 38 lines, 80x faster
...
* **The challenge** - SQL requires 38 lines with 7 JOINs and is slow because each join adds index scans and materialized result sets
* **The solution** - Cypher does it in 10 lines and is 80x faster by following pointers in memory instead of repeated index lookups and join materialization
