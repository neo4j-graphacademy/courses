...
[NOTE]
.GraphAcademy Sandbox vs. Manual Aura Setup
====
**For GraphAcademy delivery:** The online course uses a pre-provisioned sandbox (`:usecase: blank-sandbox`). Learners do not need to create their own Aura instance - credentials are provided automatically in the course interface.

**For in-person or live delivery:** If you're delivering this workshop outside GraphAcademy, learners will need to create their own Aura Free instance. The "Create Your Aura Instance" lesson below covers this setup. Skip this lesson when using the GraphAcademy sandbox.
====
...
* link:https://graphacademy.neo4j.com/courses/neo4j-fundamentals[Neo4j Fundamentals^] - Cypher query language fundamentals
* link:https://graphacademy.neo4j.com/courses/modeling-fundamentals[Modeling Fundamentals^] - Graph data modeling concepts and practices
* link:https://graphacademy.neo4j.com/courses/importing-fundamentals[Importing Data Fundamentals^] - Import tool tool and CSV import workflows
* link:https://graphacademy.neo4j.com/courses/aura-fundamentals[Neo4j Aura Fundamentals^] - Neo4j Aura setup and management
...
* Import tool interface (image: `01_data_importer_main.jpg`)
...
[source,cypher]
----
MATCH (c:Customer {id: 'ALFKI'})-[:PLACED]->(:Order)-[:CONTAINS]->(p:Product)
...
* Side-by-side: SQL with join table vs Cypher with relationship
...
. *Explain WHY relationships are better than relational approach* (CRITICAL - spend time here)
   ...
   * "In relational DBs, you model connections with foreign keys and join tables"
   * Show diagram of orders table with foreign key
   * Show order_details join table connecting orders and products
...
FROM orders o
JOIN order_details od ON o.orderID = od.orderID  -- join table
JOIN products p ON od.productID = p.productID
...
[source,cypher]
----
MATCH (o:Order {id: 10248})-[rel:CONTAINS]->(p:Product)
RETURN p.name, rel.quantity, rel.unitPrice
----
...
. *Why Import tool for this workshop:*
...
*Key Message:* "Different import methods for different scenarios - Import tool is perfect for learning and CSV-based imports."

'''

=== Lesson 2: Introduction to Import tool (5 min)

*What to Show:*

* Open Import tool in Aura
...
. *Access Import tool:*
   * "Go to your Aura instance → Click Open → Select Import tool"
...
*Key Message:* "Import tool gives you a visual, point-and-click way to build your graph without writing any code."
...
. *DEMO: Order ORDERS Product* (has properties - important!)
   ...
   * "It's the join table from relational world"
...
* Constraints panel in Import tool
...
. *Constraints already created:*
   * "When you set ID properties, Import tool created uniqueness constraints"
...
. *In Import tool:*
   ...
   * "Import tool handles this for you automatically"

*Key Message:* "Constraints and indexes ensure data quality and query performance - Import tool creates them automatically on ID properties."
...
*Key Message:* "Import is one click - Import tool does all the heavy lifting behind the scenes."
...
----
MATCH (c:Customer {id: 'ALFKI'})-[:PLACED]->(o:Order)
RETURN c.companyName, o.id, o.date
ORDER BY o.date
----
...
RETURN o.id, o.date, o.requiredDate
LIMIT 10
----
...
*Issue: Import tool shows "Connection Error"*

* Check if instance is running (should show green status)
* Try refreshing the browser
* Check browser console for errors
* Try closing and reopening Import tool
