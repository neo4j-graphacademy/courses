...
. Upload your CSV file (drag and drop or click to browse). If you have a zip of Northwind CSVs, unzip it first—Data Importer does not accept zip files directly.
+
...
For this course, download the Northwind CSV files from link:https://github.com/neo4j-graph-examples/northwind/tree/main/import[Northwind CSV files^] and upload each file you need (e.g. `customers.csv`, `orders.csv`, `products.csv`, and the rest).
...
<<<<<<< HEAD
. If using Workspace (not Aura), connect to your database when prompted
=======
>>>>>>> 70de014d9 (removed sandbox references)
