...
In this lesson, you will learn how to use the Neo4j Data Importer to upload CSV files, model your data, and create nodes.
...
The Data Importer canvas starts blank, ready for you to design your graph model:
...
. Enter the label name (e.g., `Customer`, `Product`, `Order`)
...
* Rename columns to follow Neo4j naming conventions (e.g., `customer_id` → `customerID`)
...
. Give your data model a name (e.g., "Northwind Model")
. Click **Save**
...
After importing, verify your data in the Query tool:
...
When importing nodes, watch out for these common mistakes:
