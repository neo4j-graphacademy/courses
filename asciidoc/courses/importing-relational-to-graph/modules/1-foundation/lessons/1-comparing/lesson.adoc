...
====
**Wrong:** Converting every table to a node.

**Reality:** Entity tables (customers, products, orders) → nodes. Junction tables (e.g. `order_details`) → relationships with properties. Lookup tables → often properties. Audit tables → often stay out.
====
...
====
**Wrong:** Columns map to relationships.

**Reality:** Rows → node instances. Columns → properties. Foreign keys → relationships. Junction tables → relationships with properties.
====
