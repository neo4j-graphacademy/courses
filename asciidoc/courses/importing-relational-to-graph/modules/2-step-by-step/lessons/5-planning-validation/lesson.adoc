...
In this lesson, you will plan constraints and indexes for your graph model. **Constraints** maintain data integrity (uniqueness, existence). **Indexes** speed up property lookups—they are not used for "joins" like in SQL; in a graph you traverse relationships directly.
...
* **Constraints** ensure data uniqueness and existence (and enforce property data types where applicable)
* **Indexes** improve query performance for **property lookups** (e.g. finding nodes by `customerID`, filtering by `country`); they are not used for relationship traversal (unlike SQL indexes that support JOINs)
...
Ensures a property value is unique across all nodes with a specific label. The constrained property has a type (string, integer, etc.) inferred from the data; Neo4j will enforce uniqueness for that property.
...
This is equivalent to a primary key in relational databases. Creating a unique constraint also creates a backing index used for MERGE and lookups.
...
== Planning constraints (Northwind as example)
...
== Indexes: for lookups, not for JOINs

In a relational database, indexes are often used to speed up JOINs. In Neo4j, **indexes are used to find starting nodes or to filter by property values**—e.g. "find the Customer with this ID" or "find all Orders where shipCountry = 'Germany'". Relationship traversal does not use indexes; it follows stored relationship pointers. Plan indexes for properties you filter on or look up by.
...
== Planning indexes (Northwind as example)

Consider your query patterns when planning indexes. Example patterns (Northwind-style) might include:
...
[TIP]
.Best practices: constraints and indexes
====
* **Create constraints before importing data** — Ensures uniqueness from the start and enables efficient MERGE operations.
* **Create indexes after importing data** — For large imports, creating indexes afterward can be faster.
* **Index properties used in WHERE clauses** — Focus on properties you filter by frequently (lookups and filters, not "joins").
