...
== Understanding a relational schema (Northwind as example)

image::images/ER.png[Northwind Entity-Relationship Diagram showing all tables and their relationships]

The principles below apply to any relational schema. The course uses **Northwind** as the example—a small sample schema. First identify entity tables (e.g. customers, products), junction tables (e.g. order_details), and lookup tables in your own database or in Northwind.
...
As covered in Module 1: entity tables become nodes, junction tables become relationships, lookup tables may become properties or small nodes. The following sections apply this using Northwind as the example; use the same logic on your schema.


== Analyzing tables (Northwind example)
...
=== Composite keys and join table primary keys

In relational schemas, **join tables** often have a **composite primary key** (e.g. `(order_id, product_id)` in `order_details`). There is no separate "row ID"; the pair of foreign keys uniquely identifies the row. When mapping to a graph, you do not create a node for that table—you create one relationship per row. The relationship is uniquely identified by the pair of nodes it connects (Order + Product), and optional properties (e.g. `quantity`, `unitPrice`) come from the other columns. If you need to reference a specific order line elsewhere, you can add a synthetic ID as a relationship property (e.g. `lineId`) or use the combination of order ID and product ID in your queries. The lesson *Mapping join tables to relationships* goes into this in more detail.
...
image::images/node-or-property-1.svg[Product with language as a property: language stored as 'English' or 'French', width=90%]
...
image::images/node-or-property-2.svg[Language as a separate node connected to Product nodes, width=65%]
...
Tables that track changes or history may not belong in the main graph—or they may, depending on how you use them.

**Example:** An `order_audit` table tracking who modified orders and when:

* If you rarely query this data or only need sequential or compliance-style queries, keep it in the relational database.
* If you need to analyze it as a graph (e.g. who changed what in common patterns, or decision traces), a graph can help. Audit logs are similar in spirit to "context graphs" used for explainability of agents: decision traces and change history are less about strict sequence than about common patterns of behavior—the same reason fraud detection often benefits from a graph. In that case, consider a dedicated audit or context graph rather than overloading the main operational graph.
* For hybrid setups, keeping audit in relational (or a dedicated log store) avoids moving high-volume, append-heavy data and keeps the graph focused on core entities.
...
== The graph model (Northwind example)

Based on this analysis, the Northwind graph model includes the following. Your own model will follow the same pattern for your entities:
...
| Standard Neo4j naming convention; see link:https://neo4j.com/docs/cypher-manual/5/styleguide/[Cypher style guide^] for full conventions
...
* Which tables in your schema (or in Northwind) will become nodes in the graph
