...
You can develop your graph data model visually using the Neo4j Data Importer:

=== Neo4j Data Importer (Aura)

The Data Importer provides built-in modelling:
...
You can **Define manually** (add nodes and relationships on the canvas) or use **Generate with AI** to suggest a model from your data; if the AI creates relationships in the wrong direction, use **Reverse direction** and then fix relationship type names as needed. The Data Importer combines modelling and import in one tool—you design the model and import data in the same interface.
...
* **Schema-optional model** — You create the graph data model as you import; no predefined schema is required
* **Model and import** — The data model (node labels, relationship types, properties) is defined during the import process
* **Design for use case** — Design the model for your query objectives, not for a one-to-one copy of the source tables

In the next lesson, you will use the Data Importer to create nodes from your CSV (or Northwind) and run your first import.
