= Analyzing your relational schema
:order: 1
:type: lesson

== Introduction

Before designing a graph model, you need to understand your source schema: which tables exist, how they link, and what the data looks like.

In this lesson, you will learn how to explore a relational schema and understand its structure so you can map it to a graph. The examples use the **Northwind** sample so you have a concrete reference; apply the same steps to your own database.


== Schema overview (Northwind as example)

The following tables are the focus for graph modelling in the Northwind sample. Use the same analysis approach on your own schema.
...
To design your graph model, you need to understand the source schema: tables, columns, and foreign keys. The queries below use a common system catalog style (e.g. `information_schema`, as in PostgreSQL); adapt table/schema names to your database (BigQuery, Snowflake, MySQL, etc.). The course is about importing relational data into Neo4j—the source database is your choice.
...
In this lesson, you explored how to analyze a relational schema (using Northwind as the example) and learned:
