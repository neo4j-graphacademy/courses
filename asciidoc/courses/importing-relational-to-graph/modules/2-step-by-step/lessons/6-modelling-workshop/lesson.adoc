...
Your challenge is to design a graph model for your own relational schema (or the Northwind sample) using the Neo4j Data Importer in Aura.
...
You will use the Neo4j Data Importer in Aura. It is built into AuraDB and provides a visual modelling canvas alongside the import functionality.

**Ways to build your model in Data Importer:**

* **Generate with AI** — You can have the tool suggest a model from your data. The AI sometimes creates relationships in the wrong direction; use the **Reverse direction** control to fix them, then manually update the relationship type name if needed.
* **Define manually** — Build the model yourself on the canvas: add node labels, properties, and relationships by dragging between nodes. No database connection is required until you run the import.
...
. Use the visual canvas to define nodes and relationships (or use **Generate with AI** and then correct directions and names)
...
== Getting Started with Neo4j Data Importer
...
The Data Importer accepts local files only. Use CSV exports from your own database, or if you are using the Northwind example: download from link:https://github.com/neo4j-graph-examples/northwind/tree/main/import[Northwind CSV files^] (unzip locally if needed).

. In the **Files** panel, click **New data source** and choose **.CSV/.TSV (local)**
. Upload each CSV you need: `customers.csv`, `orders.csv`, `products.csv`, `categories.csv`, `suppliers.csv`, `employees.csv`, `shippers.csv`, `order-details.csv` (or drag and drop into the Files panel)
...
Create nodes for each of the main entities in your schema (or in Northwind, the main entities are):
...
For each node, add the main properties. In the Data Importer:

. Select a node (or add one with **Add node label**)
. In the Definition panel, map CSV columns to properties
...
Now connect the nodes with relationships. In the Data Importer:

. Hover over the edge of the source node and drag to the target node
. Set the relationship type and map **From** and **To** IDs to the CSV columns
. Add relationship properties (e.g. for CONTAINS: quantity, unitPrice, discount) from the CSV
...
// TODO: Add screenshot of complete Data Importer model
...
Data Importer saves your model automatically. You can also export it:

=== Download model (with data)

. Click the **...** menu in the Data Importer
. Select **Download model (with data)**
. This saves your model and data for backup or to share with others

You can reload the downloaded file later to continue editing or run the import again.
...
* Using the Neo4j Data Importer to design graph models
...
This optional workshop provided hands-on practice with graph modelling tools. You created a visual representation of your graph model (or the Northwind example) that you can implement in the following modules.

In the next module, you will use the Neo4j Data Importer to import your data (or the Northwind sample) into Neo4j.
