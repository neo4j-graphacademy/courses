= Import nodes from your relational data
:order: 10
:type: challenge
:disable-cache: true
:shared-dir: ../../../../shared
...
Your challenge is to use the Neo4j Data Importer to upload your own CSV files (or the Northwind sample), model your data, and create nodes.

To complete this challenge:

. Upload your CSV files to Data Importer (or use the Northwind CSVs as a working example)
. Create node labels for your main entities (e.g. Customer, Order, Product—or the Northwind equivalents: Customer, Order, Product, Category, Supplier, Employee, Shipper)
...
Access the Data Importer via AuraDB:
...
Data Importer accepts local files only (no remote URLs). It supports CSV (Comma Separated Values) and TSV (Tab Separated Values).

. Click **New data source** in the Files panel and choose **.CSV/.TSV (local)**
+
image::images/01_data_importer_source.jpg[Add new data source,width=450,align=center]

. Upload your CSV file (drag and drop or click to browse). If you use a zip of CSVs (e.g. Northwind), unzip it first—Data Importer does not accept zip files directly.
+
...
If you are using the Northwind example, download from link:https://github.com/neo4j-graph-examples/northwind/tree/main/import[Northwind CSV files^] and upload each file you need (e.g. `customers.csv`, `orders.csv`, `products.csv`). Otherwise use your own CSV exports.
...
[NOTE]
.Optional deep-dive: generated Cypher
====
The Data Importer translates your model and CSV data into Cypher statements (e.g. MERGE for nodes, CREATE for relationships). If you want to see or reuse those statements, check the Data Importer UI or export options for a way to view or copy the generated Cypher. Understanding the generated statements helps when debugging imports or when moving from the UI to scripted imports.
====
...
. Give your data model a name (e.g. "Northwind Model" if using the sample, or a name for your own schema)
. Click **Save**
...
In this challenge, you imported nodes from your data (or Northwind) using the Data Importer. In the next lesson, you will create relationships between them.
