...
.Running Cypher
====
The techniques in this lesson apply to Neo4j Aura and the GraphAcademy Sandbox. For Cypher-based solutions, use the Query tool in your AuraDB instance or Neo4j Browser / Query tab in the Sandbox.
====
...
In many schemas (e.g. Northwind), a `region`-style column is often NULL for some rows:
...
=== Batched transactions (recommended)

Use `CALL { ... } IN TRANSACTIONS OF n ROWS` (shown above) for large imports. This is the current, supported way to batch writes in Cypher. Avoid legacy syntax such as `USING PERIODIC COMMIT`, which is deprecated.

// TODO: Remove any remaining legacy transaction references once all audiences are on supported Neo4j versions.
...
As a general guideline, create unique constraints before importing any data.
...
**Example import order (Northwind or similar):**
...
====
As a general guideline, back up your database before running DELETE operations, especially DETACH DELETE, which removes nodes and all their relationships.
====
...
[source]
----
# Import runbook (example)

## Prerequisites

- [ ] Your relational source or CSV files available (or Northwind sample)
...
This ensures you execute queries in the correct dependency order and can locate maintenance queries when needed.
...
In the next lesson, you can run an optional challenge to import your own CSV end-to-end, or proceed to the next module to validate your imported graph.
