...
Comparing query performance: SQL JOINs vs Cypher traversals. Examples use a Northwind-style model; the same comparison applies to your own graph.
...
**Cost:** The index finds the customer quickly; then the database uses the index or scans the orders table to find matching rows. Cost grows with the number of matching orders.
...
**Cost:** The index finds the customer quickly; then the query follows PLACED relationship pointers to each order. Cost grows with the number of relationships followed, not with total table size.
...
**Cost:** Multiple index lookups and join operations across the four tables.
...
**Cost:** One index lookup, then following relationship pointers hop by hop. Each hop is a direct pointer follow; cost depends on how many relationships are traversed, not on total data size.
...
**Cost:** Grows with the number of employees and the depth of the hierarchy, as the recursive query iterates level by level.
...
**Cost:** Depends on the actual path length; the query follows REPORTS_TO pointers hop by hop.
...
== Query comparison: UPDATE and INSERT

Besides SELECT-style reads, you may need to update or insert data. Side-by-side:

**SQL UPDATE:** `UPDATE customers SET city = 'Berlin' WHERE customer_id = 'ALFKI';`  
**Cypher:** `MATCH (c:Customer {customerID: 'ALFKI'}) SET c.city = 'Berlin';`

**SQL INSERT (new row):** `INSERT INTO orders (order_id, customer_id, order_date) VALUES (99999, 'ALFKI', '2024-01-15');`  
**Cypher (new node):** `CREATE (o:Order {orderID: 99999, orderDate: date('2024-01-15')});` then create the relationship: `MATCH (c:Customer {customerID: 'ALFKI'}), (o:Order {orderID: 99999}) CREATE (c)-[:PLACED]->(o);`

In both systems, updates and inserts are single-statement or short scripts; use transactions for multi-step writes. Cypher has no separate "INSERT" keyword—you `CREATE` nodes and relationships or `MERGE` when you want upsert semantics.
...
====
Bookmark this lesson. The performance comparison table summarizes when to use graph traversals versus when SQL-style aggregations may be more appropriate. This guidance applies to any graph you import from your own relational data.
====
