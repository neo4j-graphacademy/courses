...
Before getting into the mapping, the following section addresses common misconceptions that frequently cause problems during migration:
...
**Example:** If you have a `status_id` foreign key pointing to a `statuses` table with values like "Pending", "Shipped", "Delivered", you might choose to store `status` as a property on the Order node rather than creating a separate Status node and relationship.
====
...
**Foreign Key:** `orders.employee_id` → `employees.employee_id`
...
Alternatively, you could model this as `(Order)-[:PROCESSED_BY]->(Employee)`. Choose based on your most common query patterns.
...
**Foreign Key:** `orders.ship_via` → `shippers.shipper_id`
...
**Foreign Key:** `order_details.order_id` → `orders.order_id` and `order_details.product_id` → `products.product_id`
...
**Foreign Key:** `employees.reports_to` → `employees.employee_id`
...
include::questions/2-misconceptions.adoc[leveloffset=+1]
