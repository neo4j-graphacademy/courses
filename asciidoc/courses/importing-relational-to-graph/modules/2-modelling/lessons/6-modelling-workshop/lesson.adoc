...
Your challenge is to design a graph model by working through the Northwind schema step by step using the Neo4j Data Importer in Aura.
...
The Data Importer accepts local files only. Download the Northwind CSV files from link:https://github.com/neo4j-graph-examples/northwind/tree/main/import[Northwind CSV files^] (if a zip archive is available, download it and unzip locally).
