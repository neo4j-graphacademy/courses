...
.Optional Lesson
====
This lesson is an optional hands-on workshop. You can skip it and continue to the next module, or use it to practice and reinforce your graph modelling skills.
====

In this workshop, you will practice designing a graph model by working through the Northwind schema step by step. You can use either Arrows.app or the Neo4j Data Importer in Aura.
...
. Log in to link:https://console.neo4j.io[Neo4j Aura^]
...
. Enter a label name (e.g., `Customer`)
...
. In the properties list, find the ID property (e.g., `customerID`)
...
. Enter the relationship type (e.g., `PLACED`)
