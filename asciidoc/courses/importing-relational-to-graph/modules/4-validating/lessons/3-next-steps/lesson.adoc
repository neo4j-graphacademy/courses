...
== Apply This to Your Own PostgreSQL Data

If you want to migrate your own PostgreSQL database to Neo4j, follow these steps:
...
. **Categorize your tables** using the framework from Lesson 2.2:
   ...
   * Lookup tables → Properties or small nodes
...
. **Define node labels** - Use singular PascalCase names (e.g., `Customer`, `Order`)
. **Define relationship types** - Use UPPER_SNAKE_CASE verbs (e.g., `PLACED`, `CONTAINS`)
...
. **Plan import order** - Follow the dependency chain:
   .. Independent nodes (no foreign keys)
   .. Dependent nodes (reference other nodes)
   .. Relationships


=== Phase 4: Execute the Import

. **Connect your PostgreSQL database** to the Data Importer (see Lesson 1.2)
...
* [ ] PostgreSQL connected to Data Importer
...
* **Lesson 4.2: SQL vs Cypher: Query Efficiency Comparison** - The performance comparison summary table
