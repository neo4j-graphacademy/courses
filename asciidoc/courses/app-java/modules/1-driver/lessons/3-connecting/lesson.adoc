...
The second argument will be an authentication token which can be created using the `AuthTokens.basic()` function, this takes two arguments; the username and password variables returned from the `getNeo4jUsername()` and `getNeo4jPassword()` methods.
