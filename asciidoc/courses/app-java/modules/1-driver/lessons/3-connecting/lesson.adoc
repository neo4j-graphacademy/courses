...
// , and above a `static Driver driver` static variable declaration
...
This function should use the `getUri`, `getUsername` and `getPassword` methods to create an instance of the Neo4j Java Driver, verify connectivity and return it.

////
and assign it to the `driver` variable.
...
Your first challenge is to modify the `initDriver` in `src/AppUtils.java` to create an instance of the driver and return it.
// and assign it to the `driver` variable declared above.
