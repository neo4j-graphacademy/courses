...
This function should use the `uri`, `username` and `password` parameters supplied to create an instance of the Neo4j Java Driver and assign it to the `driver` variable.
...
You have implemented the code to create a new driver instance with environment variables read from the `application.properties` file which verifies that the driver can connect to the DBMS before the server starts.
