...
3. Create the driver instance with the  `getNeo4jUri`, `getNeo4jUsername` and `getNeo4jPassword` methods and use the `driver.verifyConnectivity` to assert that the credentials are correct.
...
The second argument will be an authentication token which can be created using the `AuthTokens.basic()` function, this takes two arguments; `getNeo4jUsername` and `getNeo4jPassword` from System properties.
