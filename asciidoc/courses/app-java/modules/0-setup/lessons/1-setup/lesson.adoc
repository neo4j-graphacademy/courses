...
When the `config()` function exported by the `dotenv` library is called, the `application.properties` file in the root of the project is parsed and all settings are appended to the `processapplication.properties` variable.
