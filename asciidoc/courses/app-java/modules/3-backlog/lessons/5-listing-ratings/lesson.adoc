...
The list of ratings is populated by the `forMovie()` method in the link:{repository-blob}/src/main/java/neoflix/services/RatingService.java[`RatingService`^], which currently returns a hardcoded list of ratings from the fixture `ratings.json`.
...
[source,cypher]
----
include::{repository-cypher-raw}/3-backlog/5-listing-ratings/listing-ratings.cypher[]
----
