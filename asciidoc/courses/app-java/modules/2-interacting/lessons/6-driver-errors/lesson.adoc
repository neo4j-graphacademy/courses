...
In the Neo4j Java Driver, the Promise returned by the `run()` method will be rejected, and an instance of a link:https://neo4j.com/docs/api/java-driver/current/org/neo4j/driver/exceptions/Neo4jException.html[`Neo4jError`^] will be returned.

The `Neo4jError` class extends the native Java `RuntimeException`, and as such contains a `message` property that contains detailed information about the error that has occurred.

== Error Codes

The `Neo4jError` also includes a `cause` property, which provides higher-level information about the query.
...
Below is an example of how to catch an error thrown by the Driver. It is best practice to use `try` and `catch` blocks to catch any errors.

.Example
[source,java,subs="attributes+",indent=0]
----
include::{repository-raw}/main/src/main/java/example/CatchErrors.java[tag=catch]
----
