...
In the Neo4j Java Driver, the Promise returned by the `run()` method will be rejected, and an instance of a link:https://neo4j.com/docs/api/java-driver/current/class/lib6/error.js~Neo4jError.html[`Neo4jError`^] will be returned.

The `Neo4jError` class extends the native Java `Error`, and as such contains a `message` property that contains detailed information about the error that has occurred.
...
----
include::{repository-raw}/main/src/main/java/example/catch-errors.java[tag=catch]
----
...
[source,java,subs="attributes+",indent=0]
----
include::{repository-raw}/main/src/main/java/example/catch-errors.java[tag=promise]
----
