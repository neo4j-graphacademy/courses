...
1. The server directs the request to the route handler in link:{repository-blob}/main/src/main/java/neoflix/routes/AccountRoutes.java[`src/main/java/neoflix/routes/AccountRoutes.java`^], whose `/ratings/` route gets the authenticated user's id from the request.
2. The route handler uses an instance of the link:{repository-blob}/main/src/main/java/neoflix/services/RatingService.java[`RatingService`^].
