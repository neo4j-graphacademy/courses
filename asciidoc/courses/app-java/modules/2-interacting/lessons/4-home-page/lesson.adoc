...
The Cypher query itself is similar to this one, only that the property that's ordered by will be dynamically inserted based on user interaction from the API query parameters.

It fetches movies that have the sort-property set and adds the favorite flag (here defaulting to false) and returns a page based on limit and skip parameters.
...
This session then provides a `readTransaction()` method for which you pass a callback/lambda to represent the unit of work.
...
1. The Cypher query as a string, using query parameters (placeholders prefixed with `$`).
...
NOTE: Two of the parameters, namely the sort field and sort order cannot be provided as Cypher query parameters, so we need to use string substitution with `%s` instead, which we would love to avoid.
