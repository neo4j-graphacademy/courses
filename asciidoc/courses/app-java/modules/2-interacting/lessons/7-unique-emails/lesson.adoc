...
In the link:../5-registering[Registering a User challenge^], you updated the `authenticate()` method in the link:{repository-blob}main/src/main/java/neoflix/services/AuthService.java[`AuthService`^] to create a new User node in the database.

There is still one `// TODO` comment remaining in the application.
...
If we take a look at `register()` method, it has been hardcoded to throw a new `RuntimeException` if the email address is anything other than `graphacademy@neo4j.com`.

.src/main/java/neoflix/services/AuthService.java
[source,java,subs="attributes+",indent=0]
----
include::{repository-raw}/{previous-branch}/src/main/java/neoflix/services/AuthService.java[tag=register]
----
...
----
//MH: In Java, do we need a custom error or just work with the Runtime Exception and custom message?
...
//Adam: need to complete this sentence.
...
//MH: Finally not needed because the session auto-closes, right?
