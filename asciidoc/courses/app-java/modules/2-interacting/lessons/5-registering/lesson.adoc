...
The dummy register logic is already written into the `register()` method of the `AuthService` at `src/main/java/neoflix/services/AuthService.java`.  As we can see from the snippet below, at the moment, it loads the fixture from `users.json` and will only accept an email address of `graphacademy@neo4j.com`.
