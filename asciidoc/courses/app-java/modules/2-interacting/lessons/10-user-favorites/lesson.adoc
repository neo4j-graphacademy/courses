...
1. The server directs the request to the route handler in link:{repository-blob}/src/main/java/neoflix/routes/AccountRoutes.java[`AccountRoutes`^], which verifies the user's JWT token and stores it in the request, before handling the request
2. The route handler uses an instance of the link:{repository-blob}/src/main/java/neoflix/services/FavoriteService.java[`FavoriteService`^].
...
In a write transaction, run a Cypher query to create the `HAS_FAVORITE` relationship between the `User` and `Movie` nodes, with a `createdAt` property to represent when the relationship was created.
...
[source,cypher]
----
include::{repository-cypher-raw}/2-interacting/9-user-favorites/creating-the-relationship.cypher[]
----
====
...
----
include::{repository-cypher-raw}/2-interacting/9-user-favorites/deleting-the-relationship.cypher[]
----
...
[source,cypher]
----
include::{repository-cypher-raw}/2-interacting/9-user-favorites/listing-favorites.cypher[]
----
====
