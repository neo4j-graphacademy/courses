...
1. A route handler `login` in link:{repository-raw}/main/src/main/java/neoflix/routes/AuthRoutes.java[`src/main/java/neoflix/routes/AuthRoutes.java`^] listens for a `POST` request.
2. The `login` route calls the `authenticate()` method in the link:{repository-raw}/main/src/main/java/neoflix/services/AuthService.java[`AuthService`^] with the username and password from the request.
