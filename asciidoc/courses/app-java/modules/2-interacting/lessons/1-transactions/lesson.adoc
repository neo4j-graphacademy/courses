...
.Open a new Session
[source,java,role=nocopy,subs="attributes+"]
----
include::{repository-raw}/main/src/main/java/example/Index.java[tag="driver.session",indent=0]
----
...
[source,java,role=nocopy,subs="attributes+"]
----
include::{repository-raw}/main/src/main/java/example/Index.java[tag="session",indent=0]
----
...
[source,java,role=nocopy,subs="attributes+"]
----
include::{repository-raw}/main/src/main/java/example/Index.java[tag=sessionWithArgs,indent=0]
----
...
----
include::{repository-raw}/main/src/main/java/example/Index.java[tag=session.run]
----
...
The session provides a `readTransaction()` method, which expects a single parameter, a callback function that represents the unit of work.
The function will accept a single parameter, a Transaction object, on which you can call the `tx.run()` method with two arguments: the Cypher query as a string and an optional set of query parameters.

.Running a Read Transaction
[source,java,role=nocopy,subs="attributes+",indent=0]
----
include::{repository-raw}/main/src/main/java/example/Index.java[tag=session.readTransaction]
----

[TIP]
.Parameterized Queries
In the query above, the `$` prefix of `$title` (1) indicates that this value relates to the parameter defined in the second argument (2) of the `run()` method call.
...
If anything goes wrong within of the unit of work or there is a problem on Neo4j's side, the read-transaction will be automatically rolled back and the database will remain in its previous state.
...
[source,java,role=nocopy,subs="attributes+",indent=0]
----
include::{repository-raw}/main/src/main/java/example/Index.java[tag=session.writeTransaction,indent=0]
----
...
[source,java,role=nocopy,subs="attributes+"]
----
include::{repository-raw}/main/src/main/java/example/Index.java[tag=session.beginTransaction,indent=0]
----
...
[source,java,role=nocopy,subs="attributes+",indent=0]
----
include::{repository-raw}/main/src/main/java/example/Index.java[tag=session.beginTransaction.Try]
----
...
[source,java,role=nocopy,subs="attributes+",indent=0]
----
include::{repository-raw}/main/src/main/java/example/Index.java[tag=session.close]
----
...
[source,java,role=nocopy,subs="attributes+",indent=0]
----
include::{repository-raw}/main/src/main/java/example/Index.java[tag=createPerson]
----
====
