...
As Neo4j is written in Java (the _j_ in Neo4**j** stands for Java after all), there are some discrepencies between the types stored in the Neo4j database and native Java types.

Some values like strings, floats, booleans, and nulls have a direct mapping to Java types but more complex types need special handling.
...
Due to a discrepancy between integers in Java and in the Neo4j type system, we need to take extra care when working with integers in the Neo4j Java Driver.

The Neo4j type system uses 64-bit signed integer values (with a range of `-(2^64^- 1)` and `(2^63^- 1)`) while Java can only safely represent integers between `-(2^53^- 1)` (`Number.MIN_SAFE_INTEGER`) and `2^53^- 1` (`and Number.MAX_SAFE_INTEGER`).

In order to conform with the Neo4j type system, the driver will not automatically convert to Java integers. It will convert them to a Float.  This ensures that no data is lost.
...
You can call the `toNumber()` method on this object to convert it back into a Java number.
...
----
include::{repository-raw}/main/example/results.java[tag=integers]
----
...
Each of these types has a `toString()` method that can be used in conjunction with `Date.parse()` in Java to convert the Cypher date into a native `Date` object.


.Working with Temporal types
[source,java,role=ncopy,subs="attributes+",indent=0]
----
include::{repository-raw}/main/example/results.java[tag=temporal]
----
...
[source,java,role=ncopy,subs="attributes+",indent=0]
----
include::{repository-raw}/main/example/results.java[tag=run]
----
...
[source,java,role=ncopy,subs="attributes+",indent=0]
----
include::{repository-raw}/main/example/results.java[tag=get]
----

The value assigned to the `person` variable will be the instance of a `Node`.
Node is a type provided by the Neo4j Java Driver to hold the information held in Neo4j for the node.
...
[source,java,role=ncopy,subs="attributes+",indent=0]
----
include::{repository-raw}/main/example/results.java[tag=node]
----
...
2. `labels` - an Array of String values, one per label stored against the node. +
    eg. `['Person', 'Actor']`
3. `properties` - A Java object containing all the properties for the node. +
...
[source,java,role=ncopy,subs="attributes+",indent=0]
----
include::{repository-raw}/main/example/results.java[tag=rel]
----
...
[source,java,role=ncopy,subs="attributes+",indent=0]
----
include::{repository-raw}/main/example/results.java[tag=path]
----
...
For this purpose, We have added a utility function to `src/utils.js` which when given an Object of properties, will iterate through the keys and where appropriate convert the value to a native Java value.
...
----
include::{repository-raw}/main/src/utils.java[tag=toNativeTypes]
----
====
