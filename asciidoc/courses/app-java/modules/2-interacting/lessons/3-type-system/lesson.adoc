...
Cypher itself has functions like `toFloat()` and `toInteger()` respectively and parameters of other types are automatically coerced.
...
These numbers can be re-used, a best practice is to always look up a node by its business key and label rather than relying on an internal ID.

=== Relationships

`Relationship` objects are also `Entity` instances, they also include an `id` and properties.
...
* `asObject()` recursively converts a `Value` into the appropriate Java object
