...
In the previous lesson, you learned about Aura's snapshot system and when to use scheduled vs. on-demand snapshots.
...
**Why these actions matter:** Each action serves a different purpose:
...
**Why export for your recommendation engine:** Export snapshots regularly to preserve your movie dataset and recommendation model beyond Aura's retention period. This ensures you can recover your recommendation engine even if you need to recreate your instance.
...
**Why this helps your recommendation engine:** Create a test instance from a snapshot to:
...
**Why this matters:** These snapshot management skills help you:
* Protect your recommendation engine's data
* Create test environments with real data
* Meet compliance requirements for data retention

**How it works:** Snapshots are point-in-time copies stored in encrypted cloud storage. You can export them, create new instances from them, or restore your current instance to them.
