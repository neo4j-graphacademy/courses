...
This is also useful for creating staging environments for application testing, or as part of disaster recovery scenarios.
...
In this lesson, you learned about Aura's backup and restore capabilities, including scheduled vs on-demand snapshots, exporting for long-term storage, and restoring from snapshots or local files.
