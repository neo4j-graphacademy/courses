...
* **Chose the right approach**: Learned when to use scheduled vs. on-demand snapshots

**Why this matters:** Snapshots protect your recommendation engine's data. Scheduled snapshots provide automatic protection, while on-demand snapshots let you create restore points before risky operations.

**How it works:** Aura automatically creates snapshots based on your tier schedule and stores them in encrypted cloud storage. You can also create snapshots manually before important changes.
