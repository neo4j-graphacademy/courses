...
**Why use Data Importer for your recommendation engine?** Instead of writing complex import scripts, visually map your movie CSV data to graph nodes (Movie, Person) and relationships (ACTED_IN). This ensures your data model supports recommendation queries from the start.
...
**Why this matters:** Your recommendation engine needs data about movies, actors, and their relationships. The CSV file contains this information in a tabular format that we'll transform into a graph. Proper data preparation ensures a smooth import and an effective graph model.
...
**Why CSV format:** CSV files are easy to work with and commonly used for data imports. The Data Importer reads the CSV structure and helps you map it to graph nodes and relationships.
...
**What you're seeing:** The Data Importer displays your CSV columns (movieId, title, personId, name, characters) and sample data rows. This preview helps you understand what data you're working with before creating your graph model.
...
**What this video shows:** How to run the import process after connecting to your Aura instance and configuring your data model.

video::https://cdn.graphacademy.neo4j.com/courses/aura-fundamentals/run-import.mp4["Run Import",role="cdn", width=100%]

**What happened:** The video demonstrated executing the import. The Data Importer processes your CSV data and creates nodes and relationships in your Neo4j instance. After the import completes, you'll see a summary of what was created.
...
**Why this matters:** If the counts look correct, your recommendation engine has the data it needs. If something seems off (e.g., zero relationships), your model mapping might need adjustment.
...
**What this video shows:** How to read and interpret the import summary to verify your data was loaded correctly.
...
**Why save the model:** If you need to import more movie data later or recreate the structure in another instance, the saved model lets you reuse the same mapping without rebuilding it.
...
**Why this matters for recommendations:** The graph structure you created (Person -[:ACTED_IN]-> Movie) enables queries like:
