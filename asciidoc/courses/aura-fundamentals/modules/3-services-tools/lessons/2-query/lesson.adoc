...
[.slide.discrete]
== Querying data for recommendations

In the previous lesson, you ran the import and verified your movie data was loaded into your Aura instance, creating a graph with Person and Movie nodes connected by ACTED_IN relationships.

**Your mission in this lesson:** Write Cypher queries that find movie recommendations by traversing relationships. You'll learn the query patterns that power recommendation engines—finding movies with the same actors, discovering co-actors, and identifying similar films.

In this lesson, you will learn how to:

* Connect to your Aura instance using the Query tool
* Write Cypher queries that traverse relationships to find recommendations
* Visualize query results to understand actor-movie connections
* Use query patterns that power recommendation systems
...
**Why the Query tool:** Cypher is the language for traversing relationships in your graph. The Query tool lets you write and test recommendation queries interactively.
...
**What you'll build:** A query that finds all movies an actor appeared in—the foundation of actor-based recommendations.
...
**Why this matters for recommendations:** This query pattern (find all movies for an actor) is the first step in recommendation logic. Once you know a user likes Tom Hanks movies, you can find all his films.
...
**What you're seeing:** The query returns a graph visualization showing Tom Hanks (Person node) connected to multiple Movie nodes via ACTED_IN relationships. Each connection represents a movie he acted in.

**Why graph visualization matters:** Seeing the connections visually helps you understand recommendation patterns. You can see how many movies an actor appeared in and identify clusters of related films.
...
**How it works:** This traverses from a movie through actors to other movies. Movies with more shared actors are more similar—exactly what recommendation engines need.

**Why these patterns matter:** These three query patterns form the foundation of recommendation systems:
...
* **Why it helps recommendations**: Visual patterns reveal which movies share actors, making it easy to spot recommendation opportunities
...
* **Why it helps recommendations**: Shows ranked results—movies with more shared actors are better recommendations
...
* **Why it helps recommendations**: Raw data helps you understand query results before building recommendation APIs
...
In this lesson, you learned how to write Cypher queries that power your recommendation engine. You:
...
**Why this matters:** These query patterns form the foundation of recommendation systems. By traversing relationships (Person -[:ACTED_IN]-> Movie), you can find movies with shared actors, discover co-actor networks, and identify similar films—exactly what recommendation engines need.

**How it works in the background:** Cypher's `MATCH` clause finds paths through your graph by following relationships. When you write `(p:Person)-[:ACTED_IN]->(m:Movie)`, Neo4j's query engine efficiently traverses all ACTED_IN relationships to find matching patterns. This is why graph databases excel at recommendations—they're optimized for relationship traversal.
