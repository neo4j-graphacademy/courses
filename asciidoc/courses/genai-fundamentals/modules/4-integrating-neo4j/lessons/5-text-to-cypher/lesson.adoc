...
To answer more specific question, you may need to perform more complex queries to find data relating to specific nodes, relationships, or properties.

You want to find the age of an actor, who acted in a movie, or determine movie recommendations based on rating.
...
Text to Cypher retrievers allow you to convert natural language queries into Cypher queries that can be executed against the graph.
    ...
    MATCH (m:Movie) WHERE m.title = 'Babe' 
    RETURN m.released
