...
export default function saveAnswer(
    ...
    return writeTransaction(async (tx: ManagedTransaction) => {
            ...
            `
              ...
              SET
                ...
                r.correct = size($answers) = size(q.correct)
                  AND all(answer in $answers where apoc.text.clean(answer) in [ans in q.correct | apoc.text.clean(ans)])
                  AND all(answer in q.correct where apoc.text.clean(answer) in [ans in $answers | apoc.text.clean(ans)])
              ...
              RETURN
                ...
                all(answer in $answers where apoc.text.clean(answer) in [ans in q.correct | apoc.text.clean(ans)]) AS a,
                all(answer in q.correct where apoc.text.clean(answer) in [ans in $answers | apoc.text.clean(ans)]) AS b,
        ...
        if (status.attemptId && shouldContinue === false) {
            ...
            if (res.passed) {
                emitter.emit(new UserCompletedCourse(user, status.course as AbridgedCertification, res.source))
            } else {
                emitter.emit(new UserFailedCertification(user, status.course as AbridgedCertification, res.source))
            }
