...
export async function createAttempt(
...
): Promise<{ enrolmentId: string; attemptId: string; questions: number }> {
    ...
    const attemptRes = await tx.run<{ id: string; questions: number }>(
      ...
      CALL {
        ...
        WITH a, c, weights, category, [ idx in range(0, coalesce(take, 0)-1) WHERE take > 0 | questions[level][idx] ] AS selected
...
export default async function startCertification(
    slug: string,
    user: User,
    ref: string | undefined,
    team: string | undefined
): Promise<StartCertificationOutput> {
    return writeTransaction<StartCertificationOutput>(async (tx) => {
