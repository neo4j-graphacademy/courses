...
import {
  ...
  ATTRIBUTE_CLASSMARKER_ID,
  ATTRIBUTE_CLASSMARKER_REFERENCE,
  ATTRIBUTE_DURATION,
  ATTRIBUTE_KEY_POINTS,
  ATTRIBUTE_LANGUAGE,
  ATTRIBUTE_NEXT,
  ATTRIBUTE_PREREQUISITES,
  ATTRIBUTE_REDIRECT,
  ATTRIBUTE_REPOSITORY,
  ATTRIBUTE_REWARD_FORM,
  ATTRIBUTE_REWARD_IMAGE,
  ATTRIBUTE_REWARD_PRODUCT_ID,
  ATTRIBUTE_REWARD_PROVIDER,
  ATTRIBUTE_REWARD_TYPE,
  ATTRIBUTE_STATUS,
  ATTRIBUTE_THUMBNAIL,
  ATTRIBUTE_TRANSLATIONS,
  ATTRIBUTE_USECASE,
  ATTRIBUTE_DATABASE_PROVIDER,
  ATTRIBUTE_VIDEO,
  COURSE_DIRECTORY,
  ...
  DEFAULT_LANGUAGE,
  Language,
  ATTRIBUTE_GRAPH_ANALYTICS_PLUGIN,
  ATTRIBUTE_VECTOR_OPTIMIZED,
} from "../../constants";
import { existsSync } from "fs";
import { computeFileHash, courseOverviewPath } from "../../utils";
...
const loadCourse = (slug: string): Promise<CourseToImport> => {
  ...
  const categories = file
    .getAttribute(ATTRIBUTE_CATEGORIES, "")
    .split(",")
    .map((e: string) => e?.trim() || "")
    .filter((e: string) => e !== "")
  ...
  const progressToSlugs = file
    .getAttribute(ATTRIBUTE_NEXT, "")
    .split(",")
    .map((e: string) => e?.trim() || "")
    .filter((e: string) => e !== "");
  ...
  const repositories = Object.fromEntries(
    Object.entries(file.getAttributes()).filter(([key]) =>
      key.endsWith("repository"),
    ),
  ) as Record<string, string>;

  const language = file.getAttribute(ATTRIBUTE_LANGUAGE, DEFAULT_LANGUAGE);
  const translationSlugs = file
    .getAttribute(ATTRIBUTE_TRANSLATIONS, "")
    .split(",")
    .filter((e: string) => e !== "");

  // Certification?
  const certification =
    file.getAttribute(ATTRIBUTE_CERTIFICATION, "false") === "true";
  ...
  const branch =
    repository !== undefined
      ? file.getAttribute(ATTRIBUTE_BRANCH, "main")
      : undefined;
  ...
  return Promise.resolve({
    slug,
    hash: computeFileHash(courseOverviewPath(slug)),
    link: `/courses/${slug}/`,
    language,
    translationSlugs,
    ...
    vectorOptimized:
      file.getAttribute(ATTRIBUTE_VECTOR_OPTIMIZED, null) === "true",
    graphAnalyticsPlugin:
      file.getAttribute(ATTRIBUTE_GRAPH_ANALYTICS_PLUGIN, null) === "true",
    ...
    classmarkerReference: file.getAttribute(
      ATTRIBUTE_CLASSMARKER_REFERENCE,
      null,
    ),
    attributes: {
...
export default async function loadCourses(): Promise<CourseToImport[]> {
  const courses = await readdir(COURSE_DIRECTORY).then((courses) =>
    courses.filter((slug) => existsSync(courseOverviewPath(slug))),
  );
