...
async function loadModule(
  course: CourseToImport,
  slug: string,
): Promise<ModuleToImport> {
  const file = loadFile(
    join(COURSE_DIRECTORY, course.slug, "modules", slug, "module.adoc"),
  ...
  return {
    course,
    slug,
    hash: computeFileHash(moduleOverviewPath(course.slug, slug)),
...
export default async function loadModules(
  ...
  const filtered = courses.filter((course) =>
    existsSync(join(COURSE_DIRECTORY, course.slug, "modules")),
  );

  for (const course of filtered) {
    const path = join(COURSE_DIRECTORY, course.slug, "modules");
    const modules = await readdir(path).then((slugs) =>
      slugs.filter((slug) => existsSync(moduleOverviewPath(course.slug, slug))),
    );
