...
describe('createAndSaveSandbox', () => {
    ...
    it('should return undefined if no usecase is defined', async () => {
        ...
        expect(await createAndSaveSandbox(token, noUseCase)).toEqual(undefined)
    })


    it('should throw an error if token is invalid', async () => {
        expect(() => createAndSaveSandbox('', course)).rejects.toThrow()
    })

    it('should create a new sandbox', async () => {
        // Terminate existing if one already exists
        const existing = await getSandboxForUseCase(token, course.usecase!)

        if ( existing !== undefined ) {
            await stopSandbox(token, existing.sandboxHashKey)
        }

        // Attempt to create
        const output = await createAndSaveSandbox(token, course)
    ...
    it('should use an existing sandbox if it exists', async () => {
        // Attempt to create
        const first = await createAndSaveSandbox(token, course)

        expect(first).toBeDefined()

        // Should use existing sandbox
        const second = await createAndSaveSandbox(token, course)
