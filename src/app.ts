...
app.get("/:course", async (req, res) => {
  ...
  const result = await read(`MATCH (c:Course {slug: $course}) RETURN c`, {
  ...
  const css = readFileSync(
    join(__dirname, "..", "resources", "css", "banner.css")
  );
...
app.get("/:course/slides/images/:filename", (req, res) => {
  try {
    const filename = req.params.filename;
    const courseModulesPath = join(
      __dirname,
      "..",
      "asciidoc",
      "courses",
    ...
    // Search for the file in any images folder within the course modules
    const findImageFile = (dir: string): string | null => {
      try {
        ...
        for (const item of items) {
          ...
          if (statSync(itemPath).isDirectory()) {
            if (item === "images") {
              // Check if the file exists in this images folder
              ...
              const found = findImageFile(itemPath);
    ...
    const filePath = findImageFile(courseModulesPath);
    if (filePath) {
...
app.get("/:course/slides", async (req, res, next) => {
  try {
    ...
    const illustration = readFileSync(
      courseIllustrationPath(req.params.course)
    ).toString();
    ...
    for (const module of readdirSync(moduleFolder)) {
      if (statSync(join(moduleFolder, module)).isDirectory()) {
        ...
        for (const lesson of readdirSync(
          join(moduleFolder, module, "lessons")
        )) {
          if (
            statSync(
              join(moduleFolder, module, "lessons", lesson)
            ).isDirectory()
          ) {
            const lessonFile = loadFile(
              join(moduleFolder, module, "lessons", lesson, "lesson.adoc"),
              { attributes }
            );
    ...
    res.render("slides", {
      ...
      illustration,
      css: readFileSync(
        join(__dirname, "..", "resources", "css", "slides.css")
      ),
      modules,
    });
  } catch (e: any) {
