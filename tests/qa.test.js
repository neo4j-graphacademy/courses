        ...
        if (status === 'active' && certification !== 'true') {
            describe(slug, () => {
                ...
                it('should have key-points with 3-5 items', () => {
                    ...
                    const points = keyPoints.split(',').map(e => e.trim()).filter(e => e !== '')
                    ...
                    it('should have valid GitHub repository if repository is defined', async () => {
                        ...
                        if (repository) {
                            ...
                            try {
                                ...
                                throw new Error(`Repository ${repository} does not exist on GitHub (${url} returns ${statusCode})`)
                            }
                        }
                    }, 10000)
                }
                ...
                it('should have a banner image', () => {
                    const bannerPath = join(__dirname, '..', 'asciidoc',
                        'courses', slug, 'banner.png')
                ...
                for (const modulePath of modulePaths) {
                    ...
                    describe(moduleSlug, () => {
                        ...
                        it('linked lesson should be an existing lesson', () => {
                            const linkMatch = moduleAdoc.match(/link:\.\/([^\/\[]+)\/\[/)
                            const lessonSlug = linkMatch[1]
                            const lessonAdocPath = join(modulePath, 'lessons', lessonSlug, 'lesson.adoc')
                        ...
                        for (const lessonPath of lessonPaths) {
                            ...
                            describe(lessonSlug, () => {
                                ...
                                it('should have a valid :type: attribute if defined', () => {
                                    ...
                                    if (type) {
                                        const validTypes = ['video', 'lesson', 'text', 'quiz', 'activity', 'challenge']
